...
! All variable in the namelist group: cam_inparm, useless variables
    ...
    character(len=256) :: ncdata, absems_data, cam_branch_file, bnd_topo, efield_hflux_file, efield_lflux_file,   &
    ...
    namelist /cam_inparm/ phys_alltoall, phys_chnk_per_thd, phys_loadbalance, phys_twin_algorithm, nlvdry, iradae,&
                          iradlw, irad_always, rayk0
    ...
    namelist /cam_inparm/ fincl1, fincl2, fincl3, fincl4, fincl5, fincl6, fwrtpr1, fwrtpr2, fwrtpr3, fwrtpr4,     &
                          fwrtpr5, fwrtpr6
    ...
    namelist /cam_inparm/ ncdata, absems_data, cam_branch_file, bnd_topo, efield_hflux_file, efield_lflux_file,   &
                          efield_wei96_file, qbo_forcing_file
    namelist /cam_inparm/ hfilename_spec
    namelist /cam_inparm/ print_energy_errors, empty_htapes, inithist_all, readtrace, use_64bit_nc,               &
                          print_step_cost, pbuf_global_allocate, spectralflux, scm_crm_mode, scm_diurnal_avg,     &
                          scm_iop_srf_prop, scm_relaxation, tracers_flag, nlte_use_mo, qbo_cyclic, qbo_use_forcing
...
! Get Radiation Step from Namelist
    ...
    if (.not. initialized) then
        ...
        if (ierr .ne. 0) then
            ...
            ierr = 1
            do while (ierr .ne. 0)
                read(unitn, nml = cam_inparm, iostat = ierr)
                if (ierr .ne. 0) then
