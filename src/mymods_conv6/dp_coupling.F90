...
!xiex--
    real(r8), allocatable :: vort3(:, :, :)       ! vorticity on a-grid
    real(r8), allocatable :: t3(:, :, :)       ! temperature on a-grid
    real(r8), allocatable :: tdiff3(:, :, :)       ! temperature on a-grid
    real(r8), allocatable :: q3(:, :, :)       ! temperature on a-grid
    real(r8), allocatable :: qdiff3(:, :, :)       ! temperature on a-grid
    real(r8), pointer :: cosp(:),sinp(:)  ! Sine and cosine in longitude
    real(r8) :: dlon,dlat  ! Sine and cosine in longitude
    real(r8) :: dx,dy      ! Sine and cosine in longitude
!--
...
!xiex--
    ...
    allocate (vort3(ifirstxy:ilastxy, km, jfirstxy:jlastxy))
    allocate (t3(ifirstxy:ilastxy, km, jfirstxy:jlastxy))
    allocate (tdiff3(ifirstxy:ilastxy, km, jfirstxy:jlastxy))
    allocate (q3(ifirstxy:ilastxy, km, jfirstxy:jlastxy))
    allocate (qdiff3(ifirstxy:ilastxy, km, jfirstxy:jlastxy))
    t3 = 0.
    tdiff3 = 0.
    q3 = 0.
    qdiff3 = 0.
    ...
    do j=jfirstxy,jlastxy
        do i=ifirstxy,ilastxy
            do k=1,km
                t3(i,k,j) = ptxy(i,j,k) / (D1_0 + zvir*tracer(i,j,k,1))
                q3(i,k,j) = tracer(i,j,k,1)
            end do
        end do
    end do
!--
...
!corner
        i = ifirstxy
        j = jfirstxy
        dx = dlon*rearth*cosp(j)
!          vort3(i,k,j) = dx
        if (dx == 0.) then
            vort3(i,k,j) = 0.
        else
            vort3(i,k,j) = -(u3(i,k,j+1) - u3(i,k,j))/dy +( v3(i+1,k,j) - v3(i,k,j) )/dx
            tdiff3(i,k,j) = sqrt( ((t3(i,k,j+1) - t3(i,k,j))/dy)**2 &
                +((t3(i+1,k,j) - t3(i,k,j))/dx)**2 )
            qdiff3(i,k,j) = sqrt( ((q3(i,k,j+1) - q3(i,k,j))/dy)**2 &
                +((q3(i+1,k,j) - q3(i,k,j))/dx)**2 )
        end if
...
!          vort3(i,k,j) = dx
        if (dx == 0.) then
            ...
            vort3(i,k,j) = -(u3(i,k,j) - u3(i,k,j-1))/dy +( v3(i+1,k,j) - v3(i,k,j) )/dx
            tdiff3(i,k,j) = sqrt( ((t3(i,k,j) - t3(i,k,j-1))/dy)**2 &
                +((t3(i+1,k,j) - t3(i,k,j) )/dx)**2 )
            qdiff3(i,k,j) = sqrt( ((q3(i,k,j) - q3(i,k,j-1))/dy)**2 &
                +((q3(i+1,k,j) - q3(i,k,j) )/dx)**2 )
        end if
...
!          vort3(i,k,j) = dx
        if (dx == 0.) then
            ...
            vort3(i,k,j) = -(u3(i,k,j+1) - u3(i,k,j))/dy +( v3(i,k,j) - v3(i-1,k,j) )/dx
            tdiff3(i,k,j) = sqrt( ((t3(i,k,j+1) - t3(i,k,j))/dy)**2 &
                +((t3(i,k,j) - t3(i-1,k,j))/dx)**2 )
            qdiff3(i,k,j) = sqrt( ((q3(i,k,j+1) - q3(i,k,j))/dy)**2 &
                +((q3(i,k,j) - q3(i-1,k,j))/dx)**2 )
        end if
...
!          vort3(i,k,j) = dx
        if (dx == 0.) then
            ...
            vort3(i,k,j) = -(u3(i,k,j) - u3(i,k,j-1))/dy +( v3(i,k,j) - v3(i-1,k,j) )/dx
            tdiff3(i,k,j) = sqrt( ((t3(i,k,j) - t3(i,k,j-1))/dy)**2 &
                +((t3(i,k,j) - t3(i-1,k,j) )/dx)**2 )
            qdiff3(i,k,j) = sqrt( ((q3(i,k,j) - q3(i,k,j-1))/dy)**2 &
                +((q3(i,k,j) - q3(i-1,k,j) )/dx)**2 )
...
!              vort3(i,k,j) = dx
            if (dx == 0.) then
                ...
                vort3(i,k,j) = -(u3(i,k,j+1) - u3(i,k,j))/dy +0.5*( v3(i+1,k,j) - v3(i-1,k,j) )/dx
                tdiff3(i,k,j) = sqrt( ((t3(i,k,j+1) - t3(i,k,j))/dy)**2 &
                    +(0.5*( t3(i+1,k,j) - t3(i-1,k,j) )/dx)**2 )
                qdiff3(i,k,j) = sqrt( ((q3(i,k,j+1) - q3(i,k,j))/dy)**2 &
                    +(0.5*( q3(i+1,k,j) - q3(i-1,k,j) )/dx)**2 )
            end if
            j = jlastxy
            dx = dlon*rearth*cosp(j)
!              vort3(i,k,j) = dx
            if (dx == 0.) then
                ...
                vort3(i,k,j) = -(u3(i,k,j) - u3(i,k,j-1))/dy +0.5*( v3(i+1,k,j) - v3(i-1,k,j) )/dx
                tdiff3(i,k,j) = sqrt( ((t3(i,k,j) - t3(i,k,j-1))/dy)**2 &
                    +(0.5*(t3(i+1,k,j) - t3(i-1,k,j) )/dx)**2 )
                qdiff3(i,k,j) = sqrt( ((q3(i,k,j) - q3(i,k,j-1))/dy)**2 &
                    +(0.5*(q3(i+1,k,j) - q3(i-1,k,j) )/dx)**2 )
...
!              vort3(i,k,j) = dx
            if (dx == 0.) then
                ...
                vort3(i,k,j) = -0.5*(u3(i,k,j+1) - u3(i,k,j-1))/dy +( v3(i+1,k,j) - v3(i,k,j) )/dx
                tdiff3(i,k,j) = sqrt( (0.5*(t3(i,k,j+1) - t3(i,k,j-1))/dy)**2 &
                    +((t3(i+1,k,j) - t3(i,k,j) )/dx)**2 )
                qdiff3(i,k,j) = sqrt( (0.5*(q3(i,k,j+1) - q3(i,k,j-1))/dy)**2 &
                    +((q3(i+1,k,j) - q3(i,k,j) )/dx)**2 )
            end if
            i = ilastxy
!              vort3(i,k,j) = dx
            if (dx == 0.) then
                ...
                vort3(i,k,j) = -0.5*(u3(i,k,j+1) - u3(i,k,j-1))/dy +( v3(i,k,j) - v3(i-1,k,j) )/dx
                tdiff3(i,k,j) = sqrt( (0.5*(t3(i,k,j+1) - t3(i,k,j-1))/dy)**2 &
                    +((t3(i,k,j) - t3(i-1,k,j) )/dx)**2 )
                qdiff3(i,k,j) = sqrt( (0.5*(q3(i,k,j+1) - q3(i,k,j-1))/dy)**2 &
                    +((q3(i,k,j) - q3(i-1,k,j) )/dx)**2 )
...
!inside
        do j=jfirstxy+1, jlastxy-1
            ...
            do i=ifirstxy+1,ilastxy-1
!                  vort3(i,k,j) = dx
                vort3(i,k,j) = sqrt( (0.5*(u3(i,k,j+1) - u3(i,k,j-1))/dy)**2 &
                    +(0.5*( v3(i+1,k,j) - v3(i-1,k,j) )/dx)**2 )
                tdiff3(i,k,j) = sqrt( (0.5*(t3(i,k,j+1) - t3(i,k,j-1))/dy)**2 &
                    +(0.5*( t3(i+1,k,j) - t3(i-1,k,j) )/dx)**2 )
                qdiff3(i,k,j) = sqrt( (0.5*(q3(i,k,j+1) - q3(i,k,j-1))/dy)**2 &
                    +(0.5*( q3(i+1,k,j) - q3(i-1,k,j) )/dx)**2 )
...
!xiex
                phys_state(lchnk)%var1 (i,k) = tdiff3(ic,k,jc)
                phys_state(lchnk)%var2 (i,k) = qdiff3(ic,k,jc)
!MZ
                !phys_state(lchnk)%vort (i,k) = 0.
                phys_state(lchnk)%newvar1 (i,k) = vort3(ic,k,jc)
...
!xiex--
                bbuffer(bpter(ib,k)+7) = tdiff3(i,k,j)
                bbuffer(bpter(ib,k)+8) = qdiff3(i,k,j)
                if (do_waccm_phys()) then
                   bbuffer(bpter(ib,k)+9) = frontga(i,k,j)
                   bbuffer(bpter(ib,k)+10) = frontgf(i,k,j)
                   bbuffer(bpter(ib,k)+11) = uzm(k,j)
                end if
!--
...
!xiex--
                phys_state(lchnk)%var1  (i,k) = cbuffer(cpter(i,k)+7)
                phys_state(lchnk)%var2  (i,k) = cbuffer(cpter(i,k)+8)
                if (do_waccm_phys()) then
                   phys_state(lchnk)%frontga(i,k)  = cbuffer(cpter(i,k)+9)
                   phys_state(lchnk)%frontgf(i,k)  = cbuffer(cpter(i,k)+10)
                   phys_state(lchnk)%uzm(i,k)      = cbuffer(cpter(i,k)+11)
                end if
!--
