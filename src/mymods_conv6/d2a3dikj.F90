...
!xiex--
!      subroutine d2a3dikj( grid, u, v, ua, va, vorta )
!--
...
!xiex
      !real(r8), intent(inout) :: vorta(grid%ifirstxy:grid%ilastxy,grid%km,      &
                                    !grid%jfirstxy:grid%jlastxy)     ! vorticity
...
!!      real(r8), pointer :: dx(:),dy(:)  ! Sine and cosine in longitude
...
!!          vorta(i,k,j) = dx
          !if (dx == 0.) then
              !vorta(i,k,j) = 0.
          !else
              !vorta(i,k,j) = -(ua(i,k,j+1) - ua(i,k,j))/dy +( va(i+1,k,j) - va(i,k,j) )/dx
          !end if
...
!!          vorta(i,k,j) = dx
          !if (dx == 0.) then
              !vorta(i,k,j) = 0.
          !else
              !vorta(i,k,j) = -(ua(i,k,j) - ua(i,k,j-1))/dy +( va(i+1,k,j) - va(i,k,j) )/dx
          !end if
...
!!          vorta(i,k,j) = dx
          !if (dx == 0.) then
              !vorta(i,k,j) = 0.
          !else
              !vorta(i,k,j) = -(ua(i,k,j+1) - ua(i,k,j))/dy +( va(i,k,j) - va(i-1,k,j) )/dx
          !end if
...
!!          vorta(i,k,j) = dx
          !if (dx == 0.) then
              !vorta(i,k,j) = 0.
          !else
              !vorta(i,k,j) = -(ua(i,k,j) - ua(i,k,j-1))/dy +( va(i,k,j) - va(i-1,k,j) )/dx
          !end if
!!edge
...
!!              vorta(i,k,j) = dx
              !if (dx == 0.) then
                  !vorta(i,k,j) = 0.
              !else
                  !vorta(i,k,j) = -(ua(i,k,j+1) - ua(i,k,j))/dy +0.5*( va(i+1,k,j) - va(i-1,k,j) )/dx
              !end if
              !j = jlastxy
              !dx = dlon*rearth*cosp(j)
!!              vorta(i,k,j) = dx
              !if (dx == 0.) then
                  !vorta(i,k,j) = 0.
              !else
                  !vorta(i,k,j) = -(ua(i,k,j) - ua(i,k,j-1))/dy +0.5*( va(i+1,k,j) - va(i-1,k,j) )/dx
              !end if
          !end do
...
!!              vorta(i,k,j) = dx
              !if (dx == 0.) then
                  !vorta(i,k,j) = 0.
              !else
                  !vorta(i,k,j) = -0.5*(ua(i,k,j+1) - ua(i,k,j-1))/dy +( va(i+1,k,j) - va(i,k,j) )/dx
              !end if
              !i = ilastxy
!!              vorta(i,k,j) = dx
              !if (dx == 0.) then
                  !vorta(i,k,j) = 0.
              !else
                  !vorta(i,k,j) = -0.5*(ua(i,k,j+1) - ua(i,k,j-1))/dy +( va(i,k,j) - va(i-1,k,j) )/dx
              !end if
          !end do
!!inside
          !do j=jfirstxy+1, jlastxy-1
              !dx = dlon*rearth*cosp(j)
              !do i=ifirstxy+1,ilastxy-1
                  !vorta(i,k,j) = -0.5*(ua(i,k,j+1) - ua(i,k,j-1))/dy + 0.5*( va(i+1,k,j) - va(i-1,k,j) )/dx
!!                  vorta(i,k,j) = dx
              !enddo
          !enddo
      !enddo
