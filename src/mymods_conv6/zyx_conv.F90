...
!MZ ===== done 
    ...
    public :: zyx_conv_init, zyx_conv_tend
    public :: ecp_readnl
    public :: nplume_sh_zyx, nplume_dp_zyx
#ifndef OFFLINECP
...
!MZ<
    integer :: nplume_sh_zyx 
    integer :: nplume_dp_zyx 
!>
...
!MZ ====== to include optional CAM fields in zm_conv.F90 =========
   ...
   !character(len=16)  :: plume_model ! from namelist= 'zyx'   ! 'cam' or 'zyx' set by namelist
   character(len=16)  :: deep_scheme! 

   !public zyx_convi                !  zyx scheme not needed
   public zyx_conv_evap            ! evaporation of precip 
...
!MZ< 
   !namelist /zyxconv_nl/ plume_model
...
!         if (ierr /= 0) then
!            call endrun(subname // ':: ERROR reading namelist zyxconv_nl')
!         end if
!      end if
...
!MZ<
        nplume_sh_zyx = ecp_nplume_sh
        nplume_dp_zyx = ecp_nplume_dp
...
! initialize the convection scheme
! ==============================================================================
subroutine zyx_conv_init(innlev, limcnv_in)
...
if(i<0)then
              ...
              write(*,"(A50/,A30/,5I10/,6(A10/,3(5E15.7/)) )") &
                  ' ZZ1ZZZz in zyx_conv at beinging  ....', &
...
i=1
if(i<0)then
   write(*,*)'in zyx_conv 1.0 cape',lchnk,plume_model,deep_scheme,cape
endif

!!              write(*,*)' -- in zyx_conv after buoyan_dilute cape'
!!              write(*,*)'cape',cape
!!              write(*,*)'cape q',q
...
i=1
if(i<0)then
   write(*,*)'in zyx_conv 2.0 cape',lchnk,lengath,cape
endif
...
if(i<0)then
write(*,*)' in zyx_conv.F90 for the ZYXcam option'
write(*,*)'mu',mu 
write(*,*)'eu',eu 
write(*,*)'qu',qu 
write(*,*)'su',su 
...
!Compute Closure
 ...
 !------------------------
!! write(*,*)'in zyx_conv before closure'
...
!!   write(*,*) 'cam plume_model option done in zyx_conv!'
   !============================
 endif  ! 'cam'
...
!===============================================================================
!MZ subroutine zyx_conv_evap(inncol,lchnk, &
subroutine zyx_conv_evap(lchnk, inncol, nlev,nlevp, &
     t,pmid,pdel,q, &
...
subroutine zyx_convi(limcnv_in, no_deep_pbl_in)
