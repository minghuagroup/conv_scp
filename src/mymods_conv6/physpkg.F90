module physpkg
  !-----------------------------------------------------------------------
    ...
    use zyx_conv, only: nplume_sh_zyx, nplume_dp_zyx
  ! Purpose:
  !
  ...
  use phys_control,     only: phys_do_flux_avg, waccmx_is, plume_model
...
!xiex
    call pbuf_add_field('BFLS_T','physpkg',dtype_r8,(/pcols,pver/),bfls_t_idx)
    call pbuf_add_field('BFLS_Q','physpkg',dtype_r8,(/pcols,pver/),bfls_q_idx)
...
!!    use conv_jp, only: nplume_sh, nplume_dp
!MZ
!!    use zyx_conv, only: nplume_sh, nplume_dp
#endif
...
!MZ<
  ...
  !plume_model = 'zyx' ! default, to set in namelist
...
!xiex
  ...
  case('ZYX')
    call subcol_netcdf_setdim( nplume_sh_zyx + nplume_dp_zyx, pver)
  case('SCP')
...
!    write(*,*)'---------'
    !write(*,*) "after convect_deep: qtend = ", ptend%q
    !write(*,*) "after convect_deep: q = ", state%q(:,:,1)

    call t_stopf('convect_deep_tend')
    call physics_update(state, ptend, ztodt, tend)

    !write(*,*)'in tphysbc after deep 1'
    !write(*,*) "after convect_deep: q = ", state%q(:,:,1)
    !write(*,*)'---------'
!xiex
#ifdef SCMDIAG
    call subcol_netcdf_putclm( "taft", pver, state%t, 1 )
    !write(*,*)'in tphysbc after deep 2'
    call subcol_netcdf_putclm( "qaft", pver, state%q(1,:,1), 1 )
#endif
    !write(*,*)'in tphysbc after deep 3'

    ! Haiyang Yu
    !call foutput( "physbc:deep_convection", state)
    
    call pbuf_get_field(pbuf, prec_dp_idx, prec_dp )
    !write(*,*)'in tphysbc after deep 4', prec_dp_idx,snow_dp_idx,prec_sh_idx,snow_str_idx, &
    !  prec_str_idx,prec_sed_idx,snow_sed_idx
    ...
    !write(*,*)'in tphysbc after deep 5'
!xiex bflschoice #3 after deep conv include everything
    !call pbuf_get_field(pbuf, bfls_t_idx, bfls_t)
    !call pbuf_get_field(pbuf, bfls_q_idx, bfls_q)
    !bfls_t(:,:) = state%t(:,:)
    !bfls_q(:,:) = state%q(:,:,1)
...
!    write(*,*)'.... before shallow'
!    write(*,*)'dlf',dlf
!    write(*,*)'dlf2',dlf2
!    write(*,*)'rliq',rliq
!    write(*,*)'rliq2',rliq2
!    write(*,*)'q', state%q(:,:,1)
!    write(*,*)'ptendq', ptend%q(:,:,1)
...
!    write(*,*)'---------'
!    write(*,*) "after convect_shallow qtend = ", ptend%q
!    write(*,*)'.... after shallow'
!    write(*,*)'dlf',dlf
!    write(*,*)'dlf2',dlf2
!    write(*,*)'rliq',rliq
...
!    write(*,*)'ptendq', ptend%q(:,:,1)
    ...
    !write(*,*)'in tphysbc after shallow 1'
    !write(*,*) "after convect_shallow: q = ", state%q(:,:,1)
    ! Haiyang Yu
    ...
    i=1
if(i<0)then
write(*,*)'in physpkg tphysbc after macro.....'
write(*,*)'state%s',state%s
write(*,*)'state%q',state%q
!write(*,*)'ptend%s',ptend%s
!write(*,*)'ptend%q',ptend%q
write(*,*)'cam_in%landfrac',cam_in%landfrac
write(*,*)'cam_in%ocnfrac',cam_in%ocnfrac
write(*,*)'dlf',dlf
write(*,*)'dlf2',dlf2
write(*,*)'cmfmc',cmfmc
write(*,*)'cmfmc2',cmfmc2
write(*,*)'cam_in%ts',cam_in%ts
write(*,*)'cam_in%sst',cam_in%sst
write(*,*)'zdu',zdu
write(*,*)'cmeliq',cmeliq
write(*,*)'det_s',det_s
write(*,*)'det_ice',det_ice
write(*,*)'... before park macrop done'
endif
...
!xiex bflschoice #2 status before radiation include rad+dyn
    !call pbuf_get_field(pbuf, bfls_t_idx, bfls_t)
    !call pbuf_get_field(pbuf, bfls_q_idx, bfls_q)
    !bfls_t(:,:) = state%t(:,:)
    !bfls_q(:,:) = state%q(:,:,1)
