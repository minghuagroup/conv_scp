...
!xiex
   call addfld ('VAR1    ','m/s     ',pver, 'A','Variable1',phys_decomp)
   call addfld ('VAR2    ','m/s     ',pver, 'A','Variable2',phys_decomp)
   call addfld ('TMP3D','1     ',pver, 'I','TMP - 3D output', phys_decomp)
   call addfld ('TMP2D','1     ',1   , 'I','TMP - 2D output', phys_decomp)
!xiex.
...
!   call addfld ('PINT      ','Pa      ',pver,    'A','Pressure at Interface',phys_decomp)
!   call addfld ('INVEXNER  ','1       ',pver,    'A','Inverse Exner Function',phys_decomp)
...
!xiex
    real(r8) zdel(pcols,pver) !thickness in [m]
...
!xiex
    do i=1,ncol
       do k=1,pver
          zdel(i,k) = state%zi(i,k)-state%zi(i,k+1)
       end do
    end do
