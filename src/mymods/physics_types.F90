...
!xiex
    call shr_assert_in_domain(state%var1(:ncol,:),         is_nan=.false., &
    ...
    call shr_assert_in_domain(state%prevt(:ncol,:),     is_nan=.false., &
         varname="state%prevt",     msg=msg)
    call shr_assert_in_domain(state%prevq(:ncol,:),     is_nan=.false., &
         varname="state%prevq",     msg=msg)
    ...
    call shr_assert_in_domain(state%v(:ncol,:),         lt=posinf_r8, gt=neginf_r8, &
         varname="state%v",         msg=msg)
!xiex
    call shr_assert_in_domain(state%var1(:ncol,:),         lt=posinf_r8, gt=neginf_r8, &
         varname="state%var1",         msg=msg)
    ...
    call shr_assert_in_domain(state%prevt(:ncol,:),     lt=posinf_r8, gt=0._r8, &
         varname="state%prevt",     msg=msg)
    call shr_assert_in_domain(state%prevq(:ncol,:),     lt=posinf_r8, gt=0._r8, &
         varname="state%prevq",     msg=msg)
  ...
  subroutine physics_state_copy(state_in, state_out)
    ...
    do k = 1, pver
       do i = 1, ncol
          ...
          state_out%prevt(i,k)     = state_in%prevt(i,k) 
          state_out%prevq(i,k)     = state_in%prevq(i,k) 
...
!xiex
  allocate(state%var1(psetcols,pver), stat=ierr)
  ...
  allocate(state%prevt(psetcols,pver), stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_alloc error: allocation error for state%prevt')
  allocate(state%prevq(psetcols,pver), stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_alloc error: allocation error for state%prevq')
...
!xiex
  deallocate(state%var1, stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_dealloc error: deallocation error for state%var1')
  deallocate(state%var2, stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_dealloc error: deallocation error for state%var2')
  ...
  deallocate(state%prevt, stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_dealloc error: deallocation error for state%prevt')
  deallocate(state%prevq, stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_dealloc error: deallocation error for state%prevq')
