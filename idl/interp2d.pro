 function interp2d, A, x0, y0, x1, y1, nxny,nearest_neighbor=nearest_neighbor,$
        extrapolate=extrapolate,trigrid=trigrid,_extra=extra
;+
; NAME:
;   interp2d
; PURPOSE:
...
;   nearest_neighbor = use faster nearest_neighbor interpolation
; Returned:
...
;                              optimized memory.
;  15-Nov-2014, Zarro (ADNET) - added NEAREST_NEIGHBOR interpolation
;  7-May-2015, Zarro (ADNET) - set interpolated pixels outside input
;                              range to zero (for NEAREST_NEIGHBOR)
;-
...
;   y0 = transpose(reform(temporary(y0),nx,ny))
  ...
  sz = size(zz)
endif else sz=size(a) 

;else zz = A 			; /regular was set -- x0 and y0 are linear
...
;dprint,'% nearest_neighbor: ',nearest_neighbor
;print,'% nearest_neighbor: ',nearest_neighbor
case 1 of
 keyword_set(trigrid): return,interpolate(zz,x2,y2,_extra=extra)
 keyword_set(nearest_neighbor):  begin
  bad=where(  (x1 lt min(x0)) or (x1 gt max(x0)) or (y1 lt min(y0)) or (y1 gt max(y0)),count)
  b=a[round(x2),round(y2)]
  if count gt 0 then b[bad]=0.
  return,b
 end
 else: return,interpolate(a,x2,y2,_extra=extra)
endcase

;-- stuff below is redundant
