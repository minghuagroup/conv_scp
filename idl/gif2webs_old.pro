...
diff_expdir = '/Users/minghuazhang/unix/cmip6/'+     MIPID + '/' +   ModelID + '/' +   EXPID +'-'+CTL_EXPID

dir_jpeg1 = expdir+'/jpeg/'
dir_jpeg2 = CTL_expdir+'/jpeg/'
dir_jpeg3 = diff_expdir+'/jpeg/'
dir_jpegs = [dir_jpeg1, dir_jpeg2, dir_jpeg3]
; ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
ncolumn = n_elements(dir_jpegs)
...
; =====================================================================================
  PRO GIF2WEB_DIFF, dir_jpegs, setZ = setZ, season = season
; =====================================================================================
; =====================================================================================
...
varstr = create_struct('ncolumn', ncolumn,  'names', [ID1, ID2, ID1+'_'+ID2])
;                       ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

dir_jpegsj = dir_jpegs + Seasons[iseason]+'/'

for i = 0, n_elements(dir_jpegsj) -1 do begin
  cnames = varstr.names[i]
  files = file_search(dir_jpegsj[i]+'/*.jpeg')
