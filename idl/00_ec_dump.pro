...
VARS=['RH','CLOUD','NN','Q','QS','dNdz','Vor','PV','AVor','dUdZ','Ri','dRHdz','d2RHdz']
...
CASE VAR of
 ...
 'PV':      aa = PV2     
 'AVor':    aa = avor2  
 'dUdZ':    aa = sqrt(DUV_DZ2)
 'Ri':      aa = Ri2
 'dRHdz':   aa = dRH_dz2*100
 'd2RHdz':  aa = d2RH_dz2*100
 else:
endcase
 ...
 close,1
 openw,1,gifolder+'/logfile'
 printf,1,'var,scale, min,max',var,':  ',scale, min(aa)/scale,max(aa)/scale
...
close,1
 print,'var,scale, min,max',var,':  ',scale, min(aa)/scale,max(aa)/scale
