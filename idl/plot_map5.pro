 ...
 pro plot_map4,aa,bb,xinp,yinp,levels,levels2,title=title,maxv=maxv,minv=minv,line=line,$
    ...
    nxp=nxp,nxx0=nxx0,avex=avex,$
        ...
        mapcolor=mapcolor,mapfill=mapfill,isetz=isetz ;,E_HORIZON={FILL:1, COLOR:3} 
...
;  print,sx,sy
 ...
 MAP_SET,  latcen,loncen,0,/ISOTROPIC, limit=limit,position=!P.position,$
       title=title,color=colors.black ;, E_HORIZON={FILL:1, COLOR:3} 
...
;;    if(cnt gt 0)then c_lines[jj]=2
 ...
 if(keyword_set(mapfill))then mfill=1
  ...
  ;MAP_CONTINENTS ,color=mcolor,fill=mapfill,USA=1,countries=1,hires=1
  MAP_CONTINENTS ,color=mcolor,fill=mapfill,countries=1,hires=1,coasts=1
  MAP_CONTINENTS ,color=mcolor,fill=mapfill,countries=1,hires=0,coasts=1
;stop
