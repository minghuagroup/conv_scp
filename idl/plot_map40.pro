 ...
 pro plot_map40,aa,bb,xinp,yinp,levels,levels2,title=title,maxv=maxv,minv=minv,line=line,$
...
; line   to get lines
 ...
 ii=where((xinp gt min(xrange)) and (xinp lt max(xrange)),cnt1)
; if(min(ii) gt 0)then ii=[min(ii)-1,ii]
; if(max(ii) lt n_elements(xinp) )then ii=[ii,max(ii)+1]

 jj=where((yinp gt min(yrange)) and (yinp lt max(yrange)),cnt2)
...
;  print,sx,sy
 ...
 if(loncen eq 0)then x[n_elements(x)-1] = 360.  ;!!
...
;;    if(cnt gt 0)then c_lines[jj]=2
 ...
 if(keyword_set(mapfill))then mfill=1
  ...
  MAP_CONTINENTS ,color=mcolor,fill=mapfill,coasts=1 ;USA=1
;stop
