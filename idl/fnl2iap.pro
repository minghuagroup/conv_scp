...
iap_fileold = '../obs/INIT/IAPi_0000-01-01_128x256_L30_c120110.nc'
iap_filenew = '../obs/INIT/my_iap_init_L30_20170727_00_00.nc'

iap_fileold = '../obs/INIT/IAPi_0000-01-01_128x256_L35_c180626.nc'
iap_filenew = '../obs/INIT/my_iap_init_L35_20170727_00_00.nc'

date=20170727
idiag = 1

SPAWN, 'cp '+ iap_fileold+' '+iap_filenew
...
;=== get IAP/CAM info as target
 ...
 PHIS    = get_fld('../obs/INIT/iap_phis.nc','PHIS')
...
TS1   = my_interpol2(TS1_2,  x2,y2,lon,lat)
 jj=where(TS1 le 200.,cnt)
 if(cnt gt 1)then TS1[jj] = TS[jj]
TS2   = my_interpol2(TS2_2,  x2,y2,lon,lat)
 jj=where(TS2 le 200.,cnt)
 if(cnt gt 1)then TS2[jj] = TS[jj]
TS3   = my_interpol2(TS3_2,  x2,y2,lon,lat)
 jj=where(TS3 le 200.,cnt)
 if(cnt gt 1)then TS3[jj] = TS[jj]
TS4   = my_interpol2(TS4_2,  x2,y2,lon,lat)
 jj=where(TS4 le 200.,cnt)
 if(cnt gt 1)then TS4[jj] = TS[jj]
...
jump_111:
;=================================
fileID=ncdf_open(iap_filenew,/write)
...
;-------
VarID = NCDF_VARID(fileID, 'U')
ncdf_varput,fileID, varID,US
;-------
VarID = NCDF_VARID(fileID, 'V')
