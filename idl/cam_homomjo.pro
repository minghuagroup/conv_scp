...
'East_Asia': begin ;pentad
     xx3 = [115.,125.]
     yy3 = [20.,44]
     tt3 = [21.,54] ;[0.,72]
         end 
'East_Asia2': begin ;pentad
     xx3 = [115.,125.]
     yy3 = [-20.,50]
         end
'Tropical_INDO_Pacific': begin ;pentad
     xx3 = [40.,180.]
     yy3 = [-15.,15]
     yth = 1
     t1 = (yth-1.)*nann
...
;7 specify what field types to plot and lev1 ===========================
 ...
 iv=iv-1
  ...
  if(fields[iv] ne 'climatology')then begin
    print, 'which yr?'
    yth = 1
    read,yth
     t1 = (yth-1.)*nann
...
; 6a ==  extract data and calc climatology & anomaly ==========
 ...
 endcase
     ...
     if(strpos(region, 'Pacific') ge 0)then lev1 = cal_lev([0.,20],20)
...
;    +strtrim(mean(aaw)/mean(bbw),2)+')'
 ...
 gifname= titlename+var+'_'+fields[iv]+'_yr_'+strdigit(yth,0)+'_'+region
 if(fields[iv] eq 'climatology')then $
 gifname= titlename+var+'_'+fields[iv]+'_'+region
 ...
 window,/free,xsize=600,ysize=660,title=title
