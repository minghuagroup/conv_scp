...
ReadData         = True    # Whether to extract new data4D pickle dump data file
Trading_cost     = 0.0065
########################################################################
year_min = 2020
year_max = 2020
...
x0min = 10000 * year_min + 100*month_min  + day_min
x0max = 10000 * year_max + 100*month_max  + day_max

x0minExp = datetime.strptime(str(x0min),'%Y%m%d')
x0maxExp = datetime.strptime(str(x0max),'%Y%m%d') + timedelta(5)  #Contract with expiration 5 days after time window
...
df  = df[['Date','UnderlyingPrice','Expiration','Type','Strike','Last','Bid','Ask']]
...
strike_min = df0['UnderlyingPrice'].min() * (1.0 - 1 / np.sqrt(253) )  # use volatility 200%
strike_max = df0['UnderlyingPrice'].max() * (1.0 + 1 / np.sqrt(253) )
...
df = df[df['Strike'    ].isin(myStrikes)]
df = df[df['Expiration'].isin(myExpDates)]
df = df[df['Type'      ].isin(myRights2) ]
...
if((os.path.exists(dump_file) == False) or (ReadData)):
    ...
    #
    #for iStrike in range(3,4):
    for iStrike in range(nStrikes):
        print('Extracting Strike',  iStrike, myStrikes[iStrike], myRights[iRight] )
        for iExp in range(nExp):
            #print('Extracting Strike', iStrike, iExp, myStrikes[iStrike], myRights[iRight], myExpDates[iExp])
            df4 = df0.copy()  # use stock price dataframe as dummy to replace values for plotting purpose
            df4['Last'] = np.NaN #-999.
            df4['Ask'] = np.NaN #-999.
            df4['Bid'] = np.NaN #-999.
            #
            df2 = df[(df['Type'] == myRights2[iRight])  & (df['Expiration'] == myExpDates[iExp]) & (df['Strike'] == myStrikes[iStrike])]
            df2.reset_index(drop=True, inplace=True)
            # for i in range(df2.shape[0]):
            #     Date = df2['Date'].iloc[i]
            #     j = df4[df4['Date'] == df2['Date'].iloc[i]].index[0]
            #     df4.iloc[j] = df2.iloc[i]

            # for i in df2.index:
            #     j = df4.index[df4['Date'] == df2.iloc[i]['Date']].values[0]
            #     # print(type(j))
            #     # print('i,j,df2.at[i,Date]',i,j,df2.at[i,'Date'])
            #     df4.iloc[j] = df2.iloc[i]
            #
            #
            df4_index = df4[ df4['Date'].isin(df2['Date'])].index
            # #print('df4_loc',type(df4_loc),df4_loc.shape,df4.loc,df4_loc.index)
            # #print('df2.index', df2.index)
            #print('df4_index',df4_index)
            df4.loc[df4.index] = df2
            #print('df4b.head(4)', df4.head(4))
            #print(' df2df4', df2.shape, df4.shape)
            #exit()
            # #
            # nn = df2.shape[0]
            # for i in range(nn):
            #     optDate = df2['Date'].iloc[i]
            #     j = df4[ df4['Date'] == optDate ].index[0]
            #     df4.iloc[j] = df2.iloc[i]
            #
            data4D[iExp][iStrike][iRight][0] = df4['Date'].tolist()
            data4D[iExp][iStrike][iRight][1] = df4['Last'].tolist()
            data4D[iExp][iStrike][iRight][2] = df4['Bid'].tolist()
            data4D[iExp][iStrike][iRight][3] = df4['Ask'].tolist()
            #print('data4D[iExp][iStrike][iRight][1]',data4D[iExp][iStrike][iRight][1])
    #print('data4D[0][4][0][1][0]', data4D[0][4][0][1])
    #exit()
...
for ii in range(len(x2)):
    ...
    if(ii == 0):    # first day
        ...
        iStrike2 = iStrike0
        #
        print('iExp_nearest, iStrike, iRight,index',iExp_nearest, iStrike, iRight,index)
        print('myRights,myStrikes, myExpDates',myRights,myStrikes, myExpDates)
