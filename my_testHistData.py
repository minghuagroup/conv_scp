...
from myContract_Spec import *
from myReadSamples import get_filename1
...
#######################################################################
    ...
    myContracts = myContract_Specs(mySymbol, myType)  ### modify Expiration date, strike etc for OPT and FOP

    myfilenames = ['']*1000
    if(myType == 'OPT' or myType =='FOP'):
        ...
        for Right in myRights:                #[0:1]:
            for Strike in myStrikes:          #[0:1]:
                for Exp in myExpDates:
                    filename = get_filename1(myHistDate, mySymbol, myType, Right, Strike, Exp)
                    myfilenames[ii] = filename
                    ii = ii + 1
        NF = ii
    else:
        myfilenames = [get_filename1(myHistDate, mySymbol, myType, 'C', 1.0, '')]
        NF=1
        myContracts = [myContracts]

    myfilenames = myfilenames[0:NF]
    for ii in range(NF):
        print('YYY',ii, myfilenames,myContracts[ii])
        #if (os.path.isfile(myfilenames[ii]) == False):
        data = [myfilenames[ii]]
        df = pandas.DataFrame(data)
        df.to_csv(myfilenames[ii])
        app = TestApp(myID2 = ii, myContract2 = myContracts[ii], myfilename2 = myfilenames[ii], myHistDate2 = myHistDate)
