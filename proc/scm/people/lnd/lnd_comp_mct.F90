...
!                                 set_nextsw_cday
    ...
    use colm_cpl7        , only : set_timemgr_init, set_nextsw_cday
!   use clm_atmlnd       , only : clm_l2a
    use colm_cpl7        , only : clm_l2a
!   use clm_initializeMod, only : initialize1, initialize2
    use colm_cpl7        , only : initialize1, initialize2
!   use clm_varctl       , only : finidat,single_column, set_clmvarctl, iulog, noland, &
!                                 downscale
    use colm_cpl7        , only : iulog, noland, downscale
!   use clm_varpar       , only : rtmlon, rtmlat
#ifdef RTM
    use colm_rtmVar      , only : rtmlon, rtmlat
#endif
!   use clm_varorb       , only : eccen, obliqr, lambm0, mvelpp
    use colm_cplMod      , only : eccen, obliqr, lambm0, mvelpp
!   use controlMod       , only : control_setNL
    use colm_cpl7        , only : control_setNL
!   use decompMod        , only : get_proc_bounds, get_proc_bounds_atm
!   use domainMod        , only : adomain
    use colm_cpl7        , only : get_proc_bounds, get_proc_bounds_atm
    use colm_cpl7        , only : adomain
    use shr_kind_mod     , only : r8 => shr_kind_r8
...
!   use spmdMod          , only : masterproc, spmd_init
    use spmd             , only : masterproc=>p_master, spmd_init
...
!                                 nsrContinue, nsrBranch
    use colm_varctl      , only : create_glacier_mec_landunit, nsrStartup, &
                                  nsrContinue, nsrBranch, nsrest
...
!-----------------------------------------------------------------------
    ...
    if (masterproc) then
       ...
       write(iulog,format) "CoLM land model initialization"
    else
       iulog = shrlogunit
    end if
...
!   endif   ! create_glacier_mec_landunit
    ...
    ! nextsw_cday for calculate Cosine solar zenith angle at next time step
...
!   use clm_atmlnd      ,only : clm_l2a, atm_a2l, clm_a2l, clm_downscale_a2l
    use colm_cpl7       ,only : clm_l2a, clm_a2l
!   use clm_driver      ,only : clm_drv
    use colm_cpl7       ,only : clm_drv
!   use clm_time_manager,only : get_curr_date, get_nstep, get_curr_calday, get_step_size, &
!                               advance_timestep, set_nextsw_cday,update_rad_dtime
    use timemgr         ,only : get_curr_date, get_nstep, get_curr_calday, get_step_size, get_prev_date
    use colm_cpl7       ,only : set_nextsw_cday,update_rad_dtime,advance_timestep
!   use domainMod       ,only : adomain, ldomain
!   use decompMod       ,only : get_proc_bounds_atm, get_proc_bounds
    use colm_cpl7       ,only : adomain, ldomain
    use colm_cpl7       ,only : get_proc_bounds_atm, get_proc_bounds
    use abortutils      ,only : endrun
!   use clm_varctl      ,only : iulog, downscale
    use colm_cpl7       ,only : iulog, downscale
!   use clm_varorb      ,only : eccen, obliqr, lambm0, mvelpp
    use colm_cplMod     ,only : eccen, obliqr, lambm0, mvelpp
    use shr_file_mod    ,only : shr_file_setLogUnit, shr_file_setLogLevel, &
                                shr_file_getLogUnit, shr_file_getLogLevel
...
!   use clm_varctl      ,only : create_glacier_mec_landunit, downscale
    use colm_cpl7       ,only : create_glacier_mec_landunit
...
!   use clm_varorb      ,only : eccen, mvelpp, lambm0, obliqr
    use colm_cplMod     ,only : eccen, mvelpp, lambm0, obliqr
...
!---------------------------------------------------------------------------
 ...
 !* How to use  nlend_sync & rstwr_sync???
...
!   use domainMod    , only : adomain
    use colm_cpl7    , only : get_proc_bounds_atm, adecomp
    use colm_cpl7    , only : adomain
...
!   use clm_atmlnd         , only : lnd2atm_type
    use colm_cpl7          , only : lnd2atm_type
!   use domainMod          , only : adomain
    use colm_cpl7          , only : adomain
...
!   use clm_atmlnd      , only: atm2lnd_type
    use colm_cpl7       , only: atm2lnd_type
!   use clm_varctl      , only: co2_type, co2_ppmv
    use colm_varctl     , only: co2_type, co2_ppmv
!   use clm_varcon      , only: rair, o2_molar_const
    use phycon_module   , only: rair, o2_molar_const
#if (defined C13)
...
!   use clm_varctl      , only: iulog
    use colm_cpl7       , only: iulog
    use colm_cpl7       , only: clm_a2l_rainc, clm_a2l_rainl, clm_a2l_snowc, clm_a2l_snowl
    use mct_mod         , only: mct_aVect
    use clm_cpl_indices
    implicit none
! !ARGUMENTS:
...
!---------------------------------------------------------------------------
    ...
    do g = begg,endg
        ...
        clm_a2l_rainc(g)   = forc_rainc
        clm_a2l_rainl(g)   = forc_rainl
        clm_a2l_snowc(g)   = forc_snowc
        clm_a2l_snowl(g)   = forc_snowl
...
!   use clm_varpar  , only : rtmlon, rtmlat
    use colm_rtmVar , only : rtmlon, rtmlat
    use RunoffMod   , only : runoff, get_proc_rof_bounds
    use abortutils  , only : endrun
!   use clm_varctl  , only : iulog
    use colm_cpl7   , only : iulog
    use mct_mod     , only : mct_gsMap, mct_gsMap_init
...
!---------------------------------------------------------------------------
    ...
    integer beg_lnd_rof, end_lnd_rof, beg_ocn_rof, end_ocn_rof
...
!   endif
    ...
    gsize = rtmlon*rtmlat
    lsize = end_ocn_rof - beg_ocn_rof + 1
    allocate(gindex(lsize),stat=ier)

    ni = 0
    do n = beg_ocn_rof, end_ocn_rof
       ni = ni + 1
       gindex(ni) = (runoff%ocn_jxy(n)-1)*rtmlon + runoff%ocn_ixy(n)
    end do

    if (ni /= (runoff%end_ocn-runoff%beg_ocn+1)) then
       write(iulog,*) sub, ' : ERROR runoff total count',ni,(runoff%end_ocn-runoff%beg_ocn+1)
       call endrun( sub//' ERROR: runoff not equal to expected' )
    end if
...
!   use clm_varcon  , only : re
    use phycon_module,only : re
    use RtmMod      , only : longxy_r, latixy_r, area_r
    use RunoffMod   , only : runoff, get_proc_rof_bounds
    use abortutils  , only : endrun
!   use clm_varctl  , only : iulog
    use colm_cpl7   , only : iulog
...
!   endif
    ...
    do n = beg_ocn_rof, end_ocn_rof
       ni = ni + 1
     ! liquid and ice runoff are bundled together to liquid runoff, and then ice runoff set to zero
       r2x_r%rAttr(index_r2x_Forr_roff,ni) = runoff%ocn(n) / (runoff%ocn_area(n)*1000.)
       r2x_r%rAttr(index_r2x_Forr_ioff,ni) = 0._r8

       if(r2x_r%rAttr(index_r2x_Forr_roff,ni).lt.0) r2x_r%rAttr(index_r2x_Forr_roff,ni)=0.
    end do
