!-----------------------------------------------------------------------
!Update: TianyiWang,2018.08,16, CO2 switch
!-----------------------------------------------------------------------
module atm_comp_mct
...
!wangty modify
       ...
       if(atm_cpl_dt/wrf_cpl_dt.gt.1) then
        if (nstep.gt.2) then
         call mct_aVect_copy( c2x_c1, c2x_c2 ) ! juanxiong he
         call atm_export_wrf( c2x_c1, cam_in, cam_state, cam_tend ) !juanxiong he
        end if
       else
        if (nstep.gt.1) then
         call mct_aVect_copy( c2x_c2, c2x_c1 ) ! juanxiong he
        end if
...
!zhang minghua modify
          cam_in(c)%tref2(i)      =  x2a_a%rAttr(index_x2a_Sx_tref2,  ig)  
          cam_in(c)%qref2(i)      =  x2a_a%rAttr(index_x2a_Sx_qref2,  ig)
          cam_in(c)%lhf2(i)       = -x2a_a%rAttr(index_x2a_Faxx_lat2, ig)     
          cam_in(c)%shf2(i)       = -x2a_a%rAttr(index_x2a_Faxx_sen2, ig)     
...
!===============================================================================
  ...
  subroutine atm_domain_mct( lsize, gsMap_a, dom_a )
    ...
    !
    data(:) = -9999.0_R8 
!----------- zhh debug 2014-05-27 ------------
    print*, 'At sub. atm_domain_mct, before calling mct_gGrid_importRAttr' 
!----------- zhh debug 2014-05-27 ------------
...
!----------- zhh debug 2014-05-27 ------------
    print*, 'At sub. atm_domain_mct, after calling mct_gGrid_importRAttr' 
!----------- zhh debug 2014-05-27 ------------
    !
