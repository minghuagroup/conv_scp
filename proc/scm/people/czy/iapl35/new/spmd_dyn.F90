...
!   2013-01-22  Zhang He, Jiang Jinrong
!   2013-01-30  Zhang He, Removed fv_control_mod    
!   2013-02-05  Zhang He, added lreq,rreq
!
!-----------------------------------------------------------------------
...
! !USES:
   ...
   use pmgrid,             only: plat, plon, numbnd2, &
                                 numlats, beglat, endlat, beglatdyn, endlatdyn, &  !zhh
                                 beglatxydyn, endlatxydyn, beglatxydynex, endlatxydynex, &  !zhh
                                 beglatdynex, endlatdynex, loc_JB, loc_JE, &  !zhh 2013-01-30
...
!czy 20180506
   use flexib,             only: Ndt, Ndq, DTDY, DTlin, DTadv, DTIMEQ
...
!                                                  (excl. tracers)
   ...
   integer, allocatable :: cutex(:,:) ! extended partition  !zhh 2013-02-01
   ...
   integer, allocatable :: ydist(:)    ! number of levels per subdomain
   integer lreq(2),rreq(2)
...
! !DESCRIPTION: Read in IAP-specific namelist variables.  Must be 
!               performed before dyn\_init
!
! !REVISION HISTORY:
!   2010.05.15   Sawyer  Creation
!   2013.01.21   ZhangHe  Reviewed
!
!EOP
!=========================================================================
!BOC
   ...
   namelist /spmd_iap_inparm/ npr_yz, geopktrans,        &
        geopkblocks,                                &
...
!czy        modc_onetwo, modc_tracers
   ...
   if (masterproc) then
      write(iulog,*) 'Read in spmd_iap_inparm namelist from: ', trim(nlfilename)
      unitn = getunit()
      open( unitn, file=trim(nlfilename), status='old' )

      ! Look for spmd_iap_inparm group name in the input file.  If found, leave the
      ! file positioned at that namelist group.
      call find_group_name(unitn, 'spmd_iap_inparm', status=ierr)
      if (ierr == 0) then  ! found spmd_iap_inparm
         read(unitn, spmd_iap_inparm, iostat=ierr)  ! read the spmd_iap_inparm namelist group
         if (ierr /= 0) then
            call endrun( subname//':: namelist read returns an'// &
                 ' error condition for spmd_iap_inparm' )
         end if
      end if
      close( unitn )
      call freeunit( unitn )
...
!czy20180506
   ...
   call mpibcast (Ndt               ,1,mpiint,0,mpicom)
   call mpibcast (Ndq               ,1,mpiint,0,mpicom)
   call mpibcast (DTDY              ,1,mpir8,0,mpicom)
   call mpibcast (DTlin             ,1,mpir8,0,mpicom)
   call mpibcast (DTadv             ,1,mpir8,0,mpicom)
   call mpibcast (DTIMEQ            ,1,mpir8,0,mpicom)
...
!jjr      integer, allocatable :: ydist(:) ! number of lats per subdomain
!jjr      integer, allocatable :: zdist(:) ! number of levels per subdomain
...
! Compute local limits
      ...
      enddo
      beglatxydyn = plat+1-endlatxy             !jjr
      endlatxydyn = plat+1-beglatxy
      beglatxydynex = max(1,   beglatxydyn - 1)
      endlatxydynex = min(plat,endlatxydyn + 1)
!
...
!
        ...
        do procid=myid_y+1,npr_y-1
           ...
           if (isum >= numbnd2) goto 20
        end do
20      if (myid_y /= npr_y-1 .and. isum < numbnd2 .and. nmostlat /= plat)then
        ...
        do procid=myid_y-1,0,-1
           ...
           isum = isum + cut(2,procid) - cut(1,procid) + 1
           if (isum >= numbnd2) goto 30
        end do
30      if (myid_y /= 0 .and. isum < numbnd2 .and. smostlat /= 1) then
           call endrun ('SPMDINIT_DYN: Something wrong in computation of southern neighbors')
        end if

!        write(iulog,*)'-----------------------------------------'
!        write(iulog,*)'Number of lats passed north & south = ',numbnd2
...
!-----------------------------------------------------------------------
      ...
      call endrun ('decomp_wavenumbers() should never be called in IAP dynamics')
