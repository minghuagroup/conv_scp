...
!--------------------   czy 20180119    --------------------
    ...
    real(r8) :: frontgf(beglonxy:endlonxy,plev,beglatxy:endlatxy)
    ! frontogenesis angle for gravity wave drag
    real(r8) :: frontga(beglonxy:endlonxy,plev,beglatxy:endlatxy)
    ! needed for qbo
!    real(r8) :: uzm(plev,beglatxy:endlatxy)
    ! Virtual temperature
    real(r8), allocatable :: ptxy(:, :, :) ! ptxy=t3*(1.0+zvir*q3(1))
    real(r8), allocatable :: h2o(:, :, :) ! q3(1)
    ! edge pressure
    real(r8), allocatable :: pexy(:, :, :) ! calculate from sub plevs0
    ! pmid
    real(r8), allocatable :: pmid(:, :, :) ! calculate from sub plevs0
    ! pdel
    real(r8), allocatable :: pdel(:, :, :) ! calculate from sub plevs0
...
!--------------------   czy 20180119    --------------------
   ...
   !allocate (ptxy(beglonxy:endlonxy, plev, beglatxy:endlatxy))
!   write(iulog,*)"*******before allocate ptxy, h2o, pexy, pmid, pdel*******"
   allocate (ptxy(beglonxy:endlonxy, beglatxy:endlatxy, plev))!(i,j,k)
   allocate (h2o(beglonxy:endlonxy, beglatxy:endlatxy, plev))!(i,j,k)
   allocate (pexy(beglonxy:endlonxy, plev+1, beglatxy:endlatxy))!(i,k,j)
   allocate (pmid(beglonxy:endlonxy, plev, beglatxy:endlatxy))
   allocate (pdel(beglonxy:endlonxy, plev, beglatxy:endlatxy))
   ib = endlonxy-beglonxy+1
!   write(iulog,*)"*******before calling plevs0*******"
   do j = beglatxy, endlatxy
      call plevs0(ib,ib,plev,ps(beglonxy:endlonxy,j),&
              pexy(beglonxy:endlonxy,:,j),&
              pmid(beglonxy:endlonxy,:,j),&
              pdel(beglonxy:endlonxy,:,j))
   end do
!   write(iulog,*)"pexy(beglonxy,plev+1,:)=",pexy(beglonxy,plev+1,:)
!   write(iulog,*)"pmid=",pmid(beglonxy,plev,:)
!   write(iulog,*)"pdel=",pdel(beglonxy,plev,:)
   do i = beglonxy, endlonxy
      do j = beglatxy, endlatxy
         do k = 1, plev
            ptxy(i,j,k)=t3(i,k,j)*(1.0_r8+zvir*q3(i,k,1,j))
            h2o(i,j,k)=q3(i,k,1,j)
         end do
      end do
   end do
!   write(iulog,*)"ptxy=",ptxy(beglonxy,:,plev)
!   write(iulog,*)"h2o=",h2o(beglonxy,:,plev)
!   if ( do_circulation_diags ) then
!      write(iulog,*)"*******before calling ctem_diags*******"
!      call t_startf('DP_CPLN_ctem')
!      call ctem_diags( u3, v3, omga, ptxy,h2o,ps,pexy,grid,uzm)
!      call t_stopf('DP_CPLN_ctem')
!   endif
!   write(iulog,*)"uzm=",uzm(plev,:)
   !write(iulog,*)"*******before calling gws_src_fnct*******"
   call t_startf('DP_CPLN_gw_sources')
   call gws_src_fnct (u3,v3,ptxy,h2o,pexy, grid,frontgf, frontga)
   call t_stopf('DP_CPLN_gw_sources')
   
   !write(iulog,*)"*******after calling gws_src_fnct*******"
...
!--------------------   czy 20180119    --------------------
                phys_state(lchnk)%frontgf(i,k) = frontgf(lons(i),k,lats(i))
                phys_state(lchnk)%frontga(i,k) = frontga(lons(i),k,lats(i))
!--------------------   czy 20180119    --------------------
             end do
          end do
...
!--------------------   czy 20180119    --------------------
                buf1(bpter(ib,k)+5) = frontgf(i,k,j)
                buf1(bpter(ib,k)+6) = frontga(i,k,j)
!                buf1(bpter(ib,k)+7) = uzm(k,j)
!--------------------   czy 20170516    --------------------
...
!                      buf1(bpter(ib,k)+3+m) = q3(i,k,m,j)*(1. - q3(i,k,1,j))
                      buf1(bpter(ib,k)+3+boff+m) = q3(i,k,m,j)*(1. - q3(i,k,1,j))!czy 20180119
                   else
!                      buf1(bpter(ib,k)+3+m) = q3(i,k,m,j)
                      buf1(bpter(ib,k)+3+boff+m) = q3(i,k,m,j)!czy 20180119
                   endif
                end do

!                buf1(bpter(ib,k)+4+pcnst) = pdeld(i,k,j)
                buf1(bpter(ib,k)+4+boff+pcnst) = pdeld(i,k,j)!czy 20180119
...
!jjr       call transpose_block_to_chunk(tsize, buf1, buf2, buf2win)
       !write(iulog,*)"*******before transpose_block_to_chunk*******"!czy20180119
       call transpose_block_to_chunk(tsize, buf1, buf2)
       !write(iulog,*)"*******after transpose_block_to_chunk*******"!czy20180119
...
!--------------------   czy 20180119    --------------------
                phys_state(lchnk)%q    (i,k,1)  = buf2(cpter(i,k)+4)
                phys_state(lchnk)%frontgf(i,k) = buf2(cpter(i,k)+5)
                phys_state(lchnk)%frontga(i,k) = buf2(cpter(i,k)+6)
!                phys_state(lchnk)%uzm(i,k) = buf2(cpter(i,k)+7)

                do m=2,pcnst
                   phys_state(lchnk)%q (i,k,m) = buf2(cpter(i,k)+3+boff+m)
                end do
                phys_state(lchnk)%pdeldry(i,k) = buf2(cpter(i,k)+4+boff+pcnst)
...
!-----------------------------------------------------------------------
!$OMP PARALLEL DO PRIVATE (LCHNK, NCOL, I, K, M, LONS, LATS)
    !write(iulog,*)"*******Fill auxilliary arrays*******"
...
! log(pressure) arrays and Exner function
       ...
       !write(iulog,*)"*******before calling geopotential_t*******"
