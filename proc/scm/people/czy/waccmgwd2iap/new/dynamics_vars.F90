...
!--------------------czy 20180119 --------------------
#if 0
   subroutine dynamics_init( im, jm, km, nq,                    &
                             ifirstxy, ilastxy,                 &
                             jfirstxy, jlastxy,                 &
                             jfirst, jlast, kfirst, klast,      &
                             npes_xy, npes_yz,                  &
                             ...
                             grid )
#endif
   subroutine dynamics_init( im, jm, km, pi, nq,                    &
                             ifirstxy, ilastxy,                 &
                             jfirstxy, jlastxy,                 &
                             jfirst, jlast, kfirst, klast,      &
                             npes_xy, npes_yz,                  &
...
!--------------------czy 20180119 --------------------
! !USES:
!zhh      use fv_control_mod, only: trac_decomp
      implicit none
...
!!      integer, intent(in)   :: jord                  !  Horz. scheme #
      ...
      real(r8), intent(in)  :: pi                    !  Pi czy 20170119
...
!--------------------czy 20180119 --------------------
! initialize grid%dp, grid%dl, grid%cosp, grid%sinp needed by gw calculation
...
!zhh      call dynpkg_init( pi, ae, om, dt, im, &
!zhh                        jm, jord, grid )

!
! Level-dependent variables  (was in vert_init, now removed)
!
!zhh      ALLOCATE(GRID%AK(km+1))
!zhh      ALLOCATE(GRID%BK(km+1))

!zhh      GRID%AK   = AK
!zhh      GRID%BK   = BK
!zhh      GRID%PTOP = GRID%AK(1)
!zhh      GRID%PINT = GRID%AK(ks+1)
