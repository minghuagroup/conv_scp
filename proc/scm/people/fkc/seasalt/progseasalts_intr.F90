...
public ncnst  ! number of sea salt constituents
  ...
  integer, parameter:: nsst =5
  real(r8), parameter :: scalefactor = 1.62_r8
#elif (defined MODAL_AERO_3MODE)
  integer, parameter:: nsst =4
  real(r8), parameter :: scalefactor = 1.00_r8 ! modified by fkc 
#endif
#else
  integer, parameter:: nsst =4
#endif
  ...
  real(r8), parameter :: tv = 12.0_r8 
#if (defined MODAL_AERO)
#if (defined MODAL_AERO_7MODE)
  integer, parameter :: ncnst=nsst+5                 ! number of constituents
#elif (defined MODAL_AERO_3MODE)
...
#if (defined MODAL_AERO)
#if (defined MODAL_AERO_7MODE)
       cnst_names = (/'ncl_a1', 'ncl_a2', 'ncl_a4', 'ncl_a6', 'ncl_a8', 'num_a1', 'num_a2', 'num_a4', 'num_a6','num_a8'/)
#elif (defined MODAL_AERO_3MODE)
       cnst_names = (/'ncl_a1', 'ncl_a2', 'ncl_a3', 'ncl_a4', 'num_a1', 'num_a2', 'num_a3','num_a4'/)
! ncl_a4 and num_a4 added by fkc
#endif
#else
...
#if (defined MODAL_AERO)
#if (defined MODAL_AERO_7MODE)
  real(r8) :: stk_crc(nsst)=(/ 1.0_r8, 1.0_r8, 1.0_r8, 1.0_r8 , 1.0_r8/)
#elif (defined MODAL_AERO_3MODE)
...
!  real(r8)::  soil_erodibility(plon,plat)            ! soil erodibility factor
...
#if (defined MODAL_AERO_7MODE)
              (/0.08e-6_r8, 0.02e-6_r8, 0.3e-6_r8, 1.0e-6_r8, 10.0e-6_r8/)  ! accu, aitken, fine, coarse
#elif (defined MODAL_AERO_3MODE)
              (/0.08e-6_r8, 0.02e-6_r8, 1.0e-6_r8, 10.0e-6_r8/)  ! accu, aitken, coarse
#endif
  real(r8) :: sst_sz_range_hi(nsst) = &
#if (defined MODAL_AERO_7MODE)
              (/0.3e-6_r8, 0.08e-6_r8, 1.0e-6_r8, 10.0e-6_r8, 250.0e-6_r8/)
#elif (defined MODAL_AERO_3MODE)
              (/1.0e-6_r8, 0.08e-6_r8, 10.0e-6_r8, 300.0e-6_r8/)
#endif
#else
  ...
  integer, pointer  :: num_a1_ndx, num_a2_ndx, num_a4_ndx, num_a6_ndx, &
                       num_a8_ndx
#elif (defined MODAL_AERO_3MODE)
  integer, pointer  :: ncl_a1_ndx, ncl_a2_ndx, ncl_a3_ndx, ncl_a4_ndx
  integer, pointer  :: num_a1_ndx, num_a2_ndx, num_a3_ndx, num_a4_ndx
#endif
#endif
  real(r8) :: smt_vwr(nsst) & 
#if (defined MODAL_AERO)
#if (defined MODAL_AERO_7MODE)
              =(/0.051e-6_r8, 0.15e-6_r8, 0.56e-6_r8, 3.6e-6_r8, 20.0e-6_r8/) ![m] Mass-weighted mean diameter resolved
#elif (defined MODAL_AERO_3MODE)
              =(/0.051e-6_r8, 0.256e-6_r8, 3.6e-6_r8, 20.0e-6_r8/) ![m] Mass-weighted mean diameter resolved
#endif
...
! only use up to ~20um
  real(r8), dimension(sections) :: Dg = (/  &
                                ...
                                2.3742e-5_r8,                              &
                                3.2000e-5_r8, 4.0000e-5_r8, 6.0000e-5_r8,  &
                                8.0000e-5_r8, 10.000e-5_r8, 12.000e-5_r8,  &
                                16.000e-5_r8, 18.000e-5_r8, 20.000e-5_r8,  &
                                25.000e-5_r8/) ! from 3.2e-5 below added by fkc
                               ! 40.000e-5_r8, 50.000e-5_r8,  &
                               ! 60.000e-5_r8, 80.000e-5_r8, 100.00e-5_r8/)
!  real(r8), dimension(sections) :: r80 = 0.518_r8*(0.5_r8*Dg)**0.976
  real(r8), dimension(sections) :: am, bm, rdry, rm
  real(r8), dimension(7,sections) :: consta, constb  !constants for calculating emission polynomial
#endif
...
contains
  ...
  subroutine progseasalts_register_cnst
    ...
    call cnst_add(cnst_names(5), one, one, zero, m)
    call cnst_add(cnst_names(6), one, one, zero, m)
    call cnst_add(cnst_names(7), one, one, zero, m)
    call cnst_add(cnst_names(8), one, one, zero, m)
    call cnst_add(cnst_names(9), one, one, zero, m)
    call cnst_add(cnst_names(10), one, one, zero, m)
#else
    call cnst_add(cnst_names(2), one, one, zero, m)
    call cnst_add(cnst_names(3), one, one, zero, m)
    call cnst_add(cnst_names(4), one, one, zero, m)
  ...
  subroutine progseasalts_init_cnst(name, q, gcid)
    ...
    if ( name == cnst_names(1) ) then
       ...
       q = 0._r8
    else if ( name == cnst_names(5) ) then
       q = 0._r8
    end if
#endif
  ...
  subroutine progseasalts_initialize 
    ...
    num_a8_ndx = get_spc_ndx( 'num_a8' )
#elif (defined MODAL_AERO_3MODE)
    ncl_a1_ndx => spc_ndx(1)
    ncl_a2_ndx => spc_ndx(2)
    ncl_a3_ndx => spc_ndx(3)
    ncl_a4_ndx => spc_ndx(4)
    num_a1_ndx => spc_num_ndx(1)
    num_a2_ndx => spc_num_ndx(2)
    num_a3_ndx => spc_num_ndx(3)
    num_a4_ndx => spc_num_ndx(4)
...
! for the parameterization
    ...
    am(:)=4.7_r8*(1+30*rm(:))**(-0.017_r8*rm(:)**(-1.44_r8))
    bm(:)=(0.433_r8-log10(rm(:)))/0.433_r8  ! use in Manahan
...
! calculate constants form emission polynomials
    do m=1,sections
       ...
       elseif (m.ge.22.and.m.lt.28)then
! use monahan
         consta(4,m) = (1.373_r8*rm(m)**(-am(m))*(1+0.057_r8*rm(m)**3.45_r8)  &
                   * 10**(1.607_r8*exp(-bm(m)**2)))  &
                   * (rm(m)-rm(m-1))
! added by fkc
       elseif (m.ge.28.and.m.lt.34)then
         consta(5,m) = (rm(m)-rm(m-1))*rm(m)**(-1.0_r8) 
       elseif (m.ge.34.and.m.lt.37)then
         consta(6,m) = (rm(m)-rm(m-1))*rm(m)**(-2.8_r8)
       else
         consta(7,m) = (rm(m)-rm(m-1))*rm(m)**(-8.0_r8)
       endif
! end
    enddo
#else
...
!!$    call outfld( 'RAM1', ram1(:), pcols, lchnk )
    ...
    do m=1,nsst
       ...
       do i=1,ncol
          ...
          dep_dry(i)=dep_trb(i)+dep_grv(i)
       enddo
       tsflx(:ncol)=tsflx(:ncol)+sflx(:ncol)
       call outfld( trim(cnst_name(mm))//'DryDep',dep_dry, pcols, lchnk)
...
subroutine progseasalts_emis_intr(state, cam_in)
   ...
   real(r8) :: u10(pcols),ubot(pcols),c1(pcols), c2(pcols), c3(pcols)
   real(r8) :: k(pcols) ! uplifting coefficient
...
!    u10cubed(:ncol)=1/1.373_r8*25.5_r8*u10cubed(:ncol)**2.07_r8
    u10cubed(:ncol)=u10cubed(:ncol)**3.41_r8
    call get_const(u10,c1,c2,c3,ncol)
#if (defined MODAL_AERO)
...
! calculate number flux fi (#/m2/s)
    ...
    do m=1,sections
       ...
       elseif (m.ge.10.and.m.lt.14)then
...
! use Monahan
       ...
       elseif (m.ge.28.and.m.lt.34)then
! added by fkc
         call kup(u10,m,k,ncol)
         fi(:ncol,m)=consta(5,m)*c1(:ncol)*k(:ncol)
       elseif (m.ge.34.and.m.lt.37)then
         call kup(u10,m,k,ncol)
         fi(:ncol,m)=consta(6,m)*c2(:ncol)*k(:ncol)
       else
         call kup(u10,m,k,ncol)
         fi(:ncol,m)=consta(7,m)*c3(:ncol)*k(:ncol)
       endif
! end
    enddo
#endif
...
#endif
 ! added by fkc
   subroutine get_const(u10,c1,c2,c3,ncol)
        integer, intent(in) :: ncol 
        real(r8), intent(in) :: u10(pcols)
        real(r8), intent(out):: c1(pcols), c2(pcols), c3(pcols)
        real(r8) :: dfdr(pcols)
        integer :: i
!        real(r8), dimension(sections) :: rm,bm
   do i=1,ncol
     if (u10(i).gt.tv) then
!        rm(i)=1.814_r8*0.5_r8*Dg(i)*1.e6_r8 
!        bm(i)=(0.380_r8-log10(rm(i)))/0.65_r8  
        dfdr(i) = (15.5_r8*rm(28)**(-3)*(1+0.057_r8*rm(28)**1.05_r8)  &
                  * 10**(1.19_r8*exp(-bm(28)**2))) &
                  * u10(i)**2.07_r8
        c1(i) = dfdr(i)*rm(31)
        c2(i) = c1(i)*rm(33)**1.8_r8
        c3(i) = c2(i)*rm(36)**5.2_r8 
     else
        c1(i) = 0.0_r8
        c2(i) = 0.0_r8
        c3(i) = 0.0_r8
   ...
   end subroutine get_const  

   subroutine kup(u10,m,k,ncol)
      real(r8), intent(in) :: u10(pcols)
      integer,  intent(in) :: m,ncol
      real(r8), intent(out):: k(pcols)
      
      real(r8) :: lambda(pcols)
      integer :: i
         
      lambda=u10*u10/(rm(m)*gravit)
      do i=1,ncol
         if (u10(i).lt.tv) then
              k(i)=0.0_r8
         else if ((u10(i).ge.10).and.(u10(i).lt.20)) then
              k(i)=-100.0_r8*exp(-1.5_r8*lambda(i))+75.11_r8
         else if ((u10(i).ge.20).and.(u10(i).lt.35)) then
                 if (lambda(i).le.1.5) then
                    k(i)=90.0_r8*lambda(i)-33.88_r8
                    if (k(i).lt.0) then
                       k(i) = 0
                    endif
                 else
