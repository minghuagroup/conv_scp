...
!    integer, parameter :: ntot_amode = 4    
    ...
    character(len=3) :: trnum       ! used to hold mode number (as characters)
       ...
       ! added by fkc
       xname_massptr(:nspec_amode(4),4)   = (/  'ncl_a4  '/)
       xname_massptrcw(:nspec_amode(4),4) = (/  'ncl_c4  '/)
       xname_spectype(:nspec_amode(4),4)  = (/  'seasalt '/)
       ! end
#endif
       ...
       xname_massptr(:nspec_amode(8),8)   = (/ 'ncl_a8  '/)
       xname_massptrcw(:nspec_amode(8),8) = (/ 'ncl_c8  '/)
       xname_spectype(:nspec_amode(8),8)  = (/ 'seasalt '/)
#endif
!    write(iulog,*)'cnst_name is ',cnst_name
    do m = 1, ntot_amode
...
9241   format( i4, 2( 2x, i12, 2x, a ),                                &
     ...
     subroutine modal_aero_initialize_q( name, q )
       ...
       if ( masterproc ) then
          ...
          if (name == 'ncl_a8' ) write( *, '(2a)' ) '    doing ', name
       end if
       ...
       do k = 1, plev
          ...
          if (name == 'ncl_a8' ) q(:,k,:) = duma*0.8_r8
          ! init aerosol number
          !
          ...
          if (name == 'num_a8' ) q(:,k,:) = dumb*0.8_r8  *2.7e13_r8
