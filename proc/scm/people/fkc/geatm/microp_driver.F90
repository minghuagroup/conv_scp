  ...
  use physconst,     only: gravit, rga ! rga added by fkc 
...
! MG micro diagnostics
  ...
  real(r8) :: mmr_to_mass(pcols,pver) ! added by fkc
  ...
  integer               :: naer_all
  real(r8), pointer     :: aermmr1(:,:),aermmr(:,:) ! aermmr1 added by fkc
  ...
  integer :: numaerosols  ! added by fkc
...
! If first timestep, initialize heatflux....in pbuf at all time levels.
 ...
 !modified by fkc
       do k = 1, pver
          mmr_to_mass(:ncol,k) = rga * state1%pdeldry(:ncol,k)
       enddo
   ! get names of aerosols
       call rad_cnst_get_info( 0, naero = naer_all )
       allocate(aernames(naer_all))
       ...
       do m = 1, naer_all
          if( is_first_step() ) then
              ...
              call new_rad_cnst_get_aer_mmr( 0, m, aernames(m), state1, pbuf, aermmr1)
              aer_mmr(:ncol,:,m) = aermmr1(:ncol,:)
!              call rad_cnst_get_aer_mmr( 0, m, state1, pbuf, aermmr1 )
!              aer_mmr(:ncol,:,m) = aermmr1(:ncol,:)
              aer_mmr(:ncol,:,m) = aer_mmr(:ncol,:,m)*1.0E-9_r8/(1.293_r8*273.15_r8/state1%t(1:ncol,1:pver))
!              write(*,*)'original aer_mmr is ', aermmr1(5,5)
