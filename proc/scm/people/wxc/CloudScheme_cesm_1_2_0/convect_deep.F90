...
! Public methods
   ...
   public :: deep_scheme    ! default set in phys_control.F90, use namelist to change
...
!----------------------------------------
  ...
  call pbuf_add_field('CMEDP',    'physpkg',dtype_r8,(/pcols,pver/), cmedp_idx)

  call pbuf_add_field('slflxdp',    'physpkg',dtype_r8,(/pcols,pverp/),slflxdp_idx)
  call pbuf_add_field('qtflxdp',    'physpkg',dtype_r8,(/pcols,pverp/),qtflxdp_idx)
...
!   real(r8), intent(out) :: cme(pcols,pver)    ! cmf condensation - evaporation
   real(r8), pointer, dimension(:,:) :: cme    ! cmf condensation - evaporation
...
!
    ...
    call pbuf_get_field(pbuf, cmedp_idx,     cme )
