...
subroutine zm_convr(lchnk   ,ncol    , &
                    ...
                    lengath ,ql      ,rliq    ,landfrac,slflx   , &
                    qtflx  )
!----------------------------------------------------------------------- 
! 
! Purpose: 
...
!-----------------------------------------------------------------------
   ...
   use scamMod,      only: divq,vertdivq,divt,vertdivt,single_column
   use cam_history,  only: outfld
...
! transports can be done in outside of conv_cam.
   ...
   real(r8):: eu_km(pcols,pver)
   real(r8):: du_km(pcols,pver)
...
!      real(r8) vd(pcols,pver)
   ...
   real(r8) dbuoy(pcols,pver)
   real(r8) dcape(pcols)
   real(r8) dcape_ideep(pcols)
   real(r8) ptte(pcols,pver)
   real(r8) pqte(pcols,pver)
...
#define dcape_trigger
#ifdef dcape_trigger
dbuoy(:ncol,:pver) = 0._r8
dcape(:ncol) = 0._r8
if (single_column) then
 ptte(1,:pver) = divt(:pver) + vertdivt(:pver)
 pqte(1,:pver) = divq(:pver,1) + vertdivq(:pver,1)
! ptte(1,:pver) = divt(:pver) + scm_pttend(:pver)
! pqte(1,:pver) = divq(:pver,1) + scm_pteq(:pver)
 do i=1,ncol
  do k=lel(i),maxi(i)
    dbuoy(i,k) = dbuoy(i,k) - (t(i,k)*0.608_r8*pqte(i,k) + ptte(i,k)*(1._r8+0.608_r8*q(i,k)))*287._r8
    dcape(i) = dcape(i) + dbuoy(i,k)*log(pf(i,k+1)/pf(i,k))
  end do
 end do
end if
#endif
...
!---------------------------Local storage-------------------------------
    ...
    real(r8) :: qsat_1   (pcols,pver)    ! saturation specific humidity
...
!         ncol ,pver  ,1       ,pver    )
    call qsat(t(1:ncol, 1:pver), pmid(1:ncol, 1:pver), &
              est(1:ncol, 1:pver), qsat_1(1:ncol, 1:pver))
...
! relative humidity depression must be > 0 for evaporation
          evplimit = max(1._r8 - q(i,k)/qsat_1(i,k), 0._r8)
...
! Currently does not include heating/cooling change to qsat
          evplimit   = max(0._r8, (qsat_1(i,k)-q(i,k)) / deltat)
...
! output
   ...
   real(r8), intent(in) :: capeg(pcols) 
   real(r8) :: cape_cldprp(pcols) 
!
   ...
   real(r8), intent(out) :: eu_km(pcols,pver)       ! entrainment rate of updraft
   real(r8), intent(out) :: du_km(pcols,pver)       ! detrainement rate of updraft
...
!            eu(i,k) = 0.25*1.8e-3_r8*(1.3_r8 - q(i,k)/qst(i,k))*(qst(i,k)/qst(i,jb(i)))**3 ! Bechtold
!            du(i,k) = 1.0e-4_r8
!            mu(i,k) = mu(i,k+1) + (eu(i,k) - du(i,k))*dz(i,k)*mu(i,k+1)
         end if
      end do
   end do
...
690 continue
  ...
  do i=1,il2g
     do k=jt(i),mx(i)-1
       cape_cldprp(i) = cape_cldprp(i) + (0.5*((su(i,k)-grav/cp*zf(i,k))+(su(i,k+1)-grav/cp*zf(i,k+1))+2.*tiedke_add)*(1._r8+0.608_r8*0.5_r8*(qu(i,k)+qu(i,k+1))) - (s(i,k)-grav/cp*z(i,k))*(1._r8+0.608_r8*q(i,k)))*rgas*log(pf(i,k+1)/pf(i,k))
     end do
  end do
...
!            totpcp_test(i) = totpcp_test(i) + dz(i,k)*rprd(i,k)  ! = totpcp
         end if
      end do
   end do
...
!
   ...
   real(r8), intent(in) :: dcape_ideep(pcols)          ! dynamic available pot. energy of column
!
!--------------------------Local variables------------------------------
!
...
!            dtmdt(i,k) = (1._r8/dp(i,k))*(mu(i,k+1)* (su(i,k+1)-shat(i,k+1))+md(i,k+1)*(sd(i,k+1)-shat(i,k+1))- &
!                          mu(i,k)*(su(i,k)-shat(i,k)) - md(i,k)*(sd(i,k)-shat(i,k))) - rl/cp*du(i,k)*(beta*ql(i,k)+ (1-beta)*ql(i,k+1))
...
!
!#define dcape_closure
#ifdef dcape_closure
            dboydt(i,k) =  - (dtmdt(i,k)/t(i,k)+0.608_r8/ &
...
#ifdef dcape_closure
   do i = il1g,il2g
      if (dadt(i) /= 0._r8) mb(i) = max(-dcape_ideep(i)/dadt(i),0._r8)
   end do
#else
   do i = il1g,il2g
      dltaa = -1._r8* (cape(i)-capelmt)
       if(dcape_ideep(i)*3600..ge.100._r8) aaa=1.
       if((dcape_ideep(i)*3600..gt.0._r8).and.(dcape_ideep(i)*3600..le.50._r8)) aaa=dcape_ideep(i)*3600./150._r8
       if((dcape_ideep(i)*3600..gt.50._r8).and.(dcape_ideep(i)*3600..lt.100._r8)) aaa=dcape_ideep(i)*3600./100._r8
#ifdef dcape_trigger
      if (dadt(i) /= 0._r8) mb(i) = max(dltaa/tau/dadt(i),0._r8)*aaa  ! dcape_trigger
#else
      if (dadt(i) /= 0._r8) mb(i) = max(dltaa/tau/dadt(i),0._r8)   ! default_cape
#endif
   end do
#endif
!
...
!DIR$ NOINTERCHANGE!
   do k = kbm,pver
      ...
      !      print *, "sd, qd", sd(i,k)-shat(i,k), qd(i,k)-qhat(i,k), mu(i,k)
      !      print *, "su, qu", su(i,k)-shat(i,k), qu(i,k)-qhat(i,k), md(i,k)
      !      print *,"up", 1./dsubcld(i)*(-mu(i,k)*(su(i,k)-shat(i,k))), &
      !                    1./dsubcld(i)*(-mu(i,k)*(qu(i,k)-qhat(i,k)))
      !      print *,"dn", 1./dsubcld(i)*(-md(i,k)*(sd(i,k)-shat(i,k))), &
      !                    1./dsubcld(i)*(-md(i,k)*(qd(i,k)-qhat(i,k)))
      !      pause
         else if (k > mx(i)) then
            dsdt(i,k) = dsdt(i,k-1)
            dqdt(i,k) = dqdt(i,k-1)
...
!
     ...
     real(r8) :: qv,qsat_1,e,esat,L,eref,pref
...
qsat_1=eps1*esat/(p-esat)                      ! Sat. mixing ratio (in kg/kg).

qv = min(qtot,qsat_1)                         ! Partition qtot into vapor part only.
e = qv*p / (eps1 +qv)

entropy = (cpres + qtot*cpliq)*log( TK/tfreez) - rgas*log( (p-e)/pref ) + &
        L*qv/TK - qv*rh2o*log(qv/qsat_1)
! 
return
