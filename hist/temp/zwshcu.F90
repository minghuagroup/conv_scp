  ...
  use cloud_fraction, only: zmh_cloud_sh
...
!zmh ---------------------------------
!===============================================================================
  logical           :: zmh_shallow_lf         = .true. !false. !true.  !include large-scale forced
  logical           :: zmh_shallow_ent        = .true. !false. !true.  !new entrainment
  real              :: zmh_ent0               =  0.18   ! 0.20 is too large with wxc formula
  logical           :: zmh_shallow_pert1      = .false. !.true. !true. for cape
  logical           :: zmh_shallow_pert2      = .false. !.true. !true. for winv
  logical           :: zmh_shallow_stochastic = .false. !.true. !.true. !true.

  !the following matters only when zmh_cloud_sh = .true. (set in cloud_fraction)        
  real              :: zmh_ufrcd              =  4.0    !coef of detrained volume
  real              :: zmh_ufrc0              =  2.0    !coef
  real              :: zmh_ufrcz              =  0.0    !offset z^2 coef 

  character(len=20) :: shallow_closure_scheme = 'prognostic' !'cam'
  real              :: tau1_zmh_sh            = 600.  ! for prognostic cape , time to mb
  real              :: tau2_zmh_sh            = 600.  ! 
...
subroutine zwshcu_readnl(nlfile)
   ...
   ! Local variables
   integer :: unitn, ierr
   character(len=*), parameter :: subname = 'zwshcu_readnl'

   ! Namelist variables
   real(r8) :: zwshcu_rpen =  unset_r8    !  For penetrative entrainment efficiency

   namelist /zwshcu_nl/ zwshcu_rpen
   !-----------------------------------------------------------------------------

   if (masterproc) then
      unitn = getunit()
      open( unitn, file=trim(nlfile), status='old' )
      call find_group_name(unitn, 'zwshcu_nl', status=ierr)
      if (ierr == 0) then
         read(unitn, zwshcu_nl, iostat=ierr)
         if (ierr /= 0) then
   ...
   call mpibcast(zwshcu_rpen,            1, mpir8,  0, mpicom)
#endif
   
   rpen=zwshcu_rpen
  

end subroutine zwshcu_readnl

!===============================================================================

  subroutine init_zwshcu( kind, xlv_in, cp_in, xlf_in, zvir_in, r_in, g_in, ep2_in )
    ...
    character(len=*), parameter :: subname = 'init_zwshcu'
    ...
    if (rpen == unset_r8) then
       call endrun(subname//': zwshcu_rpen must be set in the namelist')
    end if
  ...
  end subroutine init_zwshcu

  subroutine compute_zwshcu_inv( mix      , mkx        , iend          , ncnst     , dt       ,  & 
...
!zmh
                                 omega_inv    , tpert2   ,qpert2     , massflx_sh, &
                                 ...
                                 cbmf     , qc_inv     , rliq          ,                         &
                                 cnt_inv  , cnb_inv    , qsat          , lchnk     , dpdry0_inv, wu_inv ) !wxc
...
!zmh
    real(r8),intent(in)     :: omega_inv(mix,mkx)
    real(r8), intent(in)    :: tpert2(mix)
    real(r8), intent(in)    :: qpert2(mix)
    real(r8), intent(in)    :: massflx_sh(mix)
...
!wxc zmh
    real(r8), intent(out)   :: wu_inv(mix,mkx+1)
    real(r8)                   omega(mix,mkx)
    ...
    real(r8), intent(out)   :: flxprc1_inv(mix,mkx+1)   ! uw grid-box mean rain+snow flux (kg m^-2 s^-1) for physics buffer calls in convect_shallow.F90
    real(r8), intent(out)   :: flxsnow1_inv(mix,mkx+1)  ! uw grid-box mean snow flux (kg m^-2 s^-1) for physics buffer calls in convect_shallow.F90
...
!wxc zmh
    ...
    real(r8)                :: flxprc1(mix,0:mkx)       ! uw grid-box mean rain+snow flux (kg m^-2 s^-1) for physics buffer calls in convect_shallow.F90
    real(r8)                :: flxsnow1(mix,0:mkx)      ! uw grid-box mean snow flux (kg m^-2 s^-1) for physics buffer calls in convect_shallow.F90
...
!!    print*, '------------ At the beginning of sub. compute_zwshcu_inv ---------------'
!!    print*, 'dpdry0_inv(8,1) =', dpdry0_inv(8,1)
!!    print*, '-------------------------------------------------------------------'
!======================== zhh debug 2012-02-09 =======================     
    do k = 1, mkx
       k_inv               = mkx + 1 - k
...
!zmh
       omega(:iend,k)         = omega_inv(:iend,k_inv)
       do m = 1, ncnst
...
!zmh
    if(zmh_shallow_pert1)then
     do k=1,3
    !   k=1
       qv0(:iend,k)        = qv0(:iend,k) + qpert2(:iend)
       t0(:iend,k)        = t0(:iend,k) + tpert2(:iend)
       s0(:iend,k)        = s0(:iend,k) + cp*tpert2(:iend)
      enddo
    endif
!zmh end
    ...
    call compute_zwshcu( mix  , mkx    , iend      , ncnst , dt   , &
                         ps0  , zs0    , p0        , z0    , dp0  , &
                         u0   , v0     , qv0       , ql0   , qi0  , & 
                         t0   , s0     , tr0       ,                & 
                         ...
                         cbmf , qc     , rliq      ,                &
!zmh
                         omega, massflx_sh    , tpert2, &
                         cnt  , cnb    , qsat      , lchnk , dpdry0 ,wu)  !wxc
...
!wxc zmh
    ...
    enddo
!======================== zhh debug 2012-02-09 =======================     
!!    print*, '------------ At the end of sub. compute_zwshcu_inv ---------------'
!!    print*, 'trten(8,1,17) =', trten(8,1,17)
!!    print*, '-------------------------------------------------------------------'
!======================== zhh debug 2012-02-09 =======================     

  end subroutine compute_zwshcu_inv

  subroutine compute_zwshcu( mix      , mkx       , iend         , ncnst    , dt        , &
...
!zmh
                             omega,   massflx_sh    , tpert2, &
                             cnt_out  , cnb_out   , qsat         , lchnk    , dpdry0_in ,wu_out) !wxc
...
#endif
    use mzfunctions_mod,       only :zmh_ramp,subcol_kissvec,zmh_rand_normal
...
!zmh
    real(r8), intent(in)    :: omega(mix,mkx) 
    real(r8), intent(in)    :: massflx_sh(mix)                ! previous step
    real(r8), intent(in)    :: tpert2(mix)                    ! 
...
!zmh
!    parameter (rkm = 10.0_r8)        !  Determine the amount of air that is involved in buoyancy-sorting [no unit]
...
!zmh
    ...
    integer seed1(n_rand),seed2(n_rand),seed3(n_rand),seed4(n_rand)
    real (r8) rand_num(n_rand),rand_num2(n_rand)
...
!zmh for prognostic
        etau1_zmh =  1._r8 - exp(- dt/2._r8/tau1_zmh_sh)  
        etau2_zmh =  exp(- dt/2._r8/tau2_zmh_sh)  
        random_zmh(:) = 0.2_r8
...
!======================== zhh debug 2012-02-09 =======================     
!!    print*, '-------------------- At sub. compute_zwshcu -----------------------'
!    print*, '1st: trten_out(8,1,17) =', trten_out(8,1,17)
!    print*, 'dpdry0_in(8,1) =', dpdry0_in(8,1)
!    print*, '-------------------------------------------------------------------'
!======================== zhh debug 2012-02-09 =======================     
    ...
    do i = 1, iend                                      
      do k = 1,n_rand
       seed1(k) = abs(t0_in(i,1) - t0_in(i,mkx))  * 1000*k
       seed2(k) = abs(t0_in(i,3) - t0_in(i,mkx))  * 1000*(k+1)
       seed3(k) = abs(t0_in(i,5) - t0_in(i,mkx))  * 1000*(k+2)
       seed4(k) = abs(t0_in(i,7) - t0_in(i,mkx))  * 1000*(k+3)
     enddo
!   call subcol_kissvec(seed1, seed2, seed3, seed4, rand_num)
!write(*,*)'rand_num1'
!write(*,*)rand_num

   call zmh_rand_normal(seed1, seed2, seed3, seed4, rand_num, rand_num2)
...
!wxc               wu_out(i,mkx:0:-1)       = wu_s(0:mkx)
...
!zmh w' pdt shape !!!!!!!!!
       tkeavg = tkeavg + zmh_ramp(tpert2(i),0.0_r8,2._r8)*2.0
!!!!!!!!!!!!!!!!!
       sigmaw = sqrt( rkfre * tkeavg + epsvarw ) 

!write(*,*)'my1 i=                  ',i
!write(*,*)'tkeavg,   sigmaw        ',tkeavg,sigmaw

!zmh ==============
   if(zmh_shallow_pert2)then 
        sigmaw = sqrt( rkfre * tkeavg + epsvarw ) + zmh_ramp(tpert2(i),0.0_r8,1._r8)*0.1_r8
   endif
!zmh ==============
...
!write(*,*)'sigmaw, wcrit           ',sigmaw, wcrit
!write(*,*)'cin,    cinlcl          ',cin,    cinlcl

!zmh ==============
   if(zmh_shallow_lf)then
           wcrit = wcrit-max(-omega(i,kinv-1),0._r8) *rho0inv*9.8_r8
           wcrit = max(wcrit, 0._r8) 
   endif   
!           tmp1 = wcrit-max(-omega(i,kinv-1),0._r8) *rho0inv*9.8_r8
!zmh ==============
...
!write(*,*)'mu,  mzh_wcrit          ',mu,wcrit

       !rho0inv = ps0(kinv-1)/(r*thv0top(kinv-1)*exns0(kinv-1))
       cbmf = (rho0inv*sigmaw/2.5066_r8)*exp(-mu**2)
       tmp2 = cbmf
!write(*,*)'my1 org1_cbmf           ',cbmf

!zmh ==============
!        cbmf = i4_uniform_ab (-zmh_a ,zmh_b, zmh_random ) * sigmaw
   if(zmh_shallow_stochastic)then
        cbmf_n = 0.0_r8
        cbmf   = 0.0_r8
        do k=1,n_rand
            tmp1 = rand_num(k) * sigmaw
            if(tmp1 > wcrit)then 
                cbmf   = cbmf   + tmp1
                cbmf_n = cbmf_n + 1._r8
            endif
        enddo
!write(*,*)'my1 mid1_cbmf           ',cbmf
!write(*,*)'my1 cbmf_n              ',cbmf_n
!write(*,*)'rand_num'
!write(*,*)rand_num
!write(*,*)'rand_num*sigmaw'
!write(*,*)rand_num*sigmaw

        winv  = cbmf / max(cbmf_n,1._r8)
        cbmf  = cbmf * rho0inv / dble(n_rand) 
    endif  !non-stochastic
...
!zmh ==============
   if(zmh_shallow_stochastic)then
       winv = max(winv,1.0e-4_r8)
!write(*,*)'my2 new1_cbmf winv      ',cbmf,winv

       tmp1 =(rho0inv*sigmaw/2.5066_r8)*exp(-mu**2) 
       cbmf = cbmf*tmp1/max(tmp2,1.0e-5_r8)
   else 
       cbmf = (rho0inv*sigmaw/2.5066_r8)*exp(-mu**2)
...
!write(*,*)'my2 new2_cbmf winv      ',cbmf
!write(*,*)'my2 tmp1_ratio,mu       ',tmp1/max(tmp2,1.0e-5_r8),mu
!write(*,*)
...
!zmh ==================================
      ...
      select case (shallow_closure_scheme)
       case( 'prognostic')
        cbmf      = massflx_sh(i) *etau2_zmh + cbmf*etau1_zmh
        !write(*,*)'cbmf=',massflx_sh(i),cbmf,etau1_zmh
    endselect
!zmh ==================================
...
!zmh ==========================
     if(zmh_shallow_ent)then
                fer = zmh_ent0 * bogbot_arr(k)/max(wue*wue,1.0e-4_r8)   !0.25 wxc
                fer = min( max(fer, 0.0_r8), 3.0e-2_r8)

!                tmp1 = (z0(k)-z0(krel))/( z0(kpen)-z0(krel) +10._r8)
!                tmp1 = min(tmp1, 1.0_r8)
!                fdr = 2.0e-4 * tmp1  !**2 
!                if(iter_scaleh==1) fdr = 0.0_r8


!write(*,*)'k      ',k
!write(*,*)'fer  fdr      ',fer(k),fdr(k)
!write(*,*)'fer2 wue      ',tmp1,wue
!write(*,*)'bogbot, bogtop',bogbot_arr(k),bogtop_arr(k)
!zmh test
    endif           
...
!zmh============================
     ...
     if(zmh_cloud_sh)then

       tmp3 = 0.5_r8 * zmh_ufrc0

       !tmp1 = (z0(k)-z0(krel))/( z0(kpen)-z0(krel) +10._r8)
       !tmp1 = min(tmp1, 1.0_r8) 
       !tmp1 =  zmh_ufrc0 * (1._r8 + zmh_ufrcz * tmp1 * tmp1 )

       tmp2 = fdr(k)*umf(k)*(zs0(k)-zs0(k-1))*dt / dp0(k) * 9.8_r8 * zmh_ufrcd   
       tmp2 = min(1._r8-rmaxfrac, tmp2)
...
!zmh
!    write(*,*)'k  ',k
!    write(*,*)'cofrc(k) tmp3, tmp2'
!    write(*,*) cufrc(k),tmp3, tmp2

        cufrc(k) = tmp3*cufrc(k) + tmp2
!    write(*,*) 'cufrc(k) ',cufrc(k)
...
!     call outfld( 'wu_Cu'           , wu_out,                   mix,    lchnk )
     call outfld( 'wu_Cu'           , wu_out(:,mkx:0:-1),       mix,    lchnk )
...
!======================== zhh debug 2012-02-09 =======================     
!       print*, '---------- At the end of sub. compute_zwshcu ---------------'
!       print*, '5th: trten_out(8,1,17) =', trten_out(8,1,17)
!======================== zhh debug 2012-02-09 =======================     
     return

 end subroutine compute_zwshcu
  ...
  ! ----------------------------------------------------------------- !
    ...
    ! ------------------------------------ !
     ...
     if( rhi .le. 0.01_r8 ) then
         write(iulog,*) 'Source air is too dry and pLCL is set to psmin in zwshcu.F90' 
         qsinvert = psmin
         return
     end if
     ...
     do i = 1, 10
         ...
         ps       =  ps + dps
         if( ps .lt. 0._r8 ) then
             write(iulog,*) 'pLCL iteration is negative and set to psmin in zwshcu.F90', qt, thl, psfc 
             qsinvert = psmin
             return    
         end if
     ...
     write(iulog,*) 'pLCL does not converge and is set to psmin in zwshcu.F90', qt, thl, psfc 
     qsinvert = psmin
     return
  ...
  ! ------------------------------------------------------------------------------- !
    ...
    if( dqv .gt. 1.e-20_r8 ) then
        ...
        if( aa .lt. 0.5_r8 ) then
            ...
            write(iulog,*) 'Full positive_moisture is impossible in zwshcu'
        endif
    endif 
