...
! Arguments
!zmh
   character(len=20) :: deep_zmh_qpert1 = 'none' !sgh30' !sgh30' !none
   character(len=20) :: deep_zmh_qpert2 = 'none' !grad' !'none' !grad'
   character(len=20) :: deep_zmh_tpert2 = 'none' !grad' !none' !grad'
   character(len=20) :: deep_zmh_tpert3 = 'none' !breeze'
   real(8)           :: dist_xy =  100.e3            !for subgrid domain size
   real(8)           :: dist_xy2 = 100.e3            !for sea-land breeze in temperature
   real(r8)          :: tpertmax = 1.0_r8
   real(r8)          :: qpertmax = 0.5e-3_r8
! ===================================================
...
!zmh
   real(r8), intent(inout) :: tpert(pcols)                ! changed to inout
...
!zmh 2018-12-30
  ...
  if(deep_zmh_qpert1 == 'sgh30')then
      dzm(:)    = (state%zm(:,pver-1) - state%zm(:,pver))
      !qpert2(:) = qpert2(:) + abs((state%q(:,pver,1) -  &
      !                 state%q(:,pver-1,1))/dzm(:))*sgh30(:,lchnk)*1.0 *0.5  ! to add diurnal
      !tpert2(:) = tpert2(:) + abs((state%t(:,pver)*(1.e5/state%pmid(:,pver))**0.286 - &
      !        state%t(:,pver-1)*(1.e5/state%pmid(:,pver-1))**0.286) / dzm(:))*sgh30(:,lchnk)*0.0
  endif


#if (defined BFB_CAM_SCAM_IOP )
    vort3(:) = 0.0  ! level 3 from the bottom, reversed, see gravity_waves
    tpert2(:) = 0.0 !0.5
    qpert2(:) = 0.0 !state%q(:,pver,1)*0.02
#else
    vort3(:) = state%frontga(:,3)  ! level 3 from the bottom, reversed, see gravity_waves.F90

  if(deep_zmh_qpert2 == 'grad' )then
       qpert2(:) = qpert2(:) + state%frontgf(:,2)*dist_xy*cos(state%lat(:)) !*(1.-landfrac(:))     !qdiff
  endif
  if(deep_zmh_tpert2 == 'grad' )then
     tpert2(:) = tpert2(:) + state%frontgf(:,1)*dist_xy*cos(state%lat(:))   !*(1.-landfrac(:))     !tdiff
  endif
  if(deep_zmh_tpert3 =='breeze')then
     call zenith (calday, state%lat, state%lon, coszrs, ncol)
     do i = 1, ncol
             if(abs( cam_in%landfrac(i)-0.5)  .le.  0.45 )then
               tpert2(i) = tpert2(i) + state%frontgf(i,1)*dist_xy2*max(coszrs(i),0.) !* cos(state%lat(:))
               !qpert2(i) = qpert2(i) + state%frontgf(i,2)*dist_xy2*max(cozrs(i),0.) *2.0
             endif
     enddo
  endif
#endif
  ...
  do i=1,ncol
    tpert2(i) = min(tpert2(i), tpertmax)
    qpert2(i) = min(qpert2(i), qpertmax)
  enddo

  tpert3(:) = tpert(:)   + tpert2(:) 
  qpert3(:,:) = qpert(:,:) 
  qpert3(:,1) = qpert(:,1)+ qpert2(:) 
  do i=1,ncol
    tpert(i)   = min(tpert(i)  , tpertmax)
    qpert(i,1) = min(qpert(i,1), qpertmax)
  enddo

!write(*,*)'mytphysbc,tpert,qpert(:,1),tpert2,qpert2',tpert,qpert(:,1),tpert2,qpert2

  drhlims(:) = 0.0_r8
  do i=1,ncol
    drhlims(i) = 0.1*zmh_ramp(tpert2(i) , 0.5_r8   ,   1.0_r8) &
                +0.1*zmh_ramp(qpert2(i) ,state%q(i,pver,1)*0.02, state%q(i,pver,1)*0.05)
...
if(nstep<0)then
  !write(*,*)'deep_zmh_qpert1',' ',deep_zmh_qpert1
  !write(*,*)'deep_zmh_qpert2',' ',deep_zmh_qpert2
  !write(*,*)'deep_zmh_tpert2',' ',deep_zmh_tpert2
...
! ===============================
!!write(*,*)'tpert2',lchnk,tpert2(2),qpert2(2)
...
!
   ...
   call convect_deep_tend(  prec_zmc,   &
        pblht,    cmfmc,      cmfcme,             &
        tpert3,    qpert3(:,1), vort3, dlf,        pflx,    zdu,       & !zmh qpert
        rliq,    &
        ztodt,    snow_zmc,  &
...
!===================================================
      ...
      call stratiform_tend(state, ptend, ztodt, &
           ...
           cmfmc,   cmfmc2, &
           cam_in%ts,      cam_in%sst,        zdu,  tpert3, qpert3, &  !zmh tpert2 qpert2
