...
!============Jinbo Xie============
    use startup_initialconds,       only: initial_file_get_id, topo_file_get_id,&
                                          topoGWD_file_get_id
!============Jinbo Xie============
!czy  use startup_initialconds, only : initial_file_get_id, topo_file_get_id
  ...
  use infnan, only : inf
!czy  use comsrf,       only: landm, sgh, sgh30
...
!===========
  use physconst,        only: latvap, latice, rh2o,   &
                                r_universal!change constant
  use comsrf,              only: landm, sgh,sgh30,var,hgt_m,oc,&
                                 oa1,oa2,oa3,oa4,ol1,ol2,ol3,ol4!add 10other pars
!===========
! Jinbo Xie
!===========
...
!Jinbo Xie
!====Jinbo Xie====
                call infld('HGT_M', ncid_topoGWD, dim1name,'lat', 1, pcols,begchunk,&
                                endchunk, hgt_m, found, grid_map='PHYS')
                call infld('VAR' , ncid_topoGWD, dim1name, 'lat', 1, pcols,begchunk,&
                                endchunk,  var, found, grid_map='PHYS')
                call infld('CON' , ncid_topoGWD, dim1name, 'lat', 1, pcols,begchunk, &
                                endchunk,   oc, found, grid_map='PHYS')
                call infld('OA1', ncid_topoGWD, dim1name, 'lat', 1, pcols,begchunk, &
                                endchunk,  oa1, found, grid_map='PHYS')
                call infld('OA2', ncid_topoGWD, dim1name, 'lat', 1, pcols,begchunk, &
                                endchunk,  oa2, found, grid_map='PHYS')
                call infld('OA3', ncid_topoGWD, dim1name, 'lat', 1, pcols,begchunk, &
                                endchunk,  oa3, found, grid_map='PHYS')
                call infld('OA4', ncid_topoGWD, dim1name, 'lat', 1, pcols,begchunk, &
                                 endchunk,  oa4, found, grid_map='PHYS')
                call infld('OL1', ncid_topoGWD, dim1name, 'lat', 1, pcols,begchunk, &
                                endchunk,  ol1, found, grid_map='PHYS')
                call infld('OL2', ncid_topoGWD, dim1name, 'lat', 1, pcols,begchunk, &
                                endchunk,  ol2, found, grid_map='PHYS')
                call infld('OL3', ncid_topoGWD, dim1name, 'lat', 1, pcols,begchunk, &
                                endchunk,  ol3, found, grid_map='PHYS')
                call infld('OL4', ncid_topoGWD, dim1name, 'lat', 1, pcols,begchunk, &
...
!===Jinbo Xie===
        ...
        phys_state(c)%oa(1,:)=oa1(:,c)
        phys_state(c)%oa(2,:)=oa2(:,c)
        phys_state(c)%oa(3,:)=oa3(:,c)
        phys_state(c)%oa(4,:)=oa4(:,c)
        phys_state(c)%ol(1,:)=ol1(:,c)
        phys_state(c)%ol(2,:)=ol2(:,c)
        phys_state(c)%ol(3,:)=ol3(:,c)
        phys_state(c)%ol(4,:)=ol4(:,c)
!===Jinbo Xie===
!zmh added tpert2, qpert2
