...
character(len=16) :: waccmx_opt           = 'off'  ! WACCMX run option [ionosphere | neutral | off
...
! yhy: from cas-esm
   !   history_microphysics, do_iss, tms_orocnst, tms_z0fac
   !-----------------------------------------------------------------------------
!zmh
   namelist /phys_ctl_nl/ cam_physpkg, cam_chempkg, deep_scheme, shallow_scheme, &
       ...
       history_microphysics, history_budget, history_budget_histfile_num,        &
       conv_water_in_rad, &
! zmh
       macrop_scheme, plume_model, trigger_scheme, closure_dp_scheme, closure_sh_scheme
...
!zmh   write(*,*)'NNNN ',macrop_scheme,microp_scheme,shallow_scheme
#ifdef SPMD
...
subroutine phys_getopts(deep_scheme_out, shallow_scheme_out, eddy_scheme_out, microp_scheme_out, &
                        ...
                        do_tms_out, state_debug_checks_out, &   ! below: from cas-esm
                        history_microphysics_out, do_iss_out, tms_orocnst_out, tms_z0fac_out )
!-----------------------------------------------------------------------
...
!zmh
   !character(len=16), intent(out), optional :: plume_model_out
