...
!zmh =================================
   ...
   logical           :: do_dcape            = .true. !.false.
   real              :: tau1_zmh = 600.  ! for prognostic cape , time to mb
   real              :: tau2_zmh = 600.  ! for memory of 1000 J/kg CAPE

   real(r8) :: zyx_cape_lmt  = 150. !unset_r8  added random number *(1+rand) 200-400   
   ...
   real(r8) :: zyx_rain_cr   = 0.20  !

! for deep cld amt
   real(r8) :: zmh_dfrc0     = 2.0 !  core cloud fraction coef
   real(r8) :: zmh_dfrcz     = 0.0 !
   real(r8) :: zmh_dfrcd     = 2.0 !  volume fraction of detained air
!zmh =================================
...
subroutine zyx1_conv_tend(lchnk   ,ncol    , nstep              , &
                    ...
                    lengath ,ql      ,rliq    ,slflx   ,qtflx   ,deepfrc  &
...
!    -------------------------------------------------------
    ...
    do i=1,ncol
       ...
       deep_triggers(i,3) = (omega(i,k750(i)) < - 5.e-3_r8*rand_num(i) )             !kpbl(i)
       if(deep_triggers(i,3))then
        w11 = zmh_ramp(-omega(i,k750(i)),0._r8,5.e-3_r8)
        ramp(i) = ramp(i)*w11
       endif

       deep_triggers(i,4) = (precw(i) > 0.80*precws(i))       ! 0.70 default
...
!zmh cloud amount =================
  do i=1,lengath
   ii = ideep(i)   
   dum0 = max(zm(ii,lelg(i)) - zm(ii,maxg(i)), 1.0_r8)  
   do k= lelg(i),maxg(i)

!         dum1 = (zm(ii,k)-zm(ii,maxg(i)) ) / dum0
!         dum2 = zmh_dfrc0 * (1.0_r8 + zmh_dfrcz*dum1*dum1)

!         dum2 = mu(i,k)*10.24_r8/max(w_up(ii,k),0.1_r8)/rho(ii,k) * 2.0_r8 * dum2
!write(*,*)'i,ii,k',i,ii,k
!write(*,*),'dpcld1 ',dum2

         dum2 = zmh_dfrc0*mu(i,k)*10.24_r8/max(w_up(ii,k),0.1_r8)/rho(ii,k)  &!core+detrained mass
                + du(i,k)*dz(ii,k)*delt / dp(i,k) * zmh_dfrcd 

!write(*,*),'dpcld2 ',dum2


!write(*,*)'mu(i,k),w_up(ii,k),deepfrc(ii,k)'
!write(*,*) mu(i,k),w_up(ii,k),deepfrc(ii,k)
         ...
         deepfrc(ii,k) = min(dum2, 1.0_r8)
!write(*,*)'at lel cld1 cld2 ', deepfrc(ii,lelg(i)),dum1*zmh_dfrcd
...
!
   do k = 1,pver
      do i = 1,il2g
         dz(i,k) = zf(i,k) - zf(i,k+1)
         ramp_t1(i,k) = 1.0_r8 - zmh_ramp( t(i,k) - 238.15_r8, 0._r8,  30._r8) !freeze
         ramp_t2(i,k) = 1.0_r8 - zmh_ramp( t(i,k) - 263.15_r8, 0._r8,  5._r8) !conversion
