...
!wxc
    call addfld ('CLOUD_wang','   1 ',    pver, 'A', 'cloud fraction diagnosed by Wang'                        ,phys_decomp)

    call addfld ('RHC_diag',  '   1 ',    pver, 'A', 'RHC diagnosed from PBL and shallow convection'           ,phys_decomp)
...
!zmh
  ...
  subroutine macrop_driver_tend(                             &
             state, ptend_all, dtime, pblht, landfrac,  &  !zmh added pblht
             ocnfrac,  snowh,                       &
             dlf, dlf2, cmfmc, cmfmc2, ts,          &
             sst, shflx, lhflx, zdu,tpert2,qpert2,drhlims,       &                   !wxc &!zmh
  ...
  use shr_kind_mod,     only: r8 => shr_kind_r8
  use cloud_fraction,   only: cldfrc, cldfrc_implicit_PDF, macrop_scheme, &
                              zmh_cloud_dp,zmh_cloud_sh, sh1, sh2, dp1, dp2  !wxc
...
!  use cldwat2m_macro,   only: mmacro_pcond       
  use cldwat2m_macro_pbl,   only: mmacro_pcond_pbl !wxc
...
!zmh
  ...
  real(r8), intent(in)  :: drhlims(pcols)
  ...
  real(r8) :: cld_wang(pcols,pver)
  real(r8) :: rhc_diag(pcols,pver)
  ...
  real(r8) :: deepfrc(pcols,pver)                   ! Cloud fraction from shallow convection scheme
...
!zmh
  ...
  real(r8), pointer, dimension(:,:) :: wush      !
...
!zmh
  real(r8)  u00(pcols,pver)
  real(r8)  drh00(pcols,pver)
...
! For purposes of convective ql.
 ...
 !zmh ===============================================
  if(zmh_cloud_dp)then
       deepfrc(:pcols,:pver) = pbuf(deepfrc_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
   endif  

   if( use_shfrc .or. zmh_cloud_sh ) then  !zmh
...
!zmh ===============================================
 ...
 if(macrop_scheme .eq. 'XXZMH_WXC')then !pbl clouds
! ==============================
      call cldfrc_implicit_PDF( lchnk, ncol, pbuf, state1%zm, shflx, lhflx,&
          pblht, state1%zi, state1%pdel, state1%lat, state1%lon, &    
                   state1%pmid, t_inout, qv_inout, ql_inout, qi_inout, &
                   dtime, rhc_diag, cld2, cmeliq)  !zmh !cld2 dummies

   do i = 1, ncol
    do k = pver, 2,-1

       if(state1%zm(i,k+1) .le. pblht(i))then  
           concld(i,k)      =  max(concld(i,k),cld2(i,k) )
       else
           exit
       endif
    enddo
...
!     call zmh_u00(state1%lat, state1%pmid, u00)
!     do k=1,pver
!     do i=1,ncol
!       if(state1%pmid(i,k) .le. 3.e4_r8)then
!           u00(i,k) = min(u00(i,k), 0.8_r8)
!       endif    

!       u00(i,k) = max(u00(i,k), 0.5_r8)
!     enddo
!     enddo

     call wxc_u00(ncol, state1%lat,state1%pmid, u00)
     drh00(:,:) = 0.0_r8
...
!wxc
! PBL cloud condensation, X.Wang
   call mmacro_pcond_pbl( lchnk, ncol, pbuf, dtime, state1%pmid, state1%zm, state1%pdel,    &
                          shflx, lhflx, pblht,state1%zi, &  !zmh added fields 
                          tpert2, qpert2, state%lat, state%lon,state%pint(:,pverp), &!zmh
                          u00, drh00,    &
...
!update cloudiness using new scheme
 if(macrop_scheme .eq. 'WXC')then
! ==============================
      call cldfrc_implicit_PDF( lchnk, ncol, pbuf, state1%zm, shflx, lhflx,&
          pblht, state1%zi, state1%pdel, state1%lat, state1%lon, &    
                   state1%pmid, t_inout, qv_inout, ql_inout, qi_inout, &
                   dtime, rhc_diag, cld, cmeliq)  


 endif !WXC

!write(*,*)
!write(*,*)'in_macro,macrop_scheme,zmh_cloud_dp,zmh_cloud_sh'
!write(*,*)macrop_scheme,zmh_cloud_dp,zmh_cloud_sh
!write(*,*)'pblht',pblht

 if(macrop_scheme .eq. 'ZMH_WXC')then !pbl clouds
! ==============================
      call cldfrc_implicit_PDF( lchnk, ncol, pbuf, state1%zm, shflx, lhflx,&
          pblht, state1%zi, state1%pdel, state1%lat, state1%lon, &    
                   state1%pmid, t_inout, qv_inout, ql_inout, qi_inout, &
   ...
   do i = 1, ncol
    do k = pver-1, 2,-1
       !if((state1%zi(i,k) .le. pblht(i)) .and. (state1%zi(i,k) .le. 1.5e3_r8)) then

!write(*,*)'k_in_macro',k,concld(i,k),cld2(i,k),cld(i,k)

       if(state1%zm(i,k+1) .le. pblht(i))then  
           dum1           = cld(i,k) - concld(i,k)  !stratus

           dum2           =  max(concld(i,k),cld2(i,k) )  !convective and PBL clouds

           dum3           = (1.0_r8 - dum2)*dum1 + dum2
           cld(i,k)       = max(0.0_r8, min(dum3, 1.0_r8))
           concld(i,k)    = dum2   !both convective and PBL clouds
       else
           exit
       endif
    enddo
...
!     call outfld( 'CLOUD_wang   ', cld_wang ,  pcols, lchnk )

!     call outfld( 'RHC_diag   ', rhc_diag ,  pcols, lchnk )
...
! =================
  subroutine wxc_u00(ncol, rlat, p, u00)
...
! modified by ZMH
    ...
    real(8), intent(in) :: rlat(pcols)
    real(8), intent(in) :: p(pcols,pver)
    real(8), intent(out):: u00(pcols,pver)
