...
!  use cldwat2m_macro, only: rhmini
  use cldwat2m_macro_pbl, only: rhmini
! zmh 2
...
! zmh  changes made here -------------------------------------------------
! ================================  

  logical :: kk_flag    = .false. !true.   ! 1 Correction to KK auto-conversion
  logical :: varv_flag  = .true. !.false. !true.   ! 2 variable mu in Xie and Zhang 2016 JGR 
  logical :: disp_flag1 = .true. !.false. !true.   ! 3 Dispersion effect on radiation and microphysics
  logical :: disp_flag2 = .true. !.false. !.true.   ! 4 Dispersion effect on collision kernel
  integer :: ipgam      = 2        ! option of dispersion formula to use,default 2
...
! zmh 3
   ...
   public :: ini_micro, mmicro_pcond,gamma  !zmh

   public ::  pgam_lamc_zmh,      &
              beta4_zmh, &
              beta6_zmh
! zmh 3 end
...
! all units mks unless otherwise stated
  ...
  real(r8) :: W11, W12        ! working variables
...
! zmh 2
! This needs to be changed when the compset res is different  !!!!!

#if (!defined BFB_CAM_SCAM_IOP ) 

   if ( varv_flag .eq. .true. ) then

      hgrid = get_resolution()

      if ( hgrid .eq. '1.4x1.4' ) then
         res = 1.4_r8
      else if ( hgrid .eq. '0.5x0.5' ) then
         res = 0.5_r8
      else if ( hgrid .eq. '1.9x2.5' ) then
         res = 1.9_r8
...
!!!!!!!
      ...
      do i=1, pcols
         !hscale = res/180*3.14159*cos( state%ulat(i) )*6400.0
         !call subcol_calv(pver, hscale, state%zm(i,:), state%pmid(i,:), &
            !&state%t(i,:), state%q(i,:,1), ast(i,:), liqshape(i,:), stab(i) )
         hscale = res/180*3.14159*cos( ulat(i) )*6400.
...
!czy
    iter = 4 !czy 20180708 to avoid 'Impossible case2 in instratus_condensate' in cldwat2m_macro.F90
 endif
...
! zmh 3
    if(disp_flag1)then
        call pgam_lamc_zmh(ipgam,ncic(i,k),qcic(i,k),rho(i,k), &
...
! zmh 3 end
! zmh 4
        ! zmh used enhance6(k) below, can be enhance4(k)
    if(disp_flag2)then
        if(.not. disp_flag1)then
            call pgam_lamc_zmh(ipgam,ncic(i,k),qcic(i,k),rho(i,k), &
               episilon(k),pgam(k),lamc(k) )
        endif     
            enhance4(k) = beta4_zmh(episilon(k))/1.02849371455141_r8 !bet4_zmh(0.25_r8)
            enhance6(k) = beta6_zmh(episilon(k))/1.08482658018247_r8 !bet6_zmh(0.25_r8)
    else 
            enhance4(k) = 1.0
            enhance6(k) = 1.0
    endif   
...
!zmh 1, 2, 3
           if(kk_flag)then
              W11 = (rho(i,k)/1.16_r8)**1.47_r8
           else
              W11 = 1.0
           endif

           if(varv_flag)then
              W12 = sccons2(i,k)/(sccons3(i,k)*sccons18(i,k)) 
           else
              W12 = cons2/(cons3*cons18)
           endif
 ...
 ! hm switch for sub-columns, don't include sub-grid qc
           ...
           if (sub_column) then
              ...
              prc(k) = enhance6(k)*1350._r8*qcic(i,k)**2.47_r8* &
               (ncic(i,k)/1.e6_r8*rho(i,k))**(-1.79_r8) * W11     !zmh
              ...
              !prc(k) = cons2/(cons3*cons18)*1350._r8*qcic(i,k)**2.47_r8* &
              !(ncic(i,k)/1.e6_r8*rho(i,k))**(-1.79_r8) *(rho(i,k)/1.16_r8)**1.47_r8

              prc(k) = enhance6(k)*W12 *1350._r8*qcic(i,k)**2.47_r8* &
               (ncic(i,k)/1.e6_r8*rho(i,k))**(-1.79_r8) * W11      !zmh
...
! zmh 2
             if(varv_flag)then
               W11 = sccons9(i,k)/(sccons3(i,k)*sccons19(i,k))
               W12 = sccons10(i,k)/(sccons3(i,k)*scv(i,k))
             else
               W11 = cons9/(cons3*cons19)
               W12 = cons10/(cons3*qcvar)
             endif
...
! zmh 2
             if(varv_flag)then
               W11 = gamma(scv(i,k)+4._r8/3._r8)/(sccons3(i,k)*scv(i,k)**(4._r8/3._r8))
               W12 = gamma(scv(i,k)+1._r8/3._r8)/(sccons3(i,k)*scv(i,k)**(1._r8/3._r8))
             else
               W11 = gamma(qcvar+4._r8/3._r8)/(cons3*qcvar**(4._r8/3._r8))
               W12 = gamma(qcvar+1._r8/3._r8)/(cons3*qcvar**(1._r8/3._r8))
             endif
...
!zmh 1, 2
           if(kk_flag)then
              W11 = (rho(i,k)/1.16_r8)**1.30_r8
           else
              W11 = 1.0
           endif

           if(varv_flag)then
              W12 = sccons12(i,k)/(sccons3(i,k)*sccons20(i,k))
           else
              W12 = cons12/(cons3*cons20)
           endif
           ...
           if (sub_column) then
              ...
              pra(k) = 67._r8*(qcic(i,k)*qric(i,k))**1.15_r8 * W11     !zmh

              npra(k) = pra(k)/(qcic(i,k)/ncic(i,k))
 
           else

              !pra(k) = (cons12/(cons3*cons20)* 67._r8  &
                  !*(qcic(i,k)*qric(i,k))**1.15_r8
              pra(k) = W12 * (67._r8  &
                  *(qcic(i,k)*qric(i,k))**1.15_r8)* W11               !zmh
...
!zmh 3 
   if (disp_flag1)then
         call pgam_lamc_zmh(ipgam,dumnc(i,k),dumc(i,k),rho(i,k), &
                            episilon(k),pgam(k),lamc(k) )
   else 
         pgam(k)=0.0005714_r8*(ncic(i,k)/1.e6_r8*rho(i,k))+0.2714_r8
         pgam(k)=1._r8/(pgam(k)**2)-1._r8
...
! zmh 3
!================================================================================
subroutine pgam_lamc_zmh(ipgam,ncic,qcic,rho, &
         eps,pgam,lamc)
!-----------------------------------------
...
end subroutine pgam_lamc_zmh

!========================================

    real(8) function beta4_zmh(eps)
...
!----------------------------------------
    ...
    end function beta4_zmh


!========================================

    real(8) function beta6_zmh(eps)
...
!----------------------------------------
    ...
    beta6_zmh = (a/b)**0.166667

    return

    end function beta6_zmh
