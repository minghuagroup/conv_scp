...
!zmh
  ...
  ! -------------------------------------- !
  ! PBL Parameters used in the UW & ZW PBL !
  ! -------------------------------------- !

  character,        parameter :: sftype         = 'l'           ! Method for calculating saturation fraction

! Zhang Minghua changed the cloud-top entrainments from radf and evhc

!zmh added option to use the cloudy portion only
  character(len=4), parameter :: choice_evhc    = 'maxi'        ! 'orig',   'ramp',   'maxi'   : recommended to be used with choice_radf 
  !character(len=4), parameter :: choice_evhc    = 'mzhang'        ! 'orig',   'ramp',   'maxi'   : recommended to be used with choice_radf 

!zmh added option to use the cloudy portion only
  character(len=6), parameter :: choice_radf    = 'maxi'        ! 'orig',   'ramp',   'maxi'   : recommended to be used with choice_evhc 
  !character(len=6), parameter :: choice_radf    = 'mzhang'        ! 'orig',   'ramp',   'maxi'   : recommended to be used with choice_evhc 
...
!  real(r8),         parameter :: a1l            =   0.05_r8     ! zmh
  ...
  !real(r8),         parameter :: a1i            =   0.1_r8      ! zmh
  ...
  !real(r8),         parameter :: b1             =   10.0_r8      ! zmh
  real(r8)                    :: b123                           ! b1**(2/3)
!zmh
  real(r8),         parameter :: tunl           =   0.085_r8    ! Asympt leng = tunl*(turb lay depth)
!  real(r8),         parameter :: tunl           =   0.05_r8    ! zmh
...
!  real(r8),         parameter :: rinc           =  -0.01_r8      ! zmh
!zmh
 ...
 real(r8),         parameter :: tfac           =   1._r8       ! Ratio of 'tpert' to (w't')/wpert for CL. Same ratio also used for q
  ...
  subroutine compute_eddy_diff( lchnk  ,                                                            &
                                ...
                                ipbl   , kpblh  , wstarPBL , turbtype, sm_aw , lengi) !wxc
...
!zmh changed to inout
    real(r8), intent(inout)   :: tpert(pcols)              ! Convective temperature excess [ K ]
    real(r8), intent(inout)   :: qpert(pcols)              ! Convective humidity excess [ kg/kg ]
    real(r8), intent(in)   :: tpert2(pcols)              ! Convective temperature excess [ K ]
    real(r8), intent(in)   :: qpert2(pcols)              ! Convective humidity excess [ kg/kg ]
...
!wxc
    real(r8), intent(out)   :: lengi(pcols,pver+1)
...
!wxc    real(r8)                :: lengi(pcols,pver+1)       ! Turbulence length scale at all interfaces [ m ]
    ...
    do iturb = 1, nturb
     ...
     ! Calculate (qt,sl,n2,s2,ri) from a given set of (t,qv,ql,qi,u,v)
       ...
       call trbintd( &
                     ...
                     cmu      , minpblh , qsat , tpert, qpert, tpert2, qpert2)   !zmh added tpert and qpert
          ...
          do i = 1, ncol
           ...
           ! Modification : The contribution of ice should be carefully considered.
             if( choice_evhc .eq. 'ramp' .or. choice_radf .eq. 'ramp' ) then 
                 ...
                 sfuh(i,k)   = cldeff(i,k)
                 sflh(i,k)   = cldeff(i,k)
!zmh             elseif( choice_evhc .eq. 'maxi' .or. choice_radf .eq. 'maxi' ) then 
             elseif( choice_evhc .eq. 'maxi' .or. choice_radf .eq. 'maxi' &
               .or. choice_radf .eq. 'mzhang' .or. choice_evhc .eq. 'mzhang') then 
                 cldeff(i,k) = cld(i,k)
                 sfuh(i,k)   = cldeff(i,k)
                 sflh(i,k)   = cldeff(i,k)
             endif
  ...
  subroutine trbintd( pcols   , pver    , ncol    ,                               &
                      ...
                      ,tpert, qpert, tpert2, qpert2)  !zmh added
    !----------------------------------------------------------------------- !
...
!zmh
    real(r8), intent(in)  :: tpert(pcols), tpert2(pcols)
    real(r8), intent(in)  :: qpert(pcols), qpert2(pcols)
    integer,  external    :: qsat
...
!zmh !!!!!!!!!!!!!!!!!!!!!!!!!!!!
          rdzs      = 1._r8 / max( z(i,km1) - z(i,k)      , 500._r8)   !zmh
          rdzs2     = 1._r8 / max( z(i,k)   - zi(i,pver+1), 500._r8)   !zmh

          if((z(i,k)-zi(i,pver+1)) >  1.e3 )then  !1km
              rdzs  = 0.0   
              rdzs2 = 0.0   
          endif
          dsldz    = dsldz - (0.0*tpert(i) * rdzs + 0.0*tpert2(i)*rdzs2) * cpair !zmh
          dqtdz    = dqtdz - (0.0*qpert(i) * rdzs + 0.0*qpert2(i)*rdzs2)         !zmh
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!if(i>0)then
!write(*,*) 'k,chu,chs,cmu,cms,ch,cm',k,chu(i,k),chs(i,k),cmu(i,k),cms(i,k),ch,cm
!write(*,*) 'dsldz,dqtdz',dsldz,dqtdz
!write(*,*) 'ch*dsldz,cm*dqtdz',ch*dsldz,cm*dqtdz
!write(*,*)'z(i,km1) - z(i,k)',z(i,km1) , z(i,k)
!write(*,*)'sl(i,km1) - sl(i,k)',sl(i,km1) , sl(i,k),sl(i,km1) - sl(i,k)
!write(*,*)'qt(i,km1) - qt(i,k)',qt(i,km1) , qt(i,k),qt(i,km1) - qt(i,k)
!endif
!zmh
!n2(i,k) = n2(i,k)*0.5
!s2(i,k) = s2(i,k)*4.0
...
!    write(*,*)'mytribintd,tpert,qpert,tpert2,qpert2',tpert,qpert,tpert2,qpert2
...
!if(i>0)then
!write(*,*)'myeddy1=ustar ,minpblh,shflx====',ustar,minpblh,shflx
!write(*,*)'n2',n2
!write(*,*)'s2',s2
!write(*,*)'ri',ri
!write(*,*)'kvh_in', kvh_in
...
!zmh
!          lbulk = lbulk + zi(i,kt-1) + zi(i,kt) ! to increase the mixing 
...
!zmh 2018-07-15 just for the cloudy portion
          elseif( choice_evhc .eq. 'mzhang' ) then

              qleff  = ql(i,kt)/max(cldeff(i,kt),1.e-4_r8)  
              jt2slv = slv(i,max(kt-2,1)) - slv(i,kt)
              jt2slv = max( jt2slv, jbumin*slv(i,kt-1)/g )
              evhc   = 1._r8 + a2l * a3l * latvap * qleff / jt2slv
              evhc   = 1._r8 - cldeff(i,kt) + cldeff(i,kt)*evhc
              evhc   = min( evhc, evhcmax )
          endif
!zmh end  
...
!zmh 2018-07-15 for the cloudy portion only

          elseif( choice_radf .eq. 'mzhang' ) then

                  lwp         = ql(i,kt) * ( pi(i,kt+1) - pi(i,kt) ) / g /max(cldeff(i,kt),1.e-3_r8)

                  opt_depth   = 130._r8 * lwp   ! Estimated LW optical depth in the CL top layer

                  radf = (sl(i,kt) - g* z(i,kt) + Latvap*ql(i,kt) )/cpair  !T
                  if(radf > 273.16_r8)then

                   radinvfrac = (1._r8-exp(-opt_depth/2._r8)) - (1._r8-exp(-opt_depth))/2. !difference halfs
                   radinvfrac = max(0._r8, radinvfrac)
                  
                   radf = radinvfrac*5.678_r8*(radf/100._r8)**4  !if all cooling from top half-layer
                   radf = radf /(pi(i,kt)-pi(i,kt-1))*g             ! unit mass W/kg    
                   radf = min(radf, cpair/86400._r8*20._r8)      ! put maximum cooling at 20 K/day 

                   !!write(*,*)'radinvfrac, radf',radinvfrac, radf/cpair*86400.
                   radf = radf*(zi(i,kt) - zi(i,kt+1) )/2._r8
                   radf = radf * chs(i,kt) *cldeff(i,kt) 
                  ...
                  !!write(*,*)''
                  !!write(*,*)' Eddy2 ---- lwp,radinvfrac,radf,T'
                  !!write(*,*)'            ',lwp,radinvfrac,radf,(sl(i,kt) - g* z(i,kt) + Latvap*ql(i,kt) )/cpair
                  !!write(*,*)''
          endif
!zmh end
          ...
          if( id_sedfact ) then
            ! wsed    = 7.8e5_r8*(ql(i,kt)/ncliq(i,kt))**(2._r8/3._r8)
              ...
              elseif( choice_evhc .eq. 'ramp' ) then
                  ...
                  evhc = min(evhc,evhcmax)
!zmh              elseif( choice_evhc .eq. 'maxi' ) then
              elseif( choice_evhc .eq. 'maxi' .or. (choice_evhc .eq. 'mzhang')) then
                  qleff  = max(ql(i,kt-1),ql(i,kt))
                  jt2slv = slv(i,max(kt-2,1)) - slv(i,kt)
                  jt2slv = max(jt2slv, jbumin*slv(i,kt-1)/g)
...
!if(i>0)then
!write(*,*)'myeddy3===ncv,kt,kb=,lbulk',ncv,kt,kb,lbulk
!write(*,*)'wstar3,fact,wstarent',wstar3,fact,wstarent
!write(*,*)'rcrit, trmq, rootp, wet, web',rcrit, trmq, rootp, wet, web
!write(*,*)'ebrk(i,ncv)',ebrk(i,ncv)
!write(*,*)'tke(i,k),ebrk(i,ncv),rcap',tke(i,kt:kb),ebrk(i,ncv),rcap
!write(*,*)'---------------'
!endif
...
!if(i>0)then
!write(*,*)'in eddy4 ncvsurf ------',ncvsurf
!write(*,*)'tpert,qpert',tpert,qpert
!write(*,*)'tke',tke
!write(*,*)'pblh(i),ktopbl(i)',pblh(i),ktopbl(i)
!endif
...
!write(*,*)'in exacol k,riex(k),rimaxentr, ntop_turb',k,riex(k),rimaxentr,ncv 
...
!!write(*,*)'yes2'
 ...
 !write(*,*)'exacol2 ncv, ktop(ncv)',ncv, ktop(i,ncv)
