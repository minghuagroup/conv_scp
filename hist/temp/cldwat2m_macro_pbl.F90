...
!#define CLOUD_IAP
  ...
  !--------------------------------------------------- !
   ...
   use mzfunctions_mod,  only: zmh_u00,zmh_u00_single,zmh_ramp 
!   use zyx1_conv,        only: zmh_ramp 

   implicit none
   private
   save

   public :: ini_macro, mmacro_pcond_pbl
...
!  ZMH OPTIONS 
   !==============================
   character(len=20)            :: CAMstfrac    = 'cam5' !,'cam4','cam5'  !zmh 
   integer                      :: iceopt       = 5        ! 5 is cam default
                                !the above 2 parameters will be overwritten when namelist
                                !     Macrop_scheme = 'ZMH_WXC'
   logical,  parameter          :: zmh_ice_dep = .true.
   real(r8), public,  parameter :: zmh_rhmaxi   = 1.1_r8 ! supersaturation limit
   ...
   real(r8), parameter          :: qist_min     = 1.0e-7_r8 !1.e-7_r8      ! Minimum in-stratus ice IWC constraint [ kg/kg ]
...
!   logical,  parameter          :: CAMstfrac    = .false.       ! If .true. (.false.), use Slingo (triangular PDF-based) liquid stratus fraction
   ...
   subroutine mmacro_pcond_pbl( lchnk      , ncol       , pbuf       , dt           , p          , zm         , dp         , &
                                shflx      , lhflx , pblht  ,zi, tpert2     ,qpert2        , &
                                lat,  lon, pnps, u00    , drh00      ,& !zmh added 
   ...
   use cloud_fraction,   only : cldfrc_implicit_PDF, macrop_scheme, zmh_cldfrc_fice
...
!zmh
   ...
   real(r8), intent(in)    :: pnps(pcols)
   real(r8), intent(in)    :: u00(pcols,pver) 
   real(r8), intent(in)    :: drh00(pcols,pver) 
   ...
   real(r8), intent(in)    :: zm(pcols,pver)               ! Altitude at the layer mid-point [m]
   real(r8), intent(in)    :: zi(pcols,pverp) !zmh
...
!  local for PBL clouds
   real(r8)  :: rhc_null(pcols,pver)               
   real(r8)  :: cmeliq_null(pcols,pver)               
   real(r8)  :: a_st_pbl(pcols,pver)               
   real(r8)  :: a_st_pbl_perturb(pcols,pver)               
   integer   :: cu_pbl_top(pcols)
   logical   :: subgridon(pcols,pver)

!zmh
   real(r8) :: drhsrf(pcols)
   real(r8) :: zmh_fice(pcols,pver)
   real(r8) :: zmh_fsnow(pcols,pver)

   cone            = 0.999_r8
   qsmall          = 1.e-18_r8
   zeros(:ncol,:)  = 0._r8

    drhsrf(:) = 0.0_r8 !intended for cam cld scheme, not used now
...
!! write(*,*)macrop_scheme,CAMstfrac,iceopt
!zmh ----------- for new cld scheme
 if(macrop_scheme .eq. 'ZMH_WXC')then
     CAMstfrac = 'iap'
     iceopt    = 6
 endif
! write(*,*)'macrop_scheme,CAMstfrac,iceopt'
...
!zmh
      call instratus_condensate( lchnk, ncol, k,                                   &
                                 ...
                                 landfrac(:), snowh(:),u00(:,k),drh00(:,k),lat(:),lon(:),pnps(:),&  !zmh
...
!zmh
   if(zmh_ice_dep)then
      call zmh_cldfrc_fice(ncol, t0, zmh_fice, zmh_fsnow)
   else
       zmh_fice(:,:) = 0.0_r8
   endif
...
if(macrop_scheme .eq. 'WXC')then
      call cldfrc_implicit_PDF( lchnk, ncol, pbuf, zm, shflx, lhflx, pblht, zi, dp,lat,lon, &
                   p, T, qv, ql, qi, &
                   dt, rhc_null, a_st_pbl, cmeliq_null, cu_pbl_top, subgridon)

      call cldfrc_implicit_PDF( lchnk, ncol, pbuf, zm, shflx, lhflx, pblht, zi, dp,lat,lon, &
                   p, T, qv*1.01_r8, ql, qi, &
                   dt, rhc_null, a_st_pbl_perturb, cmeliq_null, cu_pbl_top, subgridon) 
endif
...
!zmh below
!      if( CAMstfrac ) then
    select case ( CAMstfrac )

      case ('cam4' )
          call astG_RHU(U_nc(:,k),p(:,k),qv(:,k),landfrac(:),snowh(:),al_st_nc(:,k),G_nc(:,k),&
            drhsrf*0.0, ncol)  !zmh added drhsrf
      case ( 'cam5' ) 
          call astG_PDF(U_nc(:,k),p(:,k),qv(:,k),landfrac(:),snowh(:),al_st_nc(:,k),G_nc(:,k),&
            drhsrf*0.0, ncol)  !zmh added drhsrf

      case ('iap')

          call astG_IAP(U_nc(:,k),p(:,k),qv(:,k),landfrac(:),snowh(:),al_st_nc(:,k),G_nc(:,k),&
            u00(:,k), drh00(:,k), lat, lon, pnps, ncol)  !zmh added drhsrf
      case default
          write(*,*)'--------- NO CLOUD scheme IS SELECTEDn cldwat2m_macro_pbl, STOPPED' 
          stop
      ...
      call aist_vector(qv(:,k),T(:,k),p(:,k),qi(:,k),landfrac(:),snowh(:),ai_st_nc(:,k),&
          u00(:,k), drh00(:,k), lat(:),lon(:),pnps(:),ncol)

      ai_st(:ncol,k)  =  (1._r8-a_cu(:ncol,k))*ai_st_nc(:ncol,k)
      al_st(:ncol,k)  =  (1._r8-a_cu(:ncol,k))*al_st(:ncol,k)
      a_st(:ncol,k)   =  max(al_st(:ncol,k),ai_st(:ncol,k))  

!---------------------------------------
if(macrop_scheme .eq. 'WXC')then

! update a_st, G_nc, a_cu, using PBL fractional cloudiness, X.Wang
      do i = 1, ncol
!        if( k.ge.cu_pbl_top(i) ) then 
        if( subgridon(i,k) ) then 
          if ( ( a_st_pbl_perturb(i,k) - a_st_pbl(i,k) ) .ne. 0 ) then

       G_nc(i,k)   = min( ( U(i,k)*1.01  - U(i,k) )/( a_st_pbl_perturb(i,k) - a_st_pbl(i,k) ),  &
                             ( U(i,k)*1.01  - U(i,k) ) * 1.e4_r8 )

             a_st(i,k)   = a_st_pbl(i,k)
             a_cu(i,k)   = 0._r8  
             dacudt(i,k) = 0._r8   
...
!
!write(*,*)'------- my1cld_mac, k, p(:,k)/100.',k, p(:,k)/100.
!write(*,*)'U, T, qv, drh00',U(:,k), T(:,k), qv(:,k), drh00(:,k)
!write(*,*)'a_cu(:ncol,k),al_st_nc(:,k),ai_st_nc(:,k)',a_cu(:,k),al_st_nc(:,k),ai_st_nc(:,k)
      ...
      do i = 1, ncol
       ...
       ! ------------------------------------------------------------ !
         ...
         if( QQ(i,k) .ge. 0._r8 ) then
             ...
             QQw(i,k) = QQ(i,k) * (1.0_r8 - zmh_fice(i,k))   !zmh
             QQi(i,k) = QQ(i,k) * zmh_fice(i,k)              !zmh
         else
             QQmin  = 0._r8
      ...
      enddo
      enddo
!write(*,*)'lat,lon,drhsrf',lat,lon,drhsrf
!rite(*,*)
   ...
   return
   end subroutine mmacro_pcond_pbl
   ...
   subroutine instratus_condensate( lchnk, ncol, k,                      &  
                                    ...
                                    landfrac, snowh,u00,drh00, lat, lon, pnps,            & !zmh added drhsrf
   ...
   real(r8), intent(in)  :: snowh(pcols)         ! Snow depth (liquid water equivalent)
!zmh
   real(r8), intent(in)  :: u00(pcols)   
   real(r8), intent(in)  :: drh00(pcols)   
   real(r8), intent(in)  :: lat(pcols)   
   real(r8), intent(in)  :: lon(pcols)   
   real(r8), intent(in)  :: pnps(pcols)   
...
!      if( CAMstfrac ) then
   select case ( CAMstfrac )
      ...
      case ('cam4' )
       call astG_RHU(U0_in(:),p_in(:),qv0_in(:),landfrac(:),snowh(:),al0_st_nc_in(:),G0_nc_in(:),&
            drh00*0.0, ncol)  !zmh added drhsrf
      case ( 'cam5' ) 
       call astG_PDF(U0_in(:),p_in(:),qv0_in(:),landfrac(:),snowh(:),al0_st_nc_in(:),G0_nc_in(:),&
            drh00*0.0, ncol)  !zmh added drhsrf

      case ('iap')
       call astG_IAP(U0_in(:),p_in(:),qv0_in(:),landfrac(:),snowh(:),al0_st_nc_in(:),G0_nc_in(:),&
            u00(:),drh00(:),lat(:), lon(:),pnps(:),ncol)  !zmh added drhsrf
      case default

   end select
!   endif

   call aist_vector(qv0_in(:),T0_in(:),p_in(:),qi0_in(:),landfrac(:),snowh(:),ai0_st_nc_in(:),&
       u00(:),drh00(:), lat(:),lon(:),pnps(:),ncol)
...
!      if( CAMstfrac ) then
   select case ( CAMstfrac )
      ...
      case ('cam4' )
              call astG_RHU_single(U0_nc,p,qv0,landfrac(i),snowh(i),al0_st_nc,G0_nc, drh00(i)*0.0)
      case ( 'cam5' ) 
              call astG_PDF_single(U0_nc,p,qv0,landfrac(i),snowh(i),al0_st_nc,G0_nc, drh00(i)*0.0)
      case ('iap')
              call astG_IAP_single(U0_nc,p,qv0,landfrac(i),snowh(i),al0_st_nc,G0_nc, u00(i),drh00(i),&
                      lat(i), lon(i), pnps(i))
      case default
   end select
...
!       endif
          ...
          call aist_single(qv0,T0,p,qi0,landfrac(i),snowh(i),ai0_st_nc,&
                  u00(i),drh00(i),lat(i),lon(i),pnps(i))
...
!      if( CAMstfrac ) then
   select case ( CAMstfrac )
      case ('cam4' )
                 call astG_RHU_single(U_nc,p,qv,landfrac(i),snowh(i),al_st_nc,G_nc,drh00(i)*0.0)
      case ( 'cam5' ) 
                 call astG_PDF_single(U_nc,p,qv,landfrac(i),snowh(i),al_st_nc,G_nc,drh00(i)*0.0)
      case ('iap')
                 call astG_IAP_single(U_nc,p,qv,landfrac(i),snowh(i),al_st_nc,G_nc,u00(i),drh00(i), &
                      lat(i),lon(i),pnps(i))
      case default
   end select
        !endif
...
!      if( CAMstfrac ) then
   select case ( CAMstfrac )
      ...
      case ( 'cam5' ) 
             call astG_PDF_single(U_nc,p,qv,landfrac(i),snowh(i),al_st_nc,G_nc,drh00(i)*0.0)
      case ('iap')
             call astG_IAP_single(U_nc,p,qv,landfrac(i),snowh(i),al_st_nc,G_nc,&
                  u00(i),drh00(i),lat(i), lon(i),pnps(i))
      case default
   end select
!         endif
   ...
   real(r8), intent(in)  :: u00,drh00,pnps
   ...
   call funcd_instratus( x1, p, T0, qv0, ql0, qi0, fice0, muQ0, qc_nc0, &
                         ...
                         qcst_crit, landfrac, snowh, u00, drh00 ,lat,lon,pnps,                   &
...
10 continue
   ...
   subroutine funcd_instratus( T, p, T0, qv0, ql0, qi0, fice0, muQ0, qc_nc0,   &
                               a_dc, ql_dc, qi_dc, a_sc, ql_sc, qi_sc, ai_st,  &
                               qcst_crit, landfrac, snowh,u00, drh00 ,  lat,lon,pnps,          & !zmh
...
!      if( CAMstfrac ) then
   select case ( CAMstfrac )
      case ('cam4' )
       call astG_RHU_single(U_nc,p,qv,landfrac,snowh,al_st_nc,G_nc,drh00*0.0)
      case ( 'cam5' ) 
       call astG_PDF_single(U_nc,p,qv,landfrac,snowh,al_st_nc,G_nc,drh00*0.0)

      case ('iap')
       call astG_IAP_single(U_nc,p,qv,landfrac,snowh,al_st_nc,G_nc,u00, drh00 ,lat,lon,pnps)
      case default
   end select
 !  endif
   al_st   =  (1._r8-a_dc-a_sc)*al_st_nc 
...
11 continue
   ...
   subroutine astG_PDF_single( U, p, qv, landfrac, snowh, a, Ga ,drhsrf)
...
!zmh
   ...
   subroutine astG_PDF( U_in, p_in, qv_in, landfrac_in, snowh_in, a_out, Ga_out, drhsrf,ncol )
   ...
   real(r8), intent(in)  :: drhsrf(pcols)
...
!zmh
   ...
   subroutine astG_RHU_single( U, p, qv, landfrac, snowh, a, Ga ,drhsrf)  !zmh added drhsrf
...
!zmh
   ...
   subroutine astG_RHU( U_in, p_in, qv_in, landfrac_in, snowh_in, a_out, Ga_out, drhsrf, ncol )
! zmh added drhsrf
   ! --------------------------------------------------------- !
   ! Compute 'stratus fraction(a)' and Gs=(dU/da) from the     !
   ! CAM35 cloud fraction formula.                             !
...
!zmh
   ...
   subroutine astG_IAP_single( U, p, qv, landfrac, snowh, a, Ga ,u00, drh00 ,lat,lon,pnps)  !zmh added drhsrf
   ...
   real(r8), intent(in)  :: landfrac        ! Land fraction
   real(r8), intent(in)  :: snowh           ! Snow depth (liquid water equivalent)
   real(r8), intent(in)  :: u00, drh00 ,lat, lon, pnps
...
!write(*,*)'my ast1',p/100.
!rite(*,*)'rhminl,stdrh,drh00',rhminl,stdrh,drh00


!pressure dependence
   !a2    = (p - 5.0e4_r8)/max((pnps - 5.0e4_r8),0.1)
   !a2    = max(a2,0._r8)

   !a     = a2*rhminl*1.7 + (1.-a2) !! 0.9 divided to facilitate the atm_in
   !a     = rhminl*1.4
!           ------------
   a = 1.0
   rhmin = U00 * a 
   rhmin = min(rhmin, 0.999_r8)

!vertical adjustment
   U2 =  min(U + drh00 ,1.) !vertical displacement
   U2 =  max(U2,0.01_r8)

!scheme
   a  = cld_func(1._r8,rhmin, U2, p)
   a2 = cld_func(1._r8,rhmin, U2-0.01_r8, p)

   if( (U2.ge.1._r8) .or. (U2.le.rhmin) ) then
    Ga = 1.e20_r8
   else
    Ga   = - 0.01/max((a2 - a),1.0e-22)
   endif

   return
   end subroutine astG_IAP_single

   ! ----------------- !
   ! End of subroutine !
   ! ----------------- !

   subroutine astG_IAP( U_in, p_in, qv_in, landfrac_in, snowh_in, a_out, Ga_out, &
           u00, drh00 , lat,lon,pnps,ncol )
   ! --------------------------------------------------------- !
   ! Compute 'stratus fraction(a)' and Gs=(dU/da) from the     !
   ! CAM35 cloud fraction formula.                             !
...
!write(*,*)'my ast2',p/100.
!write(*,*)'rhminl,stdrh,drh00',rhminl,stdrh,drh00

!pressure dependence

   !a2    = (p - 5.0e4_r8)/max((pnps(i) - 5.0e4_r8),0.1)  ! pressure dependent
   !a2    = max(a2,0._r8)
   !a     = a2*rhminl/0.8 + (1.-a2) !! 0.9 divided to facilitate the atm_in
...
!           ------------
   ...
   a = 1.0
   rhmin = u00(i) * a 
   rhmin = min(rhmin, 0.9999_r8)

!vertical adjustment
   U = min(U + drh00(i) ,1._r8) !drh00 vertical displacement
   U = max(U,0.01_r8)

!scheme

   a  = cld_func(1._r8,rhmin, U, p)
   a2 = cld_func(1._r8,rhmin, U-0.01_r8, p)
   ...
   enddo
!write(*,*)'rhmin, a, rhdif, ga, rpnps',rhmin, a, rhdif, ga, rpnps

   return
   end subroutine astG_IAP

   ! ----------------- !
   ! End of subroutine !
   ! ----------------- !

   subroutine aist_single( qv, T, p, qi, landfrac, snowh, aist , u00, drh00 ,lat,lon,pnps)
   ...
   real(r8), intent(in)  :: landfrac        ! Land fraction
   real(r8), intent(in)  :: snowh           ! Snow depth (liquid water equivalent)
   real(r8), intent(in)  :: u00
   real(r8), intent(in)  :: drh00
   real(r8), intent(in)  :: pnps
   real(r8), intent(in)  :: lat
   real(r8), intent(in)  :: lon
...
! zmh added
     elseif (iceopt.eq.6) then 
            ...
            a2 = zmh_ramp(T,258.16_r8, 15._r8)
            a2 = (1.-a2)*esl/esi+ a2
            U = (qv+qi)/qsat * a2

!pressure dependence

            !a2    = (6.0e4_r8 - p)/5.0e4_r8 
            !a2    = min( max(a2,0._r8), 1._r8)

            !a2     = a2*rhminh*0.95 + (1.-a2) ! 0.9 divided to facilitate the atm_in
            !a2     = rhminh/0.8  !*0.90 
!      ------------
            a2 = 1.0
            rhmini2 = u00 * a2
            rhmini2 = min(rhmini2,1.0_r8)

!temperature dependence of supersaturation
            a2 = zmh_ramp(T,243.0_r8, 30._r8)

             rhmaxi2 = a2 + (1.-a2)*zmh_rhmaxi 
!for S.H. ice supersaturation
            if(lat  .le. -0.349)then  !20S
            if((landfrac == 0._r8 .or. snowh .eq. 0.0) .and. (p/pnps .le. 0.8))then  !20S
             rhmaxi2 = a2 + (1.-a2)* min(zmh_rhmaxi + 0.2, 1.4_r8)
            endif 
            endif 


!vertical adjustment
            U = min(U + drh00 ,rhmaxi2) !vertical displacement
            U =  max(U,0.01_r8)
!scheme
            aist = cld_func(rhmaxi2,rhmini2,U,p)
...
!maximum
   ...
   subroutine aist_vector( qv_in, T_in, p_in, qi_in, landfrac_in, snowh_in, aist_out, &
            u00, drh00 , lat,lon, pnps, ncol)
...
!!temperature dependence of supersaturation
            a2 = zmh_ramp(T,200._r8, 73._r8)

              rhmaxi2 = a2 + (1.-a2)*zmh_rhmaxi 
!for S.H. ice supersaturation
            if(lat(i) .le. -0.349 )then  !20S
            if((landfrac == 0._r8 .or. snowh .eq. 0.0) .and. (p/pnps(i) .le. 0.8))then  !20S
              rhmaxi2 = a2 + (1.-a2)* min(zmh_rhmaxi + 0.2, 1.4_r8)
            endif
            endif
            if(lat(i) .ge. 1.106 )then  !60N
            if((landfrac == 0._r8 .or. snowh .eq. 0.0) .and. (p/pnps(i) .ge. 0.9))then  !60N
              rhmaxi2 = a2 + (1.-a2)* min(zmh_rhmaxi + 0.2, 1.4_r8)
            endif
            endif
!vertical adjustment
            U = min(U + drh00(i) ,rhmaxi2) !vertical displacement
            U =  max(U,0.01_r8)
!scheme
            aist = cld_func(rhmaxi2,rhmini2,U,p)

!            write(*,*)'rhmini2,rhmaxi2',rhmini2,rhmaxi2
!            write(*,*)'p,u,u00,aist',p/100.,u,u00,aist
     endif
...
!            write(*,*)'qi,minice,icimr,aist',qi,minice,aist
     endif 
...
!zmh ------ for IAP scheme and iceopt=6
! ------------------------------------
  real(r8) function cld_func(umax,umin,u, p)
! ------------------------------------
    implicit none
    REAL(r8) umax,umin,u,p
    REAL(r8) cld
    REAL(r8) umax2,umin2,x,a0


    umax2 = umax
    umin2 = min(umin, umax-1.0e-4_r8)

    !if(p .le. 5.0e4)then
    !    umin2 = umin2*0.5
    !endif
    !if((p .le. 8.0e4) .and. (p .ge. 5.e4))then
    !    umin2 = umin2*1.2
    !endif
   ...
   cld_func = a0 * x + (1.0_r8-a0) * x * x  !quadratic form with know derivative

  return

  end function cld_func

end module cldwat2m_macro_pbl
