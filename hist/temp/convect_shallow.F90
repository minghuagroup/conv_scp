...
!wxc zmh
   integer :: wush_idx, slflxsh_idx, qtflxsh_idx
...
!wxc zmh
  call pbuf_add('wush', 'physpkg', 1, pverp, 1, wush_idx)
  call pbuf_add('slflxsh', 'physpkg', 1, pverp, 1, slflxsh_idx)
  call pbuf_add('qtflxsh', 'physpkg', 1, pverp, 1, qtflxsh_idx)
...
!zmh
  call pbuf_add('MASSFLX_SH','global', 1, pver, 1, massflx_sh_idx)
...
!zmh
  call addfld( 'CMFMC2  '     , 'kg/m2/s ',  pverp,  'A' , 'Moist shallow convection mass flux'                        ,  phys_decomp )
  ...
  case('UW') ! Park and Bretherton shallow convection scheme
     ...
     massflx_sh_idx  = pbuf_get_fld_idx('MASSFLX_SH')

  case('ZW') ! modified UW scheme
     ...
     call init_zwshcu( r8, latvap, cpair, latice, zvir, rair, gravit, mwh2o/mwdry )
...
!zmh
  ...
  subroutine convect_shallow_tend( ztodt  , tpert   , qpert   , pblht    , & !zmh tpert
                                   cmfmc  , cmfmc2  , precc    , &
                                   qc     , qc2     , rliq     , rliq2    , & 
                                   snow   , state   , ptend_all,landfrac, pbuf ) !zmh
!                                   snow   , state   , ptend_all, pbuf )
   ...
   use zwshcu,          only : compute_zwshcu_inv
...
!zmh   real(r8) :: tpert(pcols)                                              ! PBL perturbation theta
...
!wxc   real(r8) :: slflx(pcols,pverp)                                        ! Shallow convective liquid water static energy flux
!   real(r8) :: qtflx(pcols,pverp)                                        ! Shallow convective total water flux
!wxc zmh
   real(r8), pointer                  :: wu_out(:,:)
   real(r8), pointer                  :: slflx(:,:)
   real(r8), pointer                  :: qtflx(:,:)
...
!zmh
   ...
   !real(r8), pointer, dimension(:) :: qpert
   real(r8), pointer, dimension(:) :: massflx_sh
...
!   if (lchnk==1155) then 
!      print*, '-------- At the beginning of sub. convect_shallow_tend ------------'
!      print*, 'state(1155)%q(8,1,17) =', state%q(8,1,17)
!      print*, '-------------------------------------------------------------------'
!   end if
!======================== zhh debug 2012-02-09 =======================
...
!zmh                   state%rpdel  ,  state%zm     ,  tpert      ,  qpert       ,  state%phis  ,   &
   ...
   case('UW')   ! UW shallow convection scheme
      ...
      cush => pbuf(cush_idx)%fld_ptr(1,1:pcols,1,lchnk,itim)
      tke  => pbuf(tke_idx)%fld_ptr(1,1:pcols,1:pverp,lchnk,itim)

      massflx_sh => pbuf(massflx_sh_idx)%fld_ptr(1, 1:pcols, 1, lchnk, 1)
...
!          tpert(:) = 0.2_r8
      ...
      flxprec    => pbuf(sh_flxprc_idx)%fld_ptr(1,1:pcols,1:pverp,lchnk,1)
      flxsnow    => pbuf(sh_flxsnw_idx)%fld_ptr(1,1:pcols,1:pverp,lchnk,1)
...
!wxc zmh
      ...
      wu_out    => pbuf(wush_idx)%fld_ptr(1,1:pcols,1:pverp,lchnk,1)
      slflx    => pbuf(slflxsh_idx)%fld_ptr(1,1:pcols,1:pverp,lchnk,1)
      qtflx    => pbuf(qtflxsh_idx)%fld_ptr(1,1:pcols,1:pverp,lchnk,1)
...
!wxc                               !cnt2                , cnb2           ,  fqsatd       , lchnk         , state%pdeldry )
                               cnt2                , cnb2           ,  fqsatd       , lchnk         , state%pdeldry, wu_out )
!======================== zhh debug 2012-02-09 =======================     
!!      if (lchnk==1155) then 
!         print*, '--------------------- after compute_zwshcu_inv --------------------'
!         print*, 'state(1155)%q(8,1,17) =', state%q(8,1,17)
!         print*, 'ptend_tracer(8,1,17) =', ptend_tracer(8,1,17)
!         print*, '-------------------------------------------------------------------'
!      end if
!======================== zhh debug 2012-02-09 =======================
...
!zmh
      pbuf(massflx_sh_idx)%fld_ptr(1,1:pcols,1,lchnk,1) = cbmf(1:pcols)
      ! Conservation check
   ...
   case('ZW')   ! modified UW shallow convection scheme


      cush => pbuf(cush_idx)%fld_ptr(1,1:pcols,1,lchnk,itim)
      tke  => pbuf(tke_idx)%fld_ptr(1,1:pcols,1:pverp,lchnk,itim)
!zmh
      !qpert => pbuf(qpert_idx)%fld_ptr(1, 1:pcols, 1, lchnk, 1)  ! note indices!
...
!wxc zmh
      ...
      call compute_zwshcu_inv( pcols     , pver    , ncol           , pcnst         , ztodt         ,                   &
                               state%pint, state%zi, state%pmid     , state%zm      , state%pdel    ,                   & 
                               state%u   , state%v , state%q(:,:,1) , state%q(:,:,ixcldliq), state%q(:,:,ixcldice),     &
                               state%t   , state%s , state%q(:,:,:) ,                                                   &
                               tke       , cld     , concld         , pblht         , cush          ,                   &
!zmh added input
                               state%omega,tpert    , qpert(:,1)  , massflx_sh, &
                               cmfmc2    , slflx   , qtflx          , 							&
			       flxprec, flxsnow, 			         					&
                               ptend_loc%q(:,:,1)  , ptend_loc%q(:,:,ixcldliq), ptend_loc%q(:,:,ixcldice),              &
                               ptend_loc%s         , ptend_loc%u    , ptend_loc%v   , ptend_tracer  ,                   &
...
!zmh
      ...
      ! -------------------------------------- !
      ! zwshcu does momentum transport as well !
      ! -------------------------------------- !
 ...
 ! Modification : I should check whether below computation of freqsh is correct.
   ...
   pbuf(rprdtot_idx)%fld_ptr(1,1:ncol,1:pver,lchnk,1) = rprdsh(:ncol,:pver) + rprddp(:ncol,:pver)
...
!zmh
   call outfld( 'CMFMC2' , cmfmc2                    , pcols   , lchnk )
...
!======================== zhh debug 2012-02-09 =======================
!      if (lchnk==1155) then 
!         print*, '--------------------- after physics_update --------------------'
!         print*, 'state1(1155)%q(8,1,17) =', state1%q(8,1,17)
!         print*, '-------------------------------------------------------------------'
!      end if
!======================== zhh debug 2012-02-09 =======================
   ...
   ! evaporation physics inside in it. So when 'shallow_scheme = UW, ZW', we must !
   ...
   ! ------------------------------------------------------------------------------- !
    ...
    flxprec    => pbuf(sh_flxprc_idx)%fld_ptr(1,1:pcols,1:pverp,lchnk,1)
    flxsnow    => pbuf(sh_flxsnw_idx)%fld_ptr(1,1:pcols,1:pverp,lchnk,1)
    sh_cldliq  => pbuf(sh_cldliq_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    sh_cldice  => pbuf(sh_cldice_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
...
!======================== zhh debug 2012-02-09 =======================
!   if (lchnk==1155) then 
!      print*, '------------ At the end of sub. convect_shallow_tend --------------'
!      print*, 'state(1155)%q(8,1,17) =', state%q(8,1,17)
!      print*, '-------------------------------------------------------------------'
!   end if
!======================== zhh debug 2012-02-09 =======================
