...
!
! July 2018 modified by Minghua Zhang to interface with ZYX1_CONV.F90
!---------------------------------------------------------------------------------
...
! zmh
   use zyx1_conv,     only: zyx1_conv_init, zyx1_conv_tend, zyx1_conv_evap, convtran, momtran
   use zm_conv,       only: zm_convr
   use mzfunctions_mod, only: fout_phy
...
public :: zyx1_conv_intr_init, zyx1_conv_intr_register, zyx1_conv_intr_tend, zyx1_conv_tend_2
...
subroutine zyx1_conv_intr_register
...
!zmh
!   call pbuf_add('qpert','global', 1, 1, 1, qpert_idx)
   ...
   call pbuf_add('DEEPFRC','global', 1, pver, 1, deepfrc_idx)

end subroutine zyx1_conv_intr_register

!=========================================================================================

subroutine zyx1_conv_intr_init(hypi)
...
!----------------------------------------
  ...
  use zyx1_conv,      only: zyx1_conv_init
...
!
    ...
    call addfld ('md0', '',     pver, 'I', 'md0',    phys_decomp)
    call addfld ('mu0', '',     pver, 'I', 'mu0',    phys_decomp)
    call addfld ('eu0', '',     pver, 'I', 'eu0',    phys_decomp)
    call addfld ('du0', '',     pver, 'I', 'du0',    phys_decomp)
    call addfld ('eps0', '',     pver, 'I', 'eps0',    phys_decomp)
    call addfld ('dd0', '',     pver, 'I', 'dd0',    phys_decomp)
    call addfld ('ed0', '',     pver, 'I', 'ed0',    phys_decomp)
    call addfld ('md1', '',     pver, 'I', 'md1',    phys_decomp)
    call addfld ('mu1', '',     pver, 'I', 'mu1',    phys_decomp)
    call addfld ('eu1', '',     pver, 'I', 'eu1',    phys_decomp)
    call addfld ('du1', '',     pver, 'I', 'du1',    phys_decomp)
    call addfld ('ed1', '',     pver, 'I', 'ed1',    phys_decomp)
    call addfld ('dd1', '',     pver, 'I', 'dd1',    phys_decomp)
    call addfld ('ent_turb', '',     pver, 'I', 'ent_turb',    phys_decomp)
    call addfld ('det_turb', '',     pver, 'I', 'det_turb',    phys_decomp)
    call addfld ('ent_org', '',     pver, 'I', 'ent_org',    phys_decomp)
    call addfld ('det_org', '',     pver, 'I', 'det_org',    phys_decomp)
    call addfld ('ent_tot', '',     pver, 'I', 'ent_tot',    phys_decomp)
    call addfld ('det_tot', '',     pver, 'I', 'det_tot',    phys_decomp)
    call addfld ('w_up', '',     pver, 'I', 'w_up',    phys_decomp)
    call addfld ('cldsr', '',     pver, 'I', 'cldsr',    phys_decomp)
    call addfld ('buoy_up', '',     pver, 'I', 'buoy_up',    phys_decomp)
    call addfld ('buoy_dn', '',     pver, 'I', 'buoy_dn',    phys_decomp)
#endif
...
!zmh
    ...
    call zyx1_conv_init(limcnv,no_deep_pbl_in = no_deep_pbl)
...
! zmh
    ...
    deepfrc_idx    = pbuf_get_fld_idx('DEEPFRC')
    !qpert_idx  = pbuf_get_fld_idx('QPERT')
...
end subroutine zyx1_conv_intr_init
!=========================================================================================

!subroutine zyx1_conv_intr_tend(prec    , &
!     pblh    ,mcon    ,cme     ,          &
!     tpert   ,dlf     ,pflx    ,zdu      , &
!     rliq    , &
!     ztodt   ,snow    ,&
...
!     state   ,ptend_all   ,landfrac   ,pbuf  )
 ...
 subroutine zyx1_conv_intr_tend(ztodt, landfrac, lhflx, shflx, pblh, tpert,qpert,vort3, &
   ...
   use time_manager,  only: get_nstep, is_first_step, get_curr_calday  !zmh
...
!wxc zmh
   ...
   real(r8) :: md0(pcols, pver),  mu0(pcols, pver), eu0(pcols, pver), du0(pcols, pver), &
               ...
               det_org(pcols, pver) , ent_tot(pcols, pver),  det_tot(pcols, pver), &
...
!zmh
   tke => pbuf(tke_idx)%fld_ptr(1, 1:ncol, 1:pverp, lchnk, itim)
   deepfrc => pbuf(deepfrc_idx)%fld_ptr(1, 1:pcols, 1:pver, lchnk, 1)
   !qpert => pbuf(qpert_idx)%fld_ptr(1, 1:ncol, 1, lchnk, itim)
   !massflxbase_p => pbuf(massflxbase_p_idx)%fld_ptr(1, 1:pcols, 1:pver, lchnk, itim)
   !bfls_t => pbuf(bfls_t_idx)%fld_ptr(1, 1:pcols, 1:pver, lchnk, itim)
   !bfls_q => pbuf(bfls_q_idx)%fld_ptr(1, 1:pcols, 1:pver, lchnk, itim)
...
!qpert(:) = 0.0
i=1
if(i<0 .and. nstep < 10)then
write(*,*)
write(*,*)'zyx1_intr before zyx1_conv nstep, lchnk=',nstep, lchnk
write(*,*)
write(*,*)'qpert',qpert
write(*,*)'tpert',tpert
write(*,*)'lhflx, shflx',lhflx,shflx
write(*,*)'tke',tke !(:,pver)
write(*,*)'bfls_t',bfls_t
write(*,*)'bfls_q',bfls_q
write(*,*)'mb_p  ',massflxbase_p
endif

i=1
!if(nstep==2)then  ! for IDL processing phy_proc1.pro
if(nstep<0)then
   ...
   call fout_phy(qpert(1:ncol),lchnk,ncol,1,state%lat,state%lon,'qpert')
...
!i=1
!if(i>0)then
   call zyx1_conv_tend(   lchnk   ,ncol       ,nstep,                                       &
                    ...
                    .5_r8*ztodt                                                           , &
                    state%lat     ,landfrac   ,lhflx        ,shflx                        , & !input 
                    state%ps      ,bfls_t     ,bfls_q                                     , &
                    omega         ,tpert      ,qpert, vort3                             , &
                    tke(:,pver)                                            , & 
                    massflxbase_p ,                                                         & !out 
                    mcon          ,cme        , cape                                      , &
                    dlf           ,pflx       ,zdu           ,rprd                        , &
                    ...
                    lengath(lchnk) ,ql      ,rliq  , slflx, qtflx, deepfrc & 
...
i=1
if(i<0 .and. nstep < 3)then
write(*,*)'zyx1_conv_intr before call zyx1'
write(*,*)'nstep, lchnk=',nstep, lchnk
write(*,*)
write(*,*)'qpert',qpert
write(*,*)'tpert',tpert
write(*,*)'tke',tke(:,pver)
...
write(*,*)'nstep, lchnk=',nstep, lchnk
write(*,*)
write(*,*)'ulat',state%lat*180./3.1416
write(*,*)
write(*,*)'ulon',state%lon*180./3.1416
write(*,*)
write(*,*)'landfrac',landfrac
write(*,*)
write(*,*)'tpert',landfrac
write(*,*)
endif
...
if(i<0)then
write(*,*)'nstep=',nstep
write(*,*)'zyx1_conv_intr after call zyx1'
!write(*,*)'t',state%t 
!write(*,*)'q',state%q(:,:,1)
!write(*,*)'pbl',pblh
write(*,*)'tpert',tpert
...
!
#if (defined WRITE_OUT)
   call outfld('md0',md0 , pcols, lchnk) 
   call outfld('mu0',mu0 , pcols, lchnk) 
   call outfld('eu0',eu0 , pcols, lchnk) 
   call outfld('du0',du0 , pcols, lchnk) 
   call outfld('eps0',eps0 , pcols, lchnk) 
   call outfld('dd0',dd0 , pcols, lchnk) 
   call outfld('ed0',ed0 , pcols, lchnk) 
   call outfld('eu1',eu1 , pcols, lchnk) 
   call outfld('du1',du1 , pcols, lchnk) 
   call outfld('dd1',dd1 , pcols, lchnk) 
   call outfld('ed1',ed1 , pcols, lchnk) 
   call outfld('md1',md1 , pcols, lchnk) 
   call outfld('mu1',mu1 , pcols, lchnk) 
   call outfld('ent_turb',ent_turb , pcols, lchnk) 
   call outfld('det_turb',det_turb , pcols, lchnk) 
   call outfld('ent_org' ,ent_org , pcols, lchnk) 
   call outfld('det_org', det_org , pcols, lchnk) 
     ent_tot = ent_org + ent_turb
     det_tot = det_org + det_turb
   call outfld('ent_tot',ent_tot , pcols, lchnk) 
   call outfld('det_tot',det_tot ,pcols, lchnk) 
   call outfld('w_up',w_up , pcols, lchnk) 
   call outfld('cldsr',cldsr , pcols, lchnk) 
   call outfld('buoy_up',buoy_up , pcols, lchnk) 
   call outfld('buoy_dn',buoy_dn , pcols, lchnk) 
#endif
...
!
    ...
    call zyx1_conv_evap(state1%ncol,state1%lchnk, &
...
! zmh
  pbuf(bfls_t_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,1) = state1%t
  pbuf(bfls_q_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,1) = state1%q(:,:,1)
  pbuf(massflxbase_p_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,1) = massflxbase_p
  !pbuf(bfls_t_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,itim) = state1%t
  !pbuf(bfls_q_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,itim) = state1%q(:,:,1)
  !pbuf(massflxbase_p_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,itim) = massflxbase_p

end subroutine zyx1_conv_intr_tend
!=========================================================================================


subroutine zyx1_conv_tend_2( state,  ptend,  ztodt, pbuf  )
...
end subroutine zyx1_conv_tend_2

!=========================================================================================



end module zyx1_conv_intr
