...
!+czy20181120
!=============Jinbo Xie=====================
!czy20181120use module_bl_gwdo_gsd, only: gwdo_gsd,gwdo2d
use gw_drag_xjb,        only: gwdo_gsd,gwdo2d !czy20181120
use gw_drag,            only: gw_drag_scheme  !czy20181120
...
!-czy20181120
  ...
  character(len=16)    :: shallow_scheme               ! For checking compatibility between eddy diffusion and shallow convection schemes
                                                       ...
                                                       !     'ZW'  = Modified Park and Bretherton ( UW Shallow Convection Scheme )
...
!zmh
  integer              :: tpert_idx, qpert_idx
!wxc
  integer              :: slflxpbl_idx, qtflxpbl_idx
  integer              :: lengi_idx                    ! Turbulence length
...
!simply add par
        ...
        real(r8) :: ztop(pcols,pver) = 0.0_r8            ! top interface height asl(m)
        real(r8) :: zbot(pcols,pver) = 0.0_r8            ! bottom interface height asl(m)
        real(r8) :: zmid(pcols,pver) = 0.0_r8            ! middle interface height asl(m)
        real(r8) :: dz(pcols,pver)   = 0.0_r8
!==========Jinbo Xie==================
!-czy20181120
  contains
...
! zmh
!    call pbuf_add( 'TPERT',      'global',  1,  1,  pbuf_times,  tpert_idx ) 
!    call pbuf_add( 'QPERT',      'global',  1,  1,  pbuf_times,  qpert_idx ) 
    !call pbuf_add( 'QPERT',      'global',  1,  pcnst,  1,  qpert_idx ) 
!wxc
    call pbuf_add( 'slflxpbl',      'global',  1,  pverp,  pbuf_times,  slflxpbl_idx )
    call pbuf_add( 'qtflxpbl',      'global',  1,  pverp,  pbuf_times,  qtflxpbl_idx )
    call pbuf_add( 'lengi',    'global',  1,  pverp,  pbuf_times,  lengi_idx )
    !call pbuf_add( 'slflxpbl',      'global',  1,  pverp,  1,  slflxpbl_idx )
    !call pbuf_add( 'qtflxpbl',      'global',  1,  pverp,  1,  qtflxpbl_idx )
    !call pbuf_add( 'lengi',    'global',  1,  pverp,  1,  lengi_idx )
...
!+czy20181120
        if (gw_drag_scheme == 3) then ! xjb gw drag scheme
        !=========Jinbo Xie===========
...
!         if( k == lwaterptr_amode(m) ) go to 20
  ...
  subroutine vertical_diffusion_tend( &
    ...
    use eddy_diff,          only : compute_eddy_diff, tfac, wfac, wpertmin !zmh added tfac and wfac
...
!+czy20181120
    !=======Jinbo Xie===========
    real(r8) :: rlat(pcols)     = 0.0_r8             ! latitude in radians for columns
    integer :: kpbl2d_in(pcols) = -1
    real(r8) :: ttgw(pcols,pver)= 0.0_r8                  ! temperature tendency
    real(r8) :: utgw(pcols,pver)= 0.0_r8                  ! zonal wind tendency
    real(r8) :: vtgw(pcols,pver)= 0.0_r8                  ! meridional wind tendency
    !=======Jinbo Xie================
    real(r8) :: dtaux3_fd(pcols,pver)  = 0.0_r8
    real(r8) :: dtauy3_fd(pcols,pver)  = 0.0_r8
    real(r8) :: dusfc_fd(pcols)        = 0.0_r8
    real(r8) :: dvsfc_fd(pcols)        = 0.0_r8
    !=======Jinbo Xie================
!-czy20181120
...
!+czy20181120
        if (gw_drag_scheme == 3) then!czy20181120 use xjb gw drag scheme
        !================Jinbo Xie=======================
 ...
 ! czy 20181120 kpbl2d_in=0_r8
        ...
        kpbl2d_in(i)=pblh_get_level_idx(zbot(i,pver:1:-1),pblh(i))
        kpbl2d_in(i)=pver+1-kpbl2d_in(i)!switch from top to bottom start
        end do
	...
	pi3d=state%exner(:,pver:1:-1),z=ztop,&
...
!=============Jinbo Xie=====================
        endif ! czy20181120 endif gw_drag_sheme
...
!-czy20181120
    ...
    case ( 'diag_TKE' ) 
       ...
       call compute_eddy_diff( lchnk    ,                                                                    &
                               ...
                               ipbl(:)  , kpblh(:)    , wstarPBL(:), turbtype   , smaw ,lengi) !wxc
...
!write(*,*)'qpert',qpert
       ...
       !print *, "tke in diffusion, xwang", tke
...
!wxc
        pbuf(lengi_idx)%fld_ptr(1,1:ncol,1:pverp,lchnk,time_index)     = lengi(:ncol,:)
        !pbuf(lengi_idx)%fld_ptr(1,1:ncol,1:pverp,lchnk,1)     = lengi(:ncol,:)
...
!zmh
!       pbuf(tpert_idx)%fld_ptr(1,1:ncol,1,lchnk,1)      = tpert(:)
!       pbuf(qpert_idx)%fld_ptr(1,1:ncol,1,lchnk,1)      = qpert(:)
...
!write(*,*)'myvert2 tpert, qpert ustar',tpert, qpert, ustar
!write(*,*)'shflx cflx',shflx,cflx
    do k = 2, pver
       do i = 1, ncol
          ...
          vflx_cg(i,k)  = 0._r8
!!zmh !!!!!!!!!!
!!          if(k .ge. kpblh(i))then
!!           wpert(i)  = max(wfac*sqrt(tke(i,k)),wpertmin)
!!           tpert(i)  = max((slflx(i,k)*rhoair/cpair)*tfac/wpert(i),tpert(i) )
!!           qpert(i)  = max((qtflx(i,k)*rhoair)*tfac/wpert(i), qpert(i))
!           !write(*,*)'k, slflx(i),qflx(i),wpert(i),tke(i)',k, slflx(i,k),qtflx(i,k),wpert(i),tke(i,k)
!           !write(*,*)'k, tpert,qpert',k,tpert,qpert
!!          endif
       end do
    end do
!write(*,*)'myvert3 tpert, qpert',tpert, qpert
...
!wxc
     pbuf(slflxpbl_idx)%fld_ptr(1,1:ncol,1:pverp,lchnk,time_index) = slflx(1:ncol,1:pverp)
     pbuf(qtflxpbl_idx)%fld_ptr(1,1:ncol,1:pverp,lchnk,time_index) = qtflx(1:ncol,1:pverp)
     !pbuf(slflxpbl_idx)%fld_ptr(1,1:ncol,1:pverp,lchnk,1) = slflx(1:ncol,1:pverp)
     !pbuf(qtflxpbl_idx)%fld_ptr(1,1:ncol,1:pverp,lchnk,1) = qtflx(1:ncol,1:pverp)
...
!if (.not.found) call endrun("Cannot find pblh index!")
        !if (.not.found) then
        !!write(*,*)"cannot find pblh_index!"
        !!write(*,*) pblheight
        !!write(*,*) height_array(1)
        !!write(*,*) height_array(1+1)
        !!write(*,*) height_array(pver)
        !end if
end function
!================================Jinbo Xie====================
!-czy20181120
