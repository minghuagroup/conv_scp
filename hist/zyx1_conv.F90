...
!zmh =================================
   ...
   integer  :: niter = 6
   integer, dimension(8) :: deep_trigger_flags = (/1, 0, 0, 1, 0, 0, 0, 1/)
...
!zmh
   ...
   capelmt(:) = zyx_cape_lmt*(1._r8+rand_num(:)) 
...
case('zmh_triggers') !additional limit
    ...
    do i=1,ncol
     deep_triggers(:) = .true.
       ...
       deep_triggers(1) = (cape(i)  > capelmt(i) )

!     !if(deep_triggers(2)==1)then 
       if(deep_trigger_flags(2) == 1)then 
!    -------------------------------------------------------
      call zyx1_buoyan_dilute(lchnk   ,ncol    , limcnv, &
...
!        if(deep_triggers(3))then
       if(deep_trigger_flags(3) == 1)then 
...
!
   ...
   do k = msg + 1,pver
      do i = 1,il2g
         if ((k >= jd(i) .and. k <= jd3(i)) .and.   md(i,jb(i))< -0.01_r8 ) then
            ratmjb(i) = min(abs(mu(i,jb1(i))/md(i,jd2(i))),1._r8)
