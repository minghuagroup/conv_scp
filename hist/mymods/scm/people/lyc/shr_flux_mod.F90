...
! SVN $URL: https://svn-ccsm-models.cgd.ucar.edu/csm_share/branch_tags/cesm1_0_rel_tags/cesm1_0_3_n01_share3_110527/shr/shr_flux_mod.F90 $
!===============================================================================
!BOP ===========================================================================
!
...
! mhzhang 
   real(R8)   ,intent(in) :: sss   (nMax) ! sea surface salinity, unit in `PSU?
                                          ! AT(I,J,1,2)*1000.0+35.0

#ifdef CO2 
   real(R8)   ,intent(in) :: pco2a(nMax) ! wet atm CO2 partial pressure (PPM) 
                                         ! the unit is concentration, in ppmv or ppmm?
                     ! this should be from atm state%q for CO2 same as in flux_pt.F90

   real(R8)   ,intent(in) :: pco2o(nMax) ! ocn CO2 partial pressure (PPM?)
                     !this should be the pco2o calculated in flux_pt.F90

   real(R8)   ,intent(out)::  ssfc(nMax) !  CO2 flux to the ocean
                                         ! m/s *umol/kg (in LICOM)
#endif
...
! mhzhang local variables
   real(R8)    :: pco2dry2, ES, pco2a_w, tt, ss, dpco2      
...
#ifdef CO2
        !------------------------------------------------------------
        ! CO2 flux to the ocean
        !------------------------------------------------------------

        pco2dry2 = pco2a(n)
        ES       = EXP(20.1050 - 9.7982E-3*TS(n) - 6163.10/TS(n))
        pco2a_w  = pco2dry2 * (1.0 - ES)

        tt       = ts(n) - 273.15_r8  
        ss       = (sss(n) - 35.0)/1000.
        sge      = SGEC(tt,ss,u10n,sge)

        dpco2    = pco2a_w-pco2o(n)  ! different units can be unified here
        ssfc(n)  = sge*dpco2o
#endif
