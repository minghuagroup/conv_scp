...
!     end if
      ...
      if (present(init)) then
         ...
         do g = begg, endg
          ...
          ! numrad wavebands (1=vis, 2=nir)
            ...
            clm_l2a%ram1(g)           = 0._r8
            clm_l2a%fv(g)             = 0._r8
            clm_l2a%flxdst(g,:)       = 0._r8
            clm_l2a%flxvoc(g,:)       = 0._r8
         !  clm_l2a%ddvel(g,:)        = ???

            if(isnan(lnd_nee(i))) write(6,*) 'lnd_nee is NaN'
         end do
      end if
   ...
   subroutine build_clm_a2l()
      ...
      do g = begg, endg
         ...
         forcg%pco2m(i) = clm_a2l%forc_pco2(g)

         forcg%po2m (i) = clm_a2l%forc_po2(g)
         forcg%us   (i) = clm_a2l%forc_u(g)
         forcg%vs   (i) = clm_a2l%forc_v(g)
         forcg%tm   (i) = clm_a2l%forc_t(g)
         forcg%qm   (i) = clm_a2l%forc_q(g)

         forcg%prc  (i) = clm_a2l_rainc(g) + clm_a2l_snowc(g)
         forcg%prl  (i) = clm_a2l_rainl(g) + clm_a2l_snowl(g)
         forcg%rain (i) = clm_a2l_rainc(g) + clm_a2l_rainl(g)
         forcg%snow (i) = clm_a2l_snowc(g) + clm_a2l_snowl(g)

         forcg%pbot (i) = clm_a2l%forc_pbot(g)
         forcg%psrf (i) = clm_a2l%forc_pbot(g)

         forcg%sols (i) = clm_a2l%forc_solad(g,1) ! bufR(g,index_c2l_Faxa_swvdr)  !forc_solsxy  Atm flux  W/m^2
         forcg%soll (i) = clm_a2l%forc_solad(g,2) ! bufR(g,index_c2l_Faxa_swndr)  !forc_sollxy  Atm flux  W/m^2
         forcg%solsd(i) = clm_a2l%forc_solai(g,1) ! bufR(g,index_c2l_Faxa_swvdf)  !forc_solsdxy Atm flux  W/m^2
         forcg%solld(i) = clm_a2l%forc_solai(g,2) ! bufR(g,index_c2l_Faxa_swndf)  !forc_solldxy Atm flux  W/m^2

         forcg%frl  (i) = clm_a2l%forc_lwrad(g)
         forcg%hu   (i) = clm_a2l%forc_hgt_u(g)
         forcg%ht   (i) = clm_a2l%forc_hgt_t(g)
         forcg%hq   (i) = clm_a2l%forc_hgt_q(g)
      end do
...
! ======================================================================
       ...
       ! Surface states sent to the flux coupler states are not time averaged
         ...
         CALL writehistdat(rstwr,nlend,rdate)
