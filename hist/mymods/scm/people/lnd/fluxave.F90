...
!----------------------------------------------------------------------
 ...
 ! The following two subroutines are added by Zhang Minghua
...
! zmh added two more fields     
      !fldv%fsena2 and fldv%lfevpa2

      call flux_p2g_dev(numpatch,numgrid,pgmap,wxy_patch,fldv_pft%fsena  ,fldv%fsena ,fldv%fsena2  )
      call flux_p2g_dev(numpatch,numgrid,pgmap,wxy_patch,fldv_pft%lfevpa ,fldv%lfevpa,fldv%lfevpa2 )
...
!zmh added two fields
      fldv%tref2 and fldv%qref2
      call flux_p2g_grass_dev(numpatch,numgrid,pgmap,ivt,wxy_patch,fldv_pft%tref,fldv%tref,fldv%tref2)
      call flux_p2g_grass_dev(numpatch,numgrid,pgmap,ivt,wxy_patch,fldv_pft%qref,fldv%qref,fldv%qref2)
    ...
    ! Special treatment for land cover fraction
      ...
      do g = 1, numgrid
       ...
       ! if(abs(a-1._r8).gt.1.e-6_r8) then   ! jidy@12/Sep/2014
         if(abs(a-landfrac(gxmap(g),gymap(g))).gt.1.e-6_r8) then   ! jidy@12/Sep/2014
