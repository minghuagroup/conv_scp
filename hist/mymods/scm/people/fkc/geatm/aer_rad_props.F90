...
use rad_constituents, only: rad_cnst_get_info, rad_cnst_get_aer_mmr, &
                            ...
                            new_rad_cnst_get_aer_mmr !! added by fkc
...
subroutine aer_rad_props_init()
   ...
   call add_default( 'GEATM_dust1', 1, ' ' )
   call add_default( 'GEATM_dust2', 1, ' ' )
   call add_default( 'GEATM_dust3', 1, ' ' )
   call add_default( 'GEATM_dust4', 1, ' ' )
   call add_default( 'GEATM_sslt1', 1, ' ' )
   call add_default( 'GEATM_sslt2', 1, ' ' )
   call add_default( 'GEATM_sslt3', 1, ' ' )
   call add_default( 'GEATM_sslt4', 1, ' ' )
   call add_default( 'GEATM_bcar1', 1, ' ' )
   call add_default( 'GEATM_bcar2', 1, ' ' )
   call add_default( 'GEATM_sulf', 1, ' ' )
   call add_default( 'GEATM_ocar1', 1, ' ' )
   call add_default( 'GEATM_ocar2', 1, ' ' )
  ! end
...
! Return bulk layer tau, omega, g, f for all spectral intervals.
   ...
   if(list_idx==0) then
      call modal_aero_sw(state, pbuf, nnite, idxnite, &
                         tau, tau_w, tau_w_g, tau_w_f, aernames) ! aernames added by fkc
   else
      tau    (1:ncol,:,:) = 0._r8
      tau_w  (1:ncol,:,:) = 0._r8
      tau_w_g(1:ncol,:,:) = 0._r8
...
#ifdef MODAL_AERO
   if(list_idx==0) then
      call modal_aero_lw(state, pbuf, odap_aer,aernames)
   else
      odap_aer = 0._r8
   end if
...
subroutine gea_aer_out(lchnk, aermass, aername)
      ...
      real(r8)                   , intent(in):: aermass(pcols,pver)     ! mass ofaerosols
