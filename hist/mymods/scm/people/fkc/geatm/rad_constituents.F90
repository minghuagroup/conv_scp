...
! added by fkc
use geatm_aerosol_cycle,  only: gptr,g_names
! end 
implicit none
private
save
...
!         write(iulog,*) 'aername is ',aernames(i)
      end do
...
!==============================================================================================
! added by fkc 
! This is a new subroutine which can read geatm aerosol mmr instead of reading
! cam aerosol mmr
subroutine new_rad_cnst_get_aer_mmr(list_idx, aer_idx,aername, state, pbuf, aermass)
   ...
   real(r8),                    pointer    :: aermmr(:,:), aermmr1(:,:)
   real(r8)                   , intent(out):: aermass(pcols,pver)     ! mass of aerosols
   ...
   character(len=1) :: source
   type(aerlist_t), pointer :: aerlist
   character(len=*), parameter :: subname = 'new_rad_cnst_get_aer_mmr'
   ...
   source = aerlist%aer(aer_idx)%source
   idx    = aerlist%aer(aer_idx)%idx
   aidx=pbuf_get_fld_idx(aername)
   select case( source )
   case ('P')
      aermmr => state%q(:,:,idx)
      aermass(1:pcols,1:pver) = aermmr(1:pcols,1:pver)
   case ('D')
!          write(iulog,*)'aidx is ', aidx, 'name is ', aername
      if (trim(aername)=='dust1') then
          aermmr => pbuf(gptr(1))%fld_ptr(1,:,:,lchnk,1)
          aermass(1:pcols,1:pver) = aermmr(1:pcols,1:pver)
      else if (trim(aername)=='dust2') then
          aermmr => pbuf(gptr(2))%fld_ptr(1,:,:,lchnk,1)
          aermass(1:pcols,1:pver) = aermmr(1:pcols,1:pver)
      else if (trim(aername)=='dust3') then
          aermmr => pbuf(gptr(3))%fld_ptr(1,:,:,lchnk,1)
          aermass(1:pcols,1:pver) = aermmr(1:pcols,1:pver)
      else if (trim(aername)=='dust4') then
          aermmr => pbuf(gptr(4))%fld_ptr(1,:,:,lchnk,1)
          aermass(1:pcols,1:pver) = aermmr(1:pcols,1:pver)
      else if (trim(aername)=='SSLTA') then
          aermmr => pbuf(gptr(5))%fld_ptr(1,:,:,lchnk,1)
          aermmr1=> pbuf(gptr(6))%fld_ptr(1,:,:,lchnk,1)
          aermass(1:pcols,1:pver) = aermmr(1:pcols,1:pver)+aermmr1(1:pcols,1:pver)
      else if (trim(aername)=='SSLTC') then
          aermmr => pbuf(gptr(7))%fld_ptr(1,:,:,lchnk,1)
          aermmr1=> pbuf(gptr(8))%fld_ptr(1,:,:,lchnk,1)
          aermass(1:pcols,1:pver) = aermmr(1:pcols,1:pver)+aermmr1(1:pcols,1:pver)
      else if (trim(aername)=='bcar1') then
          aermmr => pbuf(gptr(10))%fld_ptr(1,:,:,lchnk,1)
          aermass(1:pcols,1:pver) = aermmr(1:pcols,1:pver)*0.028_r8
      else if (trim(aername)=='bcar2') then
          aermmr => pbuf(gptr(10))%fld_ptr(1,:,:,lchnk,1)
          aermass(1:pcols,1:pver) = aermmr(1:pcols,1:pver)*0.972_r8
      else if (trim(aername)=='sulf') then
          aermmr => pbuf(gptr(14))%fld_ptr(1,:,:,lchnk,1)
          aermass(1:pcols,1:pver) = aermmr(1:pcols,1:pver)
      else if (trim(aername)=='ocar1') then
          aermmr => pbuf(gptr(11))%fld_ptr(1,:,:,lchnk,1)
          aermass(1:pcols,1:pver) = aermmr(1:pcols,1:pver)*0.016_r8
      else if (trim(aername)=='ocar2') then
          aermmr => pbuf(gptr(11))%fld_ptr(1,:,:,lchnk,1)
          aermass(1:pcols,1:pver) = aermmr(1:pcols,1:pver)*0.984_r8
      else
          aermmr => pbuf(gptr(13))%fld_ptr(1,:,:,lchnk,1)
          aermass(1:pcols,1:pver) = aermmr(1:pcols,1:pver)
!          call endrun( subname// ':: NOT in aerosol namelist')
      end if
   end select
!   maxaer=maxval(aermass)
!   write(iulog,*)'name is ', aername 
!   write(iulog,*)'max aermass is ', maxaer
end subroutine new_rad_cnst_get_aer_mmr
