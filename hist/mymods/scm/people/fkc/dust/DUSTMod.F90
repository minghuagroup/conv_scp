...
!!!! modified by LiuXiaoHong's Group, using Kok14 dust emission scheme
!
! !DESCRIPTION: 
...
!    real(r8) :: wnd_frc_rat         ! [frc] Wind friction threshold over wind friction -variable no longer needed, -jfk
...
!    real(r8) :: dst_slt_flx_rat_ttl  !variable no longer needed, -jfk
!    real(r8) :: flx_mss_hrz_slt_ttl  !variable no longer needed, -jfk
    ...
    real(r8) :: Cd                  ! [dimless] The dust emission coefficient, which depends on the soil's standardized threshold friction speed -jfk
    real(r8) :: wnd_frc_slt
...
! initialize variables which get passed to the atmosphere
    ...
    do fp = 1,num_nolakep
       ...
       if (lnd_frc_mbl(p) > 0.0_r8) then
          ...
          wnd_frc_thr_slt_std = wnd_frc_thr_slt * sqrt(forc_rho(g) / forc_rho_std) ! standardized soil threshold friction speed -jfk

          ! reset these variables which will be updated in the following if-block
          
          wnd_frc_slt = fv(p)
          !flx_mss_hrz_slt_ttl = 0.0_r8 !variable no longer needed, -jfk
          ...
          if (wnd_frc_slt > wnd_frc_thr_slt) then
             !wnd_frc_rat = wnd_frc_thr_slt / wnd_frc_slt !variable no longer needed, -jfk
             !flx_mss_hrz_slt_ttl = cst_slt * forc_rho(g) * (wnd_frc_slt**3.0_r8) * &
             !     (1.0_r8 - wnd_frc_rat) * (1.0_r8 + wnd_frc_rat) * (1.0_r8 + wnd_frc_rat) / grav !variable no longer needed, -jfk

             Cd = Cd0 * exp(-Ce * (wnd_frc_thr_slt_std - wnd_frc_thr_slt_std_min) / wnd_frc_thr_slt_std_min) ! the dust emission coefficient -jfk
             Cd1(p) = Cd 
             flx_mss_vrt_dst_ttl(p) = Cd * mss_frc_cly_vld(c) * forc_rho(g) * ((wnd_frc_slt**2.0_r8 - wnd_frc_thr_slt**2.0_r8) / wnd_frc_thr_slt_std) * (wnd_frc_slt / wnd_frc_thr_slt)**(Ca * (wnd_frc_thr_slt_std - wnd_frc_thr_slt_std_min) / wnd_frc_thr_slt_std_min)  ! the vertical dust flux -jfk             
             ...
             !     flx_mss_fdg_fct * liqfrac !variable no longer needed, -jfk
             flx_mss_vrt_dst_ttl(p) = flx_mss_vrt_dst_ttl(p) * lnd_frc_mbl(p) * C_tune * liqfrac ! account for bare soil fraction, frozen soil fraction, and apply global tuning parameter -jfk
          ...
          !dst_slt_flx_rat_ttl = 100.0_r8 * exp( log(10.0_r8) * (13.4_r8 * mss_frc_cly_vld(c) - 6.0_r8) ) !variable no longer needed, -jfk
          !flx_mss_vrt_dst_ttl(p) = flx_mss_hrz_slt_ttl * dst_slt_flx_rat_ttl !variable no longer needed, -jfk
