...
!added dymeter in here
        ...
        do k = kts,kte
          do i = its,ite
             dtaux3d_ls(i,k)=dtaux2d_ls(i,k)
             dtaux3d_bl(i,k)=dtaux2d_bl(i,k)
             dtaux3d_ss(i,k)=dtaux2d_ss(i,k)
             dtaux3d_fd(i,k)=dtaux2d_fd(i,k)
             dtauy3d_ls(i,k)=dtauy2d_ls(i,k)
             dtauy3d_bl(i,k)=dtauy2d_bl(i,k)
             dtauy3d_ss(i,k)=dtauy2d_ss(i,k)
             dtauy3d_fd(i,k)=dtauy2d_fd(i,k)
          enddo
        enddo
...
!Jinbo Xie
!potential  zblk higher than real drag for some region
!restrain to the real topo height (mountain height or subgrid std var)
zblk=min(zblk,var(i))
!Jinbo Xie
...
!Jinbo Xie
!surface stress is a whole-level integration of the whole column 
!from bot to top
!In finalize there seems to be a transition
!Jinbo Xie
...
!for diagnostic parameters pattern Jinbo Xie
do i= its,ite
   dusfc_ss(i) = coefm(i)
   dvsfc_ss(i) = dxy(i)
   dusfc_fd(i) = roll(i)
   dvsfc_fd(i) = zblk_col(i)!olp(i)
enddo
