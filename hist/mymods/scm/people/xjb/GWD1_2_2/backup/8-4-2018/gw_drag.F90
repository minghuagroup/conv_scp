...
!---------------------------------------------------------------------------------
  ...
  use ppgrid,        only: pcols, pver,   pverp!add another, pverp=pver+1
...
!===========================
	use physconst,      only: rh2o,zvir,pi,rearth,r_universal  !zvir is the ep1 in wrf,rearth is the radius of earth (m),r_universal is the gas constant
        use module_bl_gwdo_gsd, only: gwdo_gsd,gwdo2d
!===========
! Jinbo Xie
!===========
...
!===========================
	!these are all from physconst.F90
	!real(r8),parameter :: rh2o	!gas constant for water vapor (j/kg/K)
	!real(r8),parameter :: zvir	!rv/rd-1, constant for virtual temperature (dimensionless)
	!real(r8),parameter :: pi		!pie, 3.1415926 and more
	!real(r9),parameter :: r_universal !ideal gas constant
	!simply add par
...
!================
  !subroutine gw_intr(state, sgh, pbuf, dt, ptend, landfrac)
    subroutine gw_intr(state, sgh, pbuf, dt, ptend, landfrac,var,hgt_m,oc,oa1,oa2,oa3,oa4,ol1,ol2,ol3,ol4)
...
!================
!input par
        real(r8), intent(in)::  var(pcols) ! Convexity
        real(r8), intent(in):: hgt_m(pcols)
    real(r8), intent(in)::  oc(pcols) ! Convexity
    real(r8), intent(in):: oa1(pcols) ! Asymmetry
    real(r8), intent(in):: oa2(pcols) !
    real(r8), intent(in):: oa3(pcols) !
    real(r8), intent(in):: oa4(pcols) !
    real(r8), intent(in):: ol1(pcols) ! Effective length
    real(r8), intent(in):: ol2(pcols) !
    real(r8), intent(in):: ol3(pcols) !
    real(r8), intent(in):: ol4(pcols) !

    integer :: kpbl2d_in(pcols)
!input/output data
!	real,   dimension( ims:ime, kms:kme, jms:jme )                             , &
!            optional                                                           , &
!    real(r8), intent(inout  )   ::	dtaux3d_ls, &
!								  	dtauy3d_ls, &                                
!									dtaux3d_bl, &
!									dtauy3d_bl, &
!									dtaux3d_ss, &
...
!===========
        ...
        !state%q(0:pcols,0:pver,1) is the q, water vapor
        call gwdo_gsd(u3d=state%u(:,pver:1:-1),v3d=state%v(:,pver:1:-1),t3d=state%t(:,pver:1:-1),&
	qv3d=state%q(:,pver:1:-1,1),p3d=state%pmid(:,pver:1:-1),p3di=state%pint(:,pver:1:-1),pi3d=state%exner(:,pver:1:-1),z=ztop,  &
	rublten=utgw(:,pver:1:-1),rvblten=vtgw(:,pver:1:-1),rthblten=ttgw(:,pver:1:-1),	&
        dusfcg_ls=dusfc_ls,dvsfcg_ls=dvsfc_ls,dusfcg_bl=dusfc_bl,dvsfcg_bl=dvsfc_bl,&
dusfcg_ss=dusfc_ss,dvsfcg_ss=dvsfc_ss,dusfcg_fd=dusfc_fd,dvsfcg_fd=dvsfc_fd, &
	xland=landfrac,br=rino_pub,&
	var2d=sgh,oc12d=oc,oa2d1=oa1,oa2d2=oa2,oa2d3=oa3,oa2d4=oa4,ol2d1=ol1,ol2d2=ol2,ol2d3=ol3,ol2d4=ol4,&
	znu=etamid(pver:1:-1),dz=dz,pblh=pblh,&
	cp=cpair,g=g,rd=r,rv=rh2o,ep1=zvir,pi=pi,&
	dt=dt,dx=rearth*cos(rlat)*(2*pi/288.),dy=rearth*(pi/192-1.),&
...
! Compute the temperature tendency from energy conservation (includes spectrum).
        ...
        !Jinbo Xie
        !add the gw  and fb together
        !tau0x=0 
        !tau0y=0
        tau0x=dusfc_ls!+dusfc_bl
        tau0y=dusfc_bl!dvsfc_gw!+dvsfc_bl
        !=============
