  ...
  ! Find out whether shfrc from convect_shallow will be used in cldfrc
    ...
    call addfld ('stdvar_diag   ', '1   ', pver, 'A', 'subgrid-scale variance dignosed from PBL and shallow convection'          ,phys_decomp)
  ...
  use shr_kind_mod,     only: r8 => shr_kind_r8
  use cloud_fraction,   only: cldfrc, cldfrc_fice, cldfrc_implcit_PDF
...
!  use cldwat2m_macro,   only: mmacro_pcond
  use cldwat2m_macro_pbl,   only: mmacro_pcond_pbl
  ...
  real(r8) :: cld_ori(pcols,pver)                   ! Total cloud fraction
...
! For purposes of convective ql.
 ...
 !      'CC...'    are microphysical tendencies. 
   ...
   call mmacro_pcond_pbl( lchnk, ncol, pbuf, dtime, state_loc%pmid, state_loc%zm, state_loc%pdel,        &
                          shflx,  lhflx, &
...
!#undef cldfrc_new
#define cldfrc_new
#if (defined cldfrc_new)
      call cldfrc_implcit_PDF( lchnk, ncol, pbuf, state_loc%zm, shflx, lhflx,    &    
                   state_loc%pmid, t_inout, qv_inout, ql_inout, qi_inout, &
                   dtime, stdvar, rhc_diag, cld, cmeliq ) 
#endif

   call outfld( 'CLOUD_wang   ', cld_wang ,  pcols, lchnk )
   call outfld( 'RHC_diag   ', rhc_diag ,  pcols, lchnk )
   call outfld( 'stdvar_diag   ', stdvar ,  pcols, lchnk )
