...
!===============================================================================
  ...
  subroutine compute_uwshcu_inv( mix      , mkx        , iend          , ncnst     , dt       ,  & 
                                 ...
                                 cbmf     , qc_inv     , rliq          ,                         &
                                 cnt_inv  , cnb_inv    , lchnk         , dpdry0_inv, wu_inv  )
  ...
  subroutine compute_uwshcu( mix      , mkx       , iend         , ncnst    , dt        , &
                             ...
                             cnt_out  , cnb_out   , lchnk        , dpdry0_in, wu_out )
...
!               wu_out(i,mkx:0:-1)       = wu_s(0:mkx)
     ...
     ! OK
       ...
       do iter_xc = 1, niter_xc
          ...
          qsat_arg = thle*exne     
          call qsat(qsat_arg, pe, es, qs, gam)
          excess0  = qte - qs
...
!  X.Wang
                   aquad =  wue**2;
                   beta = ( 1._r8 + (1._r8+0.608_r8)*gam*cp/xlv*exne*thlue )/( 1._r8 + gam )
                   bquad =  -2._r8*rbuoy*g*cridis*( beta*(thlue - thle) + &  
                           (beta-cp/xlv*exne*thlue)*xlv/cp/exne*(qtue - qte) )/thv0j - 2._r8*wue**2;
                   cquad =  2._r8*rbuoy*g*cridis*(thvj -  thv0j)/thv0j +    wue**2;
!  X.Wang
                   if( ( bquad**2-4._r8*aquad*cquad ) .ge. 0._r8 ) then 
                       call roots(aquad,bquad,cquad,xs1,xs2,status)
                       x_cu = min(1._r8,max(0._r8,min(xs1,xs2)))
                   else 
                       x_cu = 1._r8;
                   endif
                   xc = x_cu;
...
!              wtw = wtw*expfac + (delbog + (1._r8-expfac)*(bogbot + delbog/(-2._r8*drage*dpe)))/(rho0j*drage)
              wtw = wtw*expfac + (1._r8-expfac)*0.5_r8*(bogbot + bogtop)/(rho0j*drage)  ! X.Wang
          else
