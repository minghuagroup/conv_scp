...
! Modifed   Zhang Minghua 2019-09-21 implict integration together with nlitti.F90
!------------------------------------------------------------------------------------------------
...
! ZMH SUBSETEPPING 
!----------------------------------------------------------------------------------------------
   IF(zmh_tend2)then                    ! substepping in X
       NZMH2 = 2                       
! temporarily
       NZMH2 = 1                        
       DTadv2 = DTadv/DBLE(NZMH2)
   ELSE
...
!----------------------------------------------------------------------------------------------
   IF(zmh_nlitti2)then  ! forward and backward combined
       NZMH = 2
   ELSE
...
!zmh
  IF(zmh_nlitti2)then
     ...
     CALL nliter_uvt(UL, VL, TL, DTadv2)  

    !IF(IZMH==1)then !forward
    !  DO J = beglatdyn, endlatdyn
    !     DO K = beglev,endlev 
    !        DO I = 1, NX
