...
!
!            2018.08.15, ZhangMinghua, added geostrophic adjustment and inertial-gravity wave
!                                      dispersion coefficients geoadpu(NY+1),geoadpv(NY)
!------------------------------------------------------------------------------------------------
...
!zmh
   ...
   integer :: Ndt                                                  !wjp 2007.06
   integer :: Ndq                                                  !zhh 2010.08.10
   real(r8):: DTDY     ! timestep of dynamical integration !wjp 2007.06
   real(r8):: DTlin     ! timestep of linear term
   real(r8):: DTadv    ! timestep of advection term
   real(r8):: DTIMEQ   ! timestep of Q
...
!zmh
   real(r8) :: LAT55_TRAN  = 55.0 ! ramp transition latitude
   real(r8) :: LAT55_SCALE = 6.0  ! spatial contraction scale
   real(r8) :: geoadpu(NY+1)      ! geostrophic adaptation coefficients
   real(r8) :: geoadpv(NY)        ! geostrophic adaptation coefficients
   real(r8) :: zmh_nth(NY)        ! 1.0 in northern hemisphere, 0.0 in southern hemisphere 
   real(r8) :: zmh_sth(NY)        ! opposite to above
   logical  :: zmh_nlitti  = .true. !true.  ! combine forth and back integration
   logical  :: zmh_tend1   = .true.  ! embedded substepping in tend_lin
   logical  :: zmh_nlitti2  = .true. !true.  ! combine forth and back integration
   logical  :: zmh_tend2   = .false.  ! embedded substepping in tend_lin
!zmh 
...
!zmh
      use mzfunctions_mod, only: mzfunc1              !zmh, 2018-08-15
...
!----------------------------------Local workspace----------------------------------------------
      integer :: i,j,k
!zmh
      real(8) :: wk1((NY+1)/2), ulat((NY+1)/2)
      real(8) :: wk2(NY/2)    , vlat(NY/2)
      real(8) :: DEGLAT, PI18, xmin
!-----------------------------------------------------------------------------------------------
      do K = 1, NZ
         if ( NL == 19 ) then
            PHALFC(K) = PHALFC19(K)
...
!zmh
      ...
      xmin   = 0.0
      do J = 1,(NY+1)/2
         ulat(J)  = DBLE(J-  1)*DEGLAT  
         ulat(J) = ulat(J)*PI18
      end do

      do J = 1,NY/2
         vlat(J)  = DBLE(J-0.5)*DEGLAT  
         vlat(J) = vlat(J)*PI18
      end do

    I = (NY+1)/2.
    J = NY/2
       call  mzfunc1 ( I, ulat , wk1, xmin , 90.*PI18, (90.-LAT55_TRAN)*PI18, LAT55_SCALE, 0)
       call  mzfunc1 ( J, vlat , wk2, xmin , 90.*PI18, (90.-LAT55_TRAN)*PI18, LAT55_SCALE, 0)

      do J = 1,NY/2+1
         geoadpu(J)      = wk1(J)
      end do
      do J = NY+1,NY/2+2,-1
         geoadpu(J) = geoadpu(NY-J+2)
      end do
         geoadpu(NY/2+1) = 1.0
 
      do J = 1,NY/2
         geoadpv(J)      = wk2(J)
         geoadpv(NY-J+1) = wk2(J)
      end do
 ...
 if(j < 0)then
      if(masterproc)then
         write(*,*)' geoadp coefficients'
         write(*,*)' geoadpu'
         do j=1,NY +1
           write(*,*) J, 90.0 - DBLE(J-  1)*DEGLAT, geoadpu(J)          
         end do
         write(*,*)' geoadpv'
         do j=1,NY 
           write(*,*) J, 90.0 - DBLE(J-  0.5)*DEGLAT, geoadpv(J)          
         end do
       end if
endif
!zmh
