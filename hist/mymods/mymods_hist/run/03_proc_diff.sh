...
set test_casename  = test35b1 #casadv2e
#                    ??????????????

            # F2000MG1MU #F2000MG1 #F2000MG1

set test_filetype = monthly_history 
#set test_filetype = time_series

set PWD1 = $PWD
#set PWD1 = /T1/xxie/output/{$test_casename}/run/$test_casename/
#  modified above by zmh for storm =======================================
set test_path_history =  {$PWD1}/$test_casename/
set test_path_climo =    {$PWD1}/$test_casename/
set test_path_diag  =    {$PWD1}/{$test_casename}/{$test_casename}_diag/
set test_path_HPSS    =  /CCSM/csm/${test_casename}/atm/hist/
mkdir $test_path_diag
cp -L 03_proc_diff.sh $test_path_diag/03_proc_diff.sh
...
set cntl_casename   = test35b #cas5b #test35b #castest2
#                    ??????????????
set cntl_filetype = monthly_history 
#set cntl_filetype = time_series

echo $test_casename ' - ' $cntl_casename

#set PWD2 = $PWD
set PWD2 = /T1/xxie/output/{$cntl_casename}/run/
set PWD2 = /T1/xxie/output/test35b/run/
# for storm
set cntl_path_history =  {$PWD2}/$cntl_casename/
set cntl_path_climo =    {$PWD2}/$cntl_casename/
...
set test_first_yr = 0002           # first year (must be >= 1)
...
set cntl_first_yr = 0002        # first year (must be >= 1)
set cntl_nyrs     = 5        # number of yrs (must be >= 1)
...
set test_name = $test_casename #beta05p2k   test case name 
set cntl_name = $cntl_casename
...
setenv DIAG_HOME /T0/xjb/apps/amwg/amwg_diag_20140804 #/ds/wlin/DIAG
...
set test_res_out = 1.9x2.5
set cntl_res_out = 1.9x2.5
...
setenv OBS_DATA   $DIAG_HOME/obs_data_20140804 #obs_data 
setenv CAM35_DATA $DIAG_HOME/cam35_data 
setenv MAP_DATA   $DIAG_HOME/map_files_20140804 #map_files 
...
echo $test_casename ' - ' $cntl_casename ' ' done!
ls -l *.tar
echo mv $test_path_diag/*.tar ~/minghua/www_gcm
mv $test_path_diag/*.tar ~/minghua/www_gcm/.
echo 'ready in ~/minghua/www_gcm for viewing!'
