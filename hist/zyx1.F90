...
! Purpose:
!
! Interface from ZYX1 convection scheme,modified from 
! ZM scheme
!
...
! Modified: Zhang Minghua, 2018-07-24 see interface zyx1_tend
!---------------------------------------------------------------------------------
...
!------------- zhh added 2013-04-24 -----------------
   ...
   real(r8) :: zyx_cape_lmt  = 200. !unset_r8    
   real(r8) :: zyx_mb        = 0.5 !1.0   !unset_r8    
   real(r8) :: zyx_alfa      = 0.2  ! 0.2 defalut unset_r8    
!------------- zhh added 2013-04-24 -----------------

   real(r8) rl         ! wg latent heat of vaporization.
   real(r8) cpres      ! specific heat at constant pressure in j/kg-degk.
!zmh   real(r8), parameter :: capelmt =400._r8 !70._r8  ! threshold value for cape for deep convection.
   real(r8) :: capelmt =400._r8 !70._r8  ! threshold value for cape for deep convection.
...
!zmh
!zmh =================================
   logical           :: deep_zmh_nolimit   = .true. !.false. !true. !false.
   character(len=20) :: deep_trigger_scheme = 'zmh_triggers' !'cam'
   integer           :: deep_ntriggers = 8
   integer  :: niter = 6
   integer, dimension(8) :: deep_trigger_flags = (/1, 1, 0, 1, 0, 0, 0, 1/)
   !integer, dimension(8) :: deep_trigger_flags = (/1, 0, 0, 0, 0, 0, 0, 0/)
                         !deep_triggers(1) = cape(i) > capelmt)                      
                         !deep_triggers(2) = cape(i) > bfls_dilucape(i)
                         !deep_triggers(3) = vort3>0 and <0          !positive values
                              !!deep_triggers(3) = cin(i)   < bfls_cin(i)          !positive values
                         !deep_triggers(4) = omega(i,kpbl(i)) < 0._r8             !kpbl(i)
                         !deep_triggers(5) = pblh(i) > 200._r8
                         !deep_triggers(6) = rh(i,kpbl(i)-2) > 0.7_r8       !rh ,kpbl(i)
                         !deep_triggers(7) = tke(i)   > cin(i)
                         !deep_triggers(8) = precw(i) > 0.75*precws !(>30.) 
                         !evaluation
   character(len=20) :: deep_closure_scheme = 'prognostic' !'cam'
   real              :: tau1_zmh = 600.  ! for prognostic cape , time to mb
   real              :: tau2_zmh = 600.  ! for memory of 1000 J/kg CAPE
!zmh =================================
...
!          ,zyx_cape_lmt, zyx_mb, zyx_alfa   !zmh
   !-----------------------------------------------------------------------------
...
!   call mpibcast(zyx_cape_lmt,      1, mpir8,  0, mpicom)   !zmh
!   call mpibcast(zyx_mb,            1, mpir8,  0, mpicom)   !zmh
!   call mpibcast(zyx_alfa,          1, mpir8,  0, mpicom)   !zmh
#endif

end subroutine zyx1_conv_readnl


subroutine zyx1_conv_init(limcnv_in, no_deep_pbl_in)
...
subroutine zyx1_conv_tend(lchnk   ,ncol    , nstep              , &
                    ...
                    heat    ,pap     ,paph    ,dpp     ,          &
                    delt    ,                                     &
                    lat     ,landfrac,lhflx   ,shflx   ,          & !input
                    psrf    ,bfls_t  ,bfls_q           ,          &
                    omega   ,tpert   ,qpert            ,          &  
                    vort3            ,tke,                  &
                    massflxbase_p    ,                            &
                    mcon    ,cme     ,cape    ,                   & !output
                    dlf     ,pflx    ,zdu     ,rprd    ,          &
                    mu      ,md      ,du      ,eu      ,ed      , &
...
#else
                    ,md0, mu0, eu0, du0, eps0, dd0, ed0, md1, mu1, &
                         eu1, du1,   dd, ed1, &
                    ent_turb, det_turb, ent_org, det_org,&
                    w_up, cldsr, buoy_up, buoy_dn )
#endif
...
! zmh
   ...
   !real(r8), intent(in) :: tdiff3(pcols)          ! grid slice of temperature at mid-layer.
   !real(r8), intent(in) :: qdiff3(pcols)          ! grid slice of temperature at mid-layer.
   ...
   real(r8)  :: etau1_zmh, etau2_zmh, w11, w12
!zmh -----------
...
!zmh -----------
   logical, dimension(8) :: deep_triggers
                         !deep_triggers(1) = cape(i) > capelmt)                      
                         !deep_triggers(2) = cape(i) > bfls_dilucape(i)
                         !deep_triggers(3) = vort3(i) > 0N, <0S  !positive values
                         !deep_triggers(4) = omega(i,kpbl(i)) < 0._r8             !kpbl(i)
                         !deep_triggers(5) = pblh(i) > 500._r8
                         !deep_triggers(6) = rh(i,kpbl(i)-2) > 0.7_r8       !rh ,kpbl(i)
                         !deep_triggers(7) = tke(i)   > cin(i)
                         !deep_triggers(8) = precw(i) > 30. !
...
!      real(r8) vd(pcols,pver)
   ...
   real(r8) qdifr
   real(r8) sdifr
   real (r8) ent_tot(pcols, pver), det_tot(pcols, pver)
   real (r8) w_init(pcols)
   real (r8) rad_init(pcols)

#if (defined WRITE_OUT)
   real(r8) :: md0(pcols, pver),  mu0(pcols, pver), eu0(pcols, pver), du0(pcols, pver), &
   eps0(pcols, pver), dd0(pcols, pver), &
   ed0(pcols, pver),  md1(pcols, pver), mu1(pcols, pver), &
   eu1(pcols, pver), du1(pcols, pver), dd(pcols, pver),ed1(pcols, pver), &
   ent_turb(pcols, pver), det_turb(pcols, pver), ent_org(pcols, pver), &
   det_org(pcols, pver) , &
   w_up(pcols, pver),     cldsr(pcols, pver), buoy_up(pcols, pver), &
   buoy_dn(pcols, pver)
#endif
...
!XXXXXX
  ...
  do i = 1, ncol
    ...
    do k= pver,msg,-1
     if (abs(z(i,k)-zs(i)-pblh(i)) < (zf(i,k)-zf(i,k+1))*0.5_r8) then
         kpbl(i) = k
         exit 
     endif
    end do
   ...
   ! for subcloud 
    do k= pver,msg,-1
        if(p(i,k)/pf(i,pver+1) .le. 0.95 )  then   
          k950(i) = k         
          exit 
        endif
    enddo    
...
!XXXXXXXX
    do k= maxi(i)+1,pver
     if(p(i,k) - p(i,maxi(i)) .ge. 50.)then
      k1 = k
      exit
     endif
    enddo 
    if(k1==pver)then
      do k = maxi(i),limcnv,-1
        if(p(i,pver) - p(i,k)) .ge. 50.)then
          k1 = k
          exit
         endif
       enddo
     endif 
    maxi1(i) = max(k1,maxi(i)) 
    maxi2(i) = min(k1,maxi(i)) 
!XXXXXXXX

    k   = get_mink(hmn(i,:),pver, msg, maxi(i)-2)
    jd(i)   = k
...
!XXXXXXXX
   ...
   do k = msg + 2,pver
      do i = 1,lengath
         sdifr = 0._r8
         qdifr = 0._r8
         if (s(i,k) > 0._r8 .or. s(i,k-1) > 0._r8) &
            sdifr = abs((s(i,k)-s(i,k-1))/max(s(i,k-1),s(i,k)))
         if (q(i,k) > 0._r8 .or. q(i,k-1) > 0._r8) &
            qdifr = abs((q(i,k)-q(i,k-1))/max(q(i,k-1),q(i,k)))
         if (sdifr > 1.E-6_r8) then
            shat(i,k) = log(s(i,k-1)/s(i,k))*s(i,k-1)*s(i,k)/(s(i,k-1)-s(i,k))
         else
            shat(i,k) = 0.5_r8* (s(i,k)+s(i,k-1))
         end if
         if (qdifr > 1.E-6_r8) then
            qhat(i,k) = log(q(i,k-1)/q(i,k))*q(i,k-1)*q(i,k)/(q(i,k-1)-q(i,k))
         else
            qhat(i,k) = 0.5_r8* (q(i,k)+q(i,k-1))
         end if


         gamma(i,k) = qsat(i,k)*(1._r8 + qsat(i,k)/eps1)*eps1*rl/(rd*t(i,k)**2)*rl/cp


         if (abs(qsat(i,k-1)-qsat(i,k)) > 1.E-6_r8) then
           qsathat(i,k) = log(qsat(i,k-1)/qsat(i,k))*qsat(i,k-1)*qsat(i,k)/ (qsat(i,k-1)-qsat(i,k))
         else
            qsathat(i,k) = qsat(i,k)
         end if
         hsthat(i,k) = cp*shat(i,k) + rl*qsathat(i,k)
...
! ----------------------   
     ...
     ent_tot(:,:) =  -zmconv_dmpdz 
     ent_tot(:,:) =  0.0002_r8  !-zmconv_dmpdz 
     det_tot(:,:) =  0.0_r8

     ent_turb(:,:) = 0.0001_r8 *(1.+0.2*min(rh(:,:),1._r8))
     det_turb(:,:) = 0.0001_r8 *(1.-0.2*min(rh(:,:),1._r8))
...
!XXXXXXXXXXXx
   ...
   call zyx1_cldprp(lchnk       ,ncol   ,limcnv  ,iter    , &
               q       ,t       ,u      ,v       ,p       ,          &
               z       ,zf      ,s      ,hmn     ,hsat    , qsat,    &
               qhat    ,shat    ,rh     ,landfrac,tpert   , qpert,   &
               c0mask  ,dz      ,gamma  ,gamhat   ,hsthat ,qsthat,   & 
               maxi    ,max1    ,maxi2   ,                           &
               lel,    ,lel2    ,jd     ,jd2,                        &  
               rl      ,lengath, rgas   ,grav    ,cpres   , msg,     &
               mu      ,eu      ,du     ,hu      ,qu      , su,      &
               md      ,ed      ,dd     ,hd      ,qd      , sd,      & 
               jlcl    ,ql     ,cmeg    ,cu      ,evp     , rprd,    &
               pflx    ,buoy_mid        , ent_tot,det_tot , eps00,   & 
               cape    ,cin     ) 
...
!to verify
write(*,*)
write(*,*)'lchnk       ,ncol   ,limcnv  ,iter ',lchnk       ,ncol   ,limcnv  ,iter 
write(*,*)'cape    ,cin',cape    ,cin
write(*,*)'maxi    ,max1    ,maxi2',maxi    ,max1    ,maxi2
write(*,*)'lel,    ,lel2    ,jd     ,jd2' lel,    ,lel2    ,jd     ,jd2
write(*,*)'q       ,t       ,u      ,v       ,p'
write(*,*) q       ,t       ,u      ,v       ,p
write(*,*)'qhat    ,shat    ,rh     ,landfrac,tpert   , qpert'
write(*,*) qhat    ,shat    ,rh     ,landfrac,tpert   , qpert
write(*,*)'rl      ,lengath, rgas   ,grav    ,cpres   , msg'
write(*,*) rl      ,lengath, rgas   ,grav    ,cpres   , msg
write(*,*)
write(*,*)'mu      ,eu      ,du     ,hu      ,qu      , su'
write(*,*) mu      ,eu      ,du     ,hu      ,qu      , su
write(*,*)'md      ,ed      ,dd     ,hd      ,qd      , sd'
write(*,*) md      ,ed      ,dd     ,hd      ,qd      , sd
write(*,*)'jlcl    ,ql     ,cmeg    ,cu      ,evp     , rprd'
write(*,*) jlcl    ,ql     ,cmeg    ,cu      ,evp     , rprd
write(*,*)'pflx    ,buoy_mid        , ent_tot,det_tot , eps00'
write(*,*) pflx    ,buoy_mid        , ent_tot,det_tot , eps00
endif

!XXXX make qhat,shat,hsat,qsat,hmn,rh etc  as input
...
!                  qpert   ,-ent_tot ,buoy_mid,         cin  )
 ...
 do i = 1, ncol
  ...
  if(cape(i) > cape_lmt)then       !deep
   conv_type(i) = 1  
  else if(cape(i) > 0._r8)then     !shallow
   conv_type(i) = 2
  endif
...
!XXXXXXXX   conv_type     cape, lel to determine profile whether deep or shallow
     call zyx1_dynamics1 (ncol ,zf  ,dz  ,lon  ,lel  ,dzb  ,dzt  ,ent_org  ,det_org , 1)

     call zyx1_dynamics2 (ncol ,limcnv ,z  ,dz  ,buoy_mid  ,w_init  ,ent_tot  ,maxi ,lel ,k100 ,&
              w_up, jt ,eps00 , dzt)

!XXXXXX      conv_type adjusted in dynamics2
...
!                     bs_wue, bs_xc(i,k), ent_turb(i,k), det_turb(i,k) )
     ...
     do i=1,ncol 
      ent_tot(i,:) = (ent_org(i,:)*eps00(i)+ent_turb(i,:))
      det_tot(i,:) = (det_org(i,:)*eps00(i)+det_turb(i,:))
     enddo
...
!     write(*,*)'cape',cape
!     write(*,*)'cin',cin
!    write(*,*)'2eps00,dzt',eps00,dzt
!     write(*,*)'1ent_tot',ent_tot
!     write(*,*)'1det_tot',det_tot
...
!     do i=1,ncol 
!      ent_tot(i,:) = 0.0_r8 !-(ent_org(i,:)+ ent_turb(i,:))*eps00(i)
!      det_tot(i,:) = 0.0_r8 !-(det_org(i,:)+ det_turb(i,:))*eps00(i)
!     enddo
     !call zyx1_dynamics2 (ncol,limcnv,z,dz,buoy_mid,w_init,ent_tot,maxi,lel,k100, w_up, jt,eps00,dzt)
! for temporary output check
!    write(*,*)'w_up2',w_up
!    write(*,*)'eps00',eps00
!    write(*,*)'jt2',jt
...
!     call zyx1_dynamics1 (ncol,zf,dz,jd,maxi,dzb,dzt,ent_org,det_org,-1)
!     write(*,*)'ent_org2',ent_org
!     write(*,*)'det_org2',det_org

!     do i=1,ncol 
!      ent_tot(i,:) = -(ent_org(i,:)+ ent_turb(i,:))*eps00(i)
!      det_tot(i,:) = -(det_org(i,:)+ det_turb(i,:))*eps00(i)
!     enddo
...
!
!
!       use buoy_mid and ht, lel, jd, to cal ent_tot
...
write(*,*) '-------'
write(*,*) 'nstep=',nstep
write(*,*) 'cape,pblt,lcl,q,t'
write(*,*) cape(i),pblt(i),lcl(i)
write(*,*) q(i,:)
write(*,*) 't'
write(*,*) t(i,:)
write(*,*) 'p'
...
!    -------------------------------------------------------
       deep_triggers(2) = (cape(i)  > bfls_cape(i))
     end if

       !if(landfrac(i)==0)then 
        deep_triggers(3) =  (vort3(i)*sin(lat(i)) .ge. 0.0) &
                            .or. (omega(i,k750(i)) < 0.) 
        if(deep_triggers(3))then
       ...
       deep_triggers(4) = (omega(i,k750(i)) < 0._r8 )             !kpbl(i)
       if(deep_triggers(4))then
        w11 = zmh_ramp(-omega(i,k750(i)),0._r8,1.e-5_r8)
        ramp(i) = ramp(i)*w11
       endif

       deep_triggers(5) = (pblh(i)          > 500._r8 )
       deep_triggers(6) = (rh(i,kpbl(i)-2)  > 0.75_r8 )       !rh ,kpbl(i)
       deep_triggers(7) = (tke(i)   > cin(i)  )
       deep_triggers(8) = (precw(i) > 0.75*precws(i))       ! 0.70 default
       if(deep_triggers(8))then
!        ramp(i) = ramp(i)*zmh_ramp(precw(i)/precws(i),0.75_r8,1.0_r8)
        ramp(i) = ramp(i)*zmh_ramp(precw(i)/precws(i),0.80_r8,0.10_r8)
       endif

         flag(i) = deep_triggers(1)
      do iflag = 2, deep_ntriggers 
         if(deep_trigger_flags(iflag) == 1)then
            flag(i) = flag(i) .and. deep_triggers(iflag)
         endif
      enddo    
...
if(i<0)then
write(*,*)'flag', flag
write(*,*)'trigg', deep_trigger_flags
...
!rite(*,*)'bfls_q  ',bfls_q
   ...
   case default
    write(*,*)'--------- NO TRIGGER SCHEME OF DEEP CONV IS SELECTED, STOPPED in ZYX1CONV'
    stop
...
!zmh !!!!!!!!!!!!!!!!
!      do i = 1,lengath
!            dsubcld(i) = dsubcld(i) + dp(i,maxg(i)-1) + dp(i,maxg(i)-2)
!      end do
!zmh !!!!!!!!!!!!!!!!
...
!
   ...
   call zyx1_cldprp(lchnk   , &
               ...
               maxg    ,j0      ,jdg      ,rl      ,lengath , &  !zmh jdg
               rgas    ,grav    ,cpres   ,msg     , &
               pflxg   ,evpg    ,cug     ,rprdg   ,limcnv  ,landfracg, &
               tpert   ,qpert   ,buoy_mid, ent_totg, det_totg, eps00g,  & 
               capeg   , cin &  !zmh
#if (! defined WRITE_OUT )
                    ) !
#else
                    ...
                    ent_turb, det_turb, ent_org, det_org, &
                    w_up, cldsr, buoy_up,  buoy_dn )
    md1 = buoy_mid/gravit*tg  ! temporary diag
...
!zmh
  ...
  select case (deep_closure_scheme)            
   case( 'cam')
   ...
   case('prognostic')
    do i=1,lengath
       etau1_zmh =  exp(- delt/tau1_zmh)                              !time toward mb
       !etau2_zmh =  exp(- delt/max( (cape(i) / 1000.)*tau2_zmh,tau) ) !memory time long for large CAPE
       etau2_zmh =  exp(- delt/tau2_zmh)

      if(cape(i) < capelmt) then
       mb(i) = 0.
      else if(flag(i))then
       mb(i) = massflxbase_p(i,1)*(etau1_zmh+etau2_zmh)*0.5  + (1.-etau1_zmh)*mb(i) 
      else
       mb(i) = massflxbase_p(i,1)*(etau1_zmh+etau2_zmh)*0.5  
      endif
...
!  write(*,*)'mb=',mb
  do i=1,lengath
      mb(i) = mb(i)*zyx_mb !/*5 ! /5.0
      if(mb(i) .le. 1.0e-4_r8)then
          mb(i) = 0._r8
      endif
  enddo
! -------------------------------------------------------
...
! -------------------------
do i = 1, lengath
    massflxbase_p(i,1) = mb(i)
enddo

if(deep_zmh_nolimit) then
   do i=1,lengath
     nt_stepping(i) = int(mumax(i)*delt/0.5_r8)+1
     nt_stepping(i) = min(nt_stepping(i),4)
     mb(i) = min(mb(i),4.*0.5_r8/(delt*mumax(i)))   !zmh
   end do
!   write(*,*)'nt_stepping',nt_stepping(i)
else
   do i=1,lengath
      if (mumax(i) > 0._r8) then
         mb(i) = min(mb(i),0.5_r8/(delt*mumax(i)))
!          write(*,*)'mb, mumax',mb(i),0.5_r8/(delt*mumax(i))
      else
         mb(i) = 0._r8
      endif
    nt_stepping(i) = 1
...
subroutine zyx1_cldprp(lchnk   , &
                  ...
                  ,buoy_mid, ent_tot, det_tot, eps00 ,capeout, cinout&
#if (! defined WRITE_OUT )
                    ) !
#else
...
!zmh newly added
   ...
   real(r8) capeout(pcols)
   real(r8) cinout(pcols)
...
!   det_turb = dd0/1000._r8
  ...
  dzb(:) = 0.3_r8
  dzt(:) = 0.2_r8
  call zyx1_dynamics1(il2g ,zf ,dz ,jd ,jb ,dzb, dzt,ent_turb  ,det_turb , -1)

if(k<0) then
write(*,*)'---jdjb eps00-',jd,jb,eps00
write(*,*)'ent_turb2',ent_turb
write(*,*)'det_turb2',det_turb
endif

  do i = 1,il2g
      ent_turb(i,:) = ent_turb(i,:)*eps00(i)
      det_turb(i,:) = det_turb(i,:)*eps00(i)
  enddo
...
1001  CONTINUE
 ...
 cinout(i)   = 0.0_r8
  do k = jt(i),jb(i)-1
   ...
   dum1       = (su(i,k)+su(i,k+1))*0.5
   dum2       = (qu(i,k)+qu(i,k+1))*0.5
   tv_up(i,k) = (dum1 - grav/cp*z(i,k))* (1._r8+1.608_r8*dum2)/ (1._r8+dum2)   

   rho(i,k)   = p(i,k)/rd/tv(i,k)*100.

   dum0 = (1._r8+dz(i,k)*c0mask(i))*ql(i,k)  
   buoy_up(i,k) = (tv_up(i,k) - tv(i,k))/tv(i,k)*grav - dum0

   if(buoy_up(i,k) > 0. )then 
    capeout(i) = capeout(i) + buoy_up(i,k)*rho(i,k)*(zf(i,k)-zf(i,k+1))
   else
    if(k > jd(i))then
     cinout(i)  = cinout(i) + buoy_up(i,k)*rho(i,k)*(zf(i,k)-zf(i,k+1))
    endif
   endif
...
!
   ...
   do i = il1g,il2g
      ...
      dltaa = -1._r8* max((cape(i)-capelmt),0._r8)
...
!!zmh         else if (k > mx(i)) then
         else if (k > k950(i)) then
            dsdt(i,k) = dsdt(i,k-1)
            dqdt(i,k) = dqdt(i,k-1)
         end if
      end do

    !dsdt0(i,:) = dsdt0(i,:) + dsdt(i,:)
    !dsdq0(i,:) = dsdq0(i,:) + dsdq(i,:)
...
!zmh
   ...
   do k = pver,msg +1, -1
      do i = 1,ncol
       if(p(i,k)>(pf(i,pver+1)-100.))then   ! in mb unit!!
       !if(p(i,k)>(pf(i,pver+1)-50.))then   ! in mb unit!!
         k100(i) = k
       else
         exit
       endif
...
!zmh         if (k >= nint(pblt(i)) .and. k <= lon(i) .and. rhd > -1.e-4_r8) then
         if (k >= min(nint(pblt(i)),k100(i)) .and. k <= lon(i) .and. rhd > -1.e-4_r8) then
            hmax(i) = hmn(i)
!zmh            mx(i) = k
         end if
      end do
   ...
   do k = pver,msg + 1,-1
      do i = 1,ncol
         hmn(i) = cp*t(i,k) + grav*z(i,k) + rl*q(i,k)
!zmh         if (k >= nint(pblt(i)) .and. k <= lon(i) .and. hmn(i) > hmax(i)) then
         if (k >= min(nint(pblt(i)),k100(i)) .and. k <= lon(i) .and. hmn(i) > hmax(i)) then
            hmax(i) = hmn(i)
!zmh            mx(i) = k
         end if
      end do
...
!! mx(:) = k100(:)
!! do i=1,ncol
!!   hmax(i) = cp*t(i,k100(i)) + grav*z(i,k100(i)) + rl*q(i,k100(i))
!! enddo
!!!!!!!!!!!!!!!!!!
...
if(i<0)then
  write(*,*)'... in dilute'
  write(*,*)'capeten',capeten
  write(*,*)'lel mx hmax tpert',    lel(i),mx(i),hmax(i),tpert(i)
  write(*,*)'q',q
  write(*,*)'t',t
...
subroutine zyx1_plume_dilute(lchnk, ncol, msg, klaunch, p, t, q, tpert, qpert,ent_tot,&
         tp, tpv, qstp, pl, tl, lcl)
...
!!zhh dmpdz=-1.e-3_r8        ! Entrainment rate. (-ve for /m)
!!dmpdz=zmconv_dmpdz  !zhh
...
!zmh 1
         ...
         qtp0(i) = q(i,k) + qpert(i)   ! Parcel launch total water (assuming subsaturated) - OK????.
         !sp0(i)  = entropy(t(i,k),p(i,k),qtp0(i))  ! Parcel launch entropy.
         sp0(i)  = entropy(t(i,k)+tpert(i),p(i,k),qtp0(i))  ! Parcel launch entropy.

!zmh
#IF(DEINFED NEW_PARCEL)
         !qtp(i) = qtp0(i)
         !sp(i)  = sp0(i) 
         qtp(i) = 0.5*(qtp0(i) + q(i,k-1))
         sp(i)  = 0.5*(sp0(i)  + entropy(t(i,k-1),p(i,k-1),q(i,k-1)) )
...
!!
!====================================================
subroutine zyx1_dynamics1 (ncol,zf,dz,jb,jt,dzb,dzt,ent,det,idir)  !XXXXx add eps00
!====================================================
!XXXXx add eps00 to do entrainment above jbl, below 1/z
...
!    
     ...
     eps00(i) = sqrt(w2*(z(i,jt2(i))-z(i,jb(i))))/dp_radius/wmax *0.10_r8 !/dzb
