...
!
        ...
        do j=1,plat
           ...
           if (( microp_scheme .eq. 'MG' ) .and. (microp_driver_implements_cnst(cnst_name(m_cnst)))) then
...
!zmh 
!       2018-08-15, smoothing terrain test of spatial derivvatives
!
!  do k=1,2  ! k times     
!    do i=1,plon
!            ip1 = min(i+1,plon)
...
!            jm1 = max(j-1,1)
!
!            ps_tmp(I,J) = (phis_tmp(I,J) + phis_tmp(IM1,J) + phis_tmp(IP1,J)        &
!                      + phis_tmp(I,JM1) + phis_tmp(IM1,JM1) + phis_tmp(IP1,JM1)   &
!                      + phis_tmp(I,JP1) + phis_tmp(IM1,JP1)                       &
!                      + phis_tmp(IP1,JP1) )/9.0
!        enddo
!     enddo
!
!    do i=1,plon
!        do j=1,plat
!         phis_tmp(I,J) = ps_tmp(I,J)
!        enddo
!    enddo
!
!  enddo ! ktimes
