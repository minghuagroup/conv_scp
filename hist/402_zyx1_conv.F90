...
!
  ...
  public zm_convr                    ! convective momentum transport
...
!------------- zhh added 2013-04-24 -----------------
   ...
   real(r8) :: zyx_cape_lmt  = 400. !unset_r8    
   real(r8) :: zyx_mb        = 1.0   !unset_r8    
   real(r8) :: zyx_alfa      = 0.2  !unset_r8    
!------------- zhh added 2013-04-24 -----------------
...
!zmh -----------
   logical, dimension(8) :: deep_triggers
    ...
    logical  :: flag(pcols)
    integer,  dimension(pcols)       :: nt_stepping, k750, k950, kpbl
    integer  :: iflag
...
!      real(r8) vd(pcols,pver)
   ...
   eu1(pcols, pver), du1(pcols, pver), dd1(pcols, pver),ed1(pcols, pver), &
...
!
!   write(*,*)'here 1 pevr, msg',lchnk,pver,msg,ncol,pcols,nstep
   do k = pver - 1,msg + 1,-1
      do i = 1,ncol
!          write(*,*)'z(i,k)-zs(i)-pblh(i)',i,k,z(i,k),zs(i),pblh(i),zf(i,k),zf(i,k+1)
!          write(*,*)'z(i,k)-zs(i)-pblh(i)',zf(i,k),zf(i,k+1)
!
!          write(*,*) '------'
!          write(*,*)'t- ', t(i,k)       ,qh(i,k,1)      ,prec(i),landfrac(i), dpp(i,k)    
!          write(*,*)'jc-', jctop(i)   ,jcbot(i), pblh(i)    ,zm(i,k)      ,geos(i)    ,zi(i,k) 
!          write(*,*) '------'

         !if (abs(z(i,k)-zs(i)-pblh(i)) < (zf(i,k)-zf(i,k+1))*0.5_r8) pblt(i) = dble(k)
         if (abs(z(i,k)-zs(i)-pblh(i)) < (zf(i,k)-zf(i,k+1))*0.5_r8) kpbl(i) = k
      end do
   end do
   pblt(:) = dble(kpbl(:))
...
! zmh relative humidity
   ...
   do i = 1, ncol
    ...
    enddo    
    !k750(i) = min(k750(i),pver-6)   !to replace PBL input
...
! ----------------------   
if(i<0)then
    write(*,*)'zyx1_conv before call buoyan'
    write(*,*)'bfls_t',bfls_t
    write(*,*)'bfls_q',bfls_q
    write(*,*)'pbl',pblh
    write(*,*)'tpert',tpert
   ...
   if( cam_physpkg_is('cam3')) then
     ...
     !if((deep_triggers(2)==1) .or. (deep_triggers(3)==1))then
     if(deep_triggers(2)==1)then 
!    -------------------------------------------------------
      call zyx1_buoyan_dilute(lchnk   ,ncol    , &
                  bfls_q       ,bfls_t       ,p       ,z       ,pf       , &
                  tp      ,qstp    ,tl      ,rl      ,bfls_cape     , &
                  pblt    ,lcl     ,lel     ,lon     ,maxi     , &
                  rgas    ,grav    ,cpres   ,msg     , tpert,    &
                  qpert   , bfls_buoy_mid   , bfls_cin   )
...
subroutine zyx1_buoyan_dilute(lchnk   ,ncol    , &
                  q       ,t       ,p       ,z       ,pf      , &
                  tp      ,qstp    ,tl      ,rl      ,cape    , &
                  pblt    ,lcl     ,lel     ,lon     ,mx      , &
                  rd      ,grav    ,cp      ,msg     , &
                  tpert   , qpert, buoy, cin)         !zmh
!----------------------------------------------------------------------- 
! 
! Purpose: 
...
!  zmh added qpert
   call zyx1_parcel_dilute(lchnk, ncol, msg, mx, p, t, q, tpert, qpert,tp, tpv, qstp, pl, tl, lcl)
...
subroutine zyx1_parcel_dilute (lchnk, ncol, msg, klaunch, p, t, q, tpert, qpert,tp, tpv, qstp, pl, tl, lcl)
...
return
end subroutine zyx1_parcel_dilute
...
!
!   write(*,*)'here 1 pevr, msg',lchnk, pver,msg,ncol,pcols
