...
!------------- zhh added 2013-04-24 -----------------
   ...
   real(r8) :: zyx_cape_lmt  = 100. !unset_r8    
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
   ...
   do i = 1,il2g
       k = jb(i) 
            hu(i,k) = hmn(i,jb(i)) + cp*(tiedke_add + tpert(i)) + rl*qpert(i)
            su(i,k) = s(i,jb(i)) + tiedke_add +tpert(i)
      end do
...
!updraft
   ...
   do k = pver,msg + 2,-1
      do i = 1,il2g
         if (( .not. done(i) .and. k > jt(i) .and. k < jb(i)) ) then
...
!
    ...
    do k = jd(i),jb(i)-1
        ...
        if(dum1<1.e-6 .or. dum2<1.e-6_r8)then
          md(i,k+1) = md(i,k)*exp((ent_turb(i,k) - det_turb(i,k))*dum0  )
          ed(i,k) = - md(i,k)*(exp(ent_turb(i,k)*dum0) -1._r8)
        ...
        if(md(i,k) < -0.01_r8)then 
          ...
          hd(i,k+1) = hd(i,k) + (ent_turb(i,k)*hmn(i,k) - det_turb(i,k)*hd(i,k) )*dum0
        endif

    enddo !k

          dd(i,jb(i)) = -md(i,jb(i))/dz(i,jb(i))
          ed(i,jb(i)) = 0.0
   ...
   do k = msg + 1,pver
      do i = 1,il2g
         if ((k >= jt(i) .and. k <= jb(i)) .and.   md(i,jb(i))< -0.01_r8 ) then
