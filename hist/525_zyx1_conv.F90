...
!zmh =================================
   ...
   real(r8) :: zyx_cape_lmt  = 200. !unset_r8  added random number *(1+rand) 200-400   
   real(r8) :: zyx_mb        = 1.0  !.5 !1.0   !unset_r8    
   real(r8) :: zyx_alfa      = 0.2  ! 0.2 defalut unset_r8    
   real(r8) :: zyx_rain_z0   = 0.0  !meter
   real(r8) :: zyx_rain_zp   = 100 !100.0 !meter over ocean
   real(r8) :: zyx_rain_cr   = 0.40  !
!zmh =================================
...
! zmh
   ...
   real(r8)  :: massflxbase_pg(pcols)       !
   real(r8)  :: etau1_zmh, etau2_zmh, w11, w12
   real(r8) :: capelmt(pcols),capelmtg(pcols)
!zmh -----------
...
!zmh -----------
   logical, dimension(pcols,10) :: deep_triggers
                         ...
                         !deep_triggers(2) = omega(i,kpbl(i)) < 0._r8             !kpbl(i)
                         !deep_triggers(3) = cape(i) > bfls_dilucape(i)
                         !deep_triggers(4) = precw(i) > X
                         !deep_triggers(5) = pblh(i) > 500._r8
                         !deep_triggers(6) = rh(i,kpbl(i)-2) > 0.7_r8       !rh ,kpbl(i)
                         !deep_triggers(7) = tke(i)   > cin(i)*X
                         !deep_triggers(8) = vort3(i) > 0N, <0S  !positive values
...
!XXXXXXXX
   ...
   ! for downdraft mass detrainment
    do k= pver,msg,-1
        if(pf(i,pver+1)-p(i,k) .ge. 25. )  then   
          jd2(i) = k         
          exit 
        endif
    enddo    
...
! initial guess
     ...
     ent_tot(:,:) =  0.00015_r8  !-zmconv_dmpdz 
     det_tot(:,:) =  0.0_r8
  ...
  DO ITER = 1, NITER
     ...
     call w_dynamics (ncol ,limcnv ,z ,dz ,buoy_mid ,w_init ,ent_tot,  maxi ,maxi1, &
         lel ,lel2, lelb, w_up, eps00 )

     if(ITER .ne. NITER)then
       call ent_dynamics (ncol ,zf  ,dz  ,maxi, maxi1, maxi2,lel, lel, dzb  ,dzt  , &
              ent_turb  ,det_turb,eps00, ent_tot ,det_tot , 1)
       do i = 1,ncol
       ...
       call ent_dynamics (ncol ,zf  ,dz  ,maxi, maxi1, maxi2,lel, lel2, dzb  ,dzt  , &
         ent_turb  ,det_turb,eps00, ent_tot ,det_tot , 1)
     endif  
...
write(*,*)'lel,lel2,lelb',lel,lel2,lelb
write(*,*)'eps00',eps00
write(*,*)'mypblh ',pblh
...
case('zmh_triggers') !additional limit
    ...
    capelmt(:) = zyx_cape_lmt*(1._r8+rand_num(:)*0.5_r8) 
    do i=1,ncol
     deep_triggers(i,1) = (cape(i)  > capelmt(i) )
    enddo  

    call subcol_kissvec(seed4, seed3, seed2, seed1, rand_num)
    do i=1,ncol
      deep_triggers(i,2) = (omega(i,k750(i)) < 0._r8 )             !kpbl(i)
!      deep_triggers(i,2) = (omega(i,k750(i)) < - rand_num(i)*5.e-3_r8 )             !kpbl(i)
      w11 = zmh_ramp(-omega(i,k750(i)),0._r8,5.e-3_r8)
      ramp(i) = ramp(i)*w11
    enddo
...
!    -------------------------------------------------------
     ...
     call subcol_kissvec(seed1, seed3, seed4, seed2, rand_num)
     do i=1,ncol
      if(massflxbase_p(i,1) < 0.001_r8)then

!      deep_triggers(i,3) = (cape(i)  > bfls_cape(i)*(1._r8+rand_num(i)) /2.)

      deep_triggers(i,4) = (precw(i) > 0.75*precws(i))       ! 0.70 default
      ramp(i) = ramp(i)*zmh_ramp(precw(i)/precws(i),0.75_r8,0.10_r8)

!      deep_triggers(i,5) = (pblh(i)          > 500._r8 )
!      deep_triggers(i,6) = (rh(i,kpbl(i)-2)  > 0.75_r8 )       !rh ,kpbl(i)
!      deep_triggers(i,7) = (tke(i)   > cin(i)  )
!      deep_triggers(i,8) =  (vort3(i)*sin(lat(i)) .ge. 0.0) &
!                            .or. (omega(i,k750(i)) < 0.) 
...
!      ramp(i) = ramp(i)*0.5*(w11 + w12)
      ...
      endif  ! for if massflxbase_p(i,1)> 0.01 
...
if(k<0)then
     do i=1,ncol
write(*,*)'jt,jd,jb    ',jt,jd,maxi
write(*,*)'jb1,jb2,jd2 ',maxi1,maxi2,jd2
write(*,*)'flag', flag
...
!zmh
     ...
     massflxbase_pg(i) = massflxbase_p(ideep(i),1)
      capelmtg(i) = capelmt(ideep(i))
   end do
!
...
!
   ...
   call zyx1_cldprp(lchnk   , &
               ...
               qu      ,su      ,zfg     ,qs      ,hmn     , &
               hsat    ,shat    ,qlg     ,                   &
               cmeg    ,maxg    ,maxi1g  ,maxi2g  ,lelg    ,lel2g ,   &
               jdg     ,jd1g    ,jd2g    ,jlcl    , &
               rl      ,lengath ,         &
               rgas    ,grav    ,cpres   ,msg     , &
   ...
   case('prognostic')
    do i=1,lengath
      ...
      if(cape(i) < capelmt(i)) then
       ...
       mb(i) = massflxbase_pg(i)*(etau1_zmh+etau2_zmh)*0.5  + (1.-etau1_zmh)*mb(i) 
      else
       mb(i) = massflxbase_pg(i)*(etau1_zmh+etau2_zmh)*0.5  
      endif
...
!DIR$ CONCURRENT
   do i = 1,lengath
      ...
      massflxbase_p(ideep(i),1) = massflxbase_pg(i)   !zmh
      jctop(ideep(i)) = jt(i)
!++bee
...
subroutine zyx1_cldprp(lchnk   , &
                  ...
                  qu      ,su      ,zf      ,qst     ,hmn     , &
                  hsat    ,shat    ,ql      ,                   &
                  cmeg    ,jb      ,jb1, jb2,jt    ,jt2, jd, jd1, jd2, jlcl,&
                  rl      ,il2g    , &
                  rd      ,grav    ,cp      ,msg     , &
                  pflx    ,evp     ,cu      ,rprd    ,limcnv  ,landfrac,tpert,qpert &
                  ,buoy_mid, ent_tot, det_tot, eps00 ,capeout, cinout,flag) !&
...
!
   ...
   integer, intent(in) :: jt2(pcols)              ! updraft base level
...
!
  ...
  DO i = 1,il2g
      ...
      rain_z0(i) = zyx_rain_z0*(1.0_r8 + 0.5*landfrac(i))  !0.5 tunable 
      rain_zp(i) = zyx_rain_zp*(1.0_r8 + 0.5*landfrac(i))
!   end do
!
...
!      do i = 1,il2g
         dz(i,k) = zf(i,k) - zf(i,k+1)
         ramp_t1(i,k) = 1.0_r8 - zmh_ramp(t(i,k) - 238.15_r8, 0._r8,  30._r8) 
         ramp_t2(i,k) = 1.0_r8 - zmh_ramp(t(i,k) - 253.15_r8, 0._r8,  15._r8) 
!      end do
   end do
...
!
 enddo
  ...
  call ent_dynamics(il2g ,zf ,dz ,jd ,jd1, jd, jd3, jd2, dzb, dzt,&
      ent_turb*0.0,det_turb*0.0,eps00, ent_org,det_org, -1)
...
!   do i = 1,il2g
    ...
    do k = jb(i),jt(i),-1
         ...
         dum0 = min(1.0-zyx_rain_cr*ramp_t2(i,k) , sqrt(dum0))
...
!downdraft
    ...
    do k = jd1(i),jd2(i)-1
        ...
        dum0 = dz(i,k)
        dum1 = 1._r8/dum0 - (ent_org(i,k) - det_org(i,k))/2._r8  
        dum2 = 1._r8/dum0 + (ent_org(i,k) - det_org(i,k))/2._r8  

        if(dum1<1.e-6 .or. dum2<1.e-6_r8)then
          md(i,k+1) = md(i,k)*exp((ent_org(i,k) - det_org(i,k))*dum0  )
          ed(i,k) = - md(i,k)*(exp(ent_org(i,k)*dum0) -1._r8)
          dd(i,k) = - md(i,k) + ed(i,k) + md(i,k+1)
        else
          md(i,k+1) = md(i,k)*dum2/dum1
          dumb = (md(i,k)+md(i,k+1))/2.0
          ed(i,k) = - ent_org(i,k) * dumb
          dd(i,k) = - det_org(i,k) * dumb
        endif
    ...
    do k = jd1(i),jd3(i)-1    
        ...
        if(md(i,k+1) < -0.01_r8)then 
          ...
          hd(i,k+1) = hd(i,k) + ent_org(i,k)*(hmn(i,k) - hd(i,k)) *dum0
        endif
...
if(k<0)then
write(*,*)'jb,jb1,jb2',jb,jb1,jb2
write(*,*)'jt,jt2',jt,jt2
write(*,*)'jd,jd1,jd2,jd3',jd,jd1,jd2,jd3
write(*,*)'md',md
write(*,*)'mu',mu
write(*,*)'k,mu(i,jb1(i)),md(i,jd2(i))',k,mu(i,jb1(i)),md(i,jd2(i))
endif
...
!re-do sd and qd with new evp
      ...
      if(md(i,k+1)< -0.01)then
       ...
       sd(i,k+1) = sd(i,k) + (ent_org(i,k)*(s(i,k)-sd(i,k)) &
           + rl/cp*evp(i,k)/dum1 )*dz(i,k)
       qd(i,k+1) = qd(i,k) + (ent_org(i,k)*(q(i,k)-qd(i,k)) &
           - evp(i,k)/dum1       )*dz(i,k)
      endif
...
!
   ...
   do k = lel(i), mx(i) - 1
      ...
      !dltaa = -1._r8* max(cape(i)-capelmt(i)/4.0_r8,0._r8)
...
!====================================================
!  magnitude of j for updraft    jb2 > jb1 > ---- > jt2 > jt 
!  magnitude of j for downfdraft jb2 < jb1 < ---- < jt2 < jt 

 integer,  intent(in) :: ncol
 integer,  intent(in) :: idir  !1: upward plume, -1 downward plume
 real(r8), intent(in) :: zf(pcols,pver+1)  
 real(r8), intent(in) :: dz(pcols,pver)  
 integer,  intent(in) :: jt(pcols)   !top index of end layer
 integer,  intent(in) :: jt2(pcols)  !bottom index of end layer
 integer,  intent(in) :: jb(pcols)   !air source
 integer,  intent(in) :: jb1(pcols)  !top index of source layer
 integer,  intent(in) :: jb2(pcols)  !bottom index of source layer
 ...
 real zt,zb,zt2,zb2 ,dzj,ents,dets,ratio
  ...
  do i = 1, ncol
   ...
   do k = jt2(i),jb1(i),idir
    if( (zz2(k) .gt. 0._r8) .and. (zz2(k) .lt. 1._r8))then
      ent(i,k) = 1._r8 - zz2(k)
    endif
   enddo 
...
! the two end layers
   ...
   do k=jb1(i)+idir,jb2(i),idir
    dzj      = zf(i,k)-dz(i,k)*0.5 - zf(i,jb2(i)+1) 
    ent(i,k) = abs(1._r8/dzj)
    det(i,k) = 0.0_r8
   enddo
   do k=jt(i),jt2(i)-idir,idir
    dzj      = zf(i,jt(i)) - zf(i,k) + dz(i,k)*0.5
    ent(i,k) = 0.0_r8
...
!    
     ...
     eps00(i) = sqrt(w2*(z(i,jt2(i))-z(i,jb(i))))/dp_radius/wmax *0.15_r8 !/dzb
