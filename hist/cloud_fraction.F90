...
!yhy
     ...
     character(len=16),public :: macrop_scheme = 'WXC' ! 'IAP','WXC','cam'
...
subroutine cldfrc_readnl(nlfile)
   ...
   namelist /cldfrc_nl/ cldfrc_freeze_dry, cldfrc_ice, cldfrc_rhminl, cldfrc_rhminh, &
                        ...
                        cldfrc_premit    , macrop_scheme  ! xwang
   !-----------------------------------------------------------------------------
...
DO i=1,ncol
  DO k = 1,pver !p
!  DO k = 16,pverp ! xwang
!     if (tke_pbl(i,k).gt.1.e-5_r8) then
!     if (tke_pbl(i,k).gt.0._r8) then
!zmh
     if(zm(i,k)-zi(i,pverp) .le. pblht(i))then
        pbl_top(i) = k
        exit
     end if
  END DO
...
!zmh
        IF(k.ge.pbl_top(i))then  !zmh condition 
...
!!zmh vertical subgrid scale

            dum1 = 1.0_r8
            !dum1 = max(cos(lat(i)),0.5_r8 )

            par_stdv_tmp = par_stdv_tmp*dum1

! ---------

            dum0 =  min((temp(i,k-1) - temp(i,k+1))/(zm(i,k-1)-zm(i,k+1))/9.8e-3_r8, 0._r8)
            dum0 =  max(1.0_r8 + dum0*2.0_r8, 0._r8) *max(min(1._r8, shflx(i)/20.),0._r8)
            ...
            !sat_deficit = aaa(i,k) * (qt(i,k) - qsl(i,k) ) / par_stdv_tmp
...
!       cloud(i,k) = max(cloud(i,k),rhc_diag(i,k))
