...
import sys
sys.path.append("/Users/minghuazhang/unix/tws/twsapi_macunix.976.01/IBJts/source/pythonclient/ibapi")
...
#MZ <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            ...
            self.reqHistoricalData(4102 + self.myID2, self.myContract2, self.myHistDate2[0],
                                   self.myHistDate2[1], self.myHistDate2[2], self.myHistDate2[3], 1, 1, False,
...
## loop through contract data and myHistDate here ########################################
        ...
        self.reqContractDetails(210+self.myID2, self.myContract2)   # for stock
        self.reqHistoricalData(4102+self.myID2, self.myContract2, self.myHistDate2[0],
                            self.myHistDate2[1], self.myHistDate2[2], self.myHistDate2[3], 1, 1, False, [])   # returned in HistoricalData
...
#    import pandas as pd
    ...
    #myExpDates = ['20201023', '20201030', '20201106', '20201113', '20201120', '20201127', '20201218']
#    df = []
    myqueryTime = (datetime.datetime.today() - datetime.timedelta(days=10)).strftime("%Y%m%d %H:%M:%S")
    myqueryTime = "20201021 24:00:00 GMT"  # end data
    myHistDate = [myqueryTime, "1 D", "20 mins", "MIDPOINT"]  # TRADES BID-ASK MIDPOINT
    ...
    myContracts = [None]*100
    myfilenames = [None] * 100

    i = 0
    for i in range(len(myRights)):
        print('------- iRight: ', myRights[i])
        ...
        myContracts[i] = ContractSamples.myOPT_Contract()
        myContracts[i].right = myRights[i]
        print('------- in main my Contracts[i]: ', i, ' ',myContracts[i])

        ###############################################################################

    for i in range(len(myRights)):
        myfolder = 'data/' + str(myContracts[i].symbol) + '/' + str(myContracts[i].secType) + '/'
        if (os.path.isdir('data/' + str(myContracts[i].symbol)) == False):
            os.mkdir('data/' + str(myContracts[i].symbol))
        if (os.path.isdir(myfolder) == False):
            os.mkdir(myfolder)
        myfilenames[i] = str(myContracts[i].symbol) + '_' + str(myContracts[i].secType) \
                          + '_' + myenddate + '_' + myduration + '_' + myfreq + '_' + mytype
        if ((myContracts[i].secType == 'OPT') or (myContracts[i].secType == 'FOP')):
            myfilenames[i] = myfilenames[i] + '_' + myContracts[i].right + '_' + str(myContracts[i].strike) \
                              + '_' + myContracts[i].lastTradeDateOrContractMonth
        myfilenames[i] = myfolder + myfilenames[i] + '.csv'
        print('----- in main my filenames[i]', i, ' ', myfilenames[i])
...
#    for i in range(1):
        #app = TestApp()
        app = TestApp(myID2 = i, myContract2 = myContracts[i], myfilename2 = myfilenames[i], myHistDate2 = myHistDate)
        # ! [connect]
        app.connect("127.0.0.1", args.port, clientId=0)
#        # ! [connect]
        print('--- my at end ',myContracts[i], '\n',myfilenames[i])
