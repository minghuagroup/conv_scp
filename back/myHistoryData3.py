...
###############################################################################
        myjunk = self.myHistDate

        myContract1 = self.myContract

        print('---- in histEnd',self.myContract)
        myfolder = 'data/' + str(myContract1.symbol) + '/' + str(myContract1.secType) + '/'
        if (os.path.isdir('data/' + str(myContract1.symbol)) == False):
            os.mkdir('data/' + str(myContract1.symbol))
        ...
        mytype = myjunk[3]
        self.myfilename = str(myContract1.symbol) + '_' + str(myContract1.secType) \
                          + '_' + myenddate + '_' + myduration + '_' + myfreq + '_' + mytype

        if ((myContract1.secType == 'OPT') or (myContract1.secType == 'FOP')):
            self.myfilename = self.myfilename + '_' + myContract1.right + '_' + str(myContract1.strike) \
                              + '_' + myContract1.lastTradeDateOrContractMonth

        self.myfilename = myfolder + self.myfilename + '.csv'
        ...
        super().historicalDataEnd(reqId, start, end)
        print("HistoricalDataEnd. ReqId:", reqId, "from", start, "to", end)
        print("--- is it here? in  HistoricalDataEnd  ", self.myfilename)
        # MZ return
###############################################################################
...
## loop through contract data and myHistDate here ########################################
        ...
        print('------- in myread_historicalDataOperations_req ->', self.myHistDate)
        i = 0
        mysleep = [1,20]
        for i in range(len(myRights)):
            ...
            # self.myContract = ContractSamples.myFUT_Contract()
            # self.myContract = ContractSamples.myFUTOPT_Contract()
            self.myContracts[i] = ContractSamples.myOPT_Contract()
            self.myContracts[i].right = myRights[i]
            #time.sleep(mysleep[i])
            print('------- self.myContract: ', self.myContracts[i])
            self.data = [self.myfilename, '\n']  # initialized
            self.myContract = self.myContracts[i]
            #time.sleep(mysleep[i])
            self.reqHeadTimeStamp(4101+i, self.myContracts[i], "TRADES", 0, 1)
            self.reqContractDetails(210+i, self.myContracts[i])   # for stock
            self.reqHistoricalData(4102+i, self.myContracts[i], self.myHistDate[0],
                            self.myHistDate[1], self.myHistDate[2], self.myHistDate[3], 1, 1, False, [])   # returned in HistoricalData
        ...
        print('---- at the end',self.myHistDate)
