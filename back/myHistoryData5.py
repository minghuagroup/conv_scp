...
# This program works for contract details and historical data print, unsure how bar is assigned value line 1029
# This will store data in tables and save in csv file
# modify:
# Everything done below line 2100, can be looped; This program does not work!!!
...
#MZ-1 this is where the contract is changed !!!
...
#class TestApp(TestWrapper, TestClient, myID2 = myID2, myContract2 = Contract2, myfilename2 = myfilename2, myHistDate2 = myHistDate2):
class TestApp(TestWrapper, TestClient):
    def __init__(self, myID2 = None, myContract2 = None, myfilename2 = None, myHistDate2 = None):
        TestWrapper.__init__(self)
        TestClient.__init__(self, wrapper=self)
        self.myID2 = myID2
        self.myContract2 = myContract2
        self.myfilename2 = myfilename2
        self.myHistDate2 = myHistDate2
        # ! [socket_init]
        self.nKeybInt = 0
        self.started = False
...
#MZ
#################################################################################################
# first change specification in ContractSample.py, then select one of the following

        self.mysave     = True
        self.myfilename = ''        # set in myHistoricalData_req, saved in HistoryDataEnd
        self.data = []              # initialized in myHistoricalData_req, assigned in HistoricalData
        self.myContract = Contract  # assigned in myHistoricalData_reg
        self.myContracts = []
        for i in range(100):
            self.myContracts.append(Contract)
...
#MZ-a2
            ...
            #self.tickDataOperations_req()                  # actual executed ticks, calls reqMktData
            #self.marketDepthOperations_req()
            #self.realTimeBarsOperations_req()              # give real time bid, ask, mid data
            #self.historicalDataOperations_req()             # historial data
#MZ
            ##self.optionsOperations_req()                  # default will exercise options (commented out)
                                                            # contains many tests by myself, LYFT etc.
            #self.marketScannersOperations_req()
            #self.fundamentalsOperations_req()
            #self.bulletinsOperations_req()
#MZ-2
            #self.contractOperations()                      # for details (describe contract, not price)
...
# MZ-2
            #self.historicalTicksOperations()                #did not do anything, no info in tick
            #self.tickByTickOperations_req()
            #self.whatIfOrderOperations()

#MZ <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            #self.myread_historicalDataOperations_req()


            #print('------- in myreqHis self.myContract: ', self.myContract2)
            self.data = [self.myfilename2, '\n']  # initialized
            # time.sleep(mysleep[i])
            self.reqHeadTimeStamp(4101 + self.myID2, self.myContract2, "TRADES", 0, 1)
            self.reqContractDetails(210 + self.myID2, self.myContract2)  # for stock
            self.reqHistoricalData(4102 + self.myID2, self.myContract2, self.myHistDate2[0],
                                   self.myHistDate2[1], self.myHistDate2[2], self.myHistDate2[3], 0, 1, False,
                                   [])  # returned in HistoricalData

            #print('---- at the end', self.myHistDate2)
            #self.disconnect()
            #return
...
# MZ >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        self.myread_historicalDataOperations_cancel()

        #df = pandas.DataFrame(self.data, columns=['DateTime', 'Close'])  #initialized in init

#        df = pandas.DataFrame(self.data)

#        df.to_csv(self.myfilename)
        
#        if (os.stat(self.myfilename).st_size) < 100:
#            os.remove(self.myfilename)
#            print('------------ no datafile is saved since ths  is empty', self.myfilename)
#        else:
#            print('--------- file saved:', self.myfilename)
...
##############################################MZ
    ...
    def tickDataOperations_req(self):
        ...
        # ! [reqmktdata]
        self.reqMktData(1000, ContractSamples.USOptionContract(), "", False, False, [])
##############################################

    @printWhenExecuting
    def tickDataOperations_req2(self):
        self.reqMarketDataType(MarketDataTypeEnum.DELAYED_FROZEN)
        # Requesting real time market data
        ...
        #MZ self.reqMktData(1013, ContractSamples.OptionWithLocalSymbol(), "", False, False, [])
        #MZ self.reqMktData(1014, ContractSamples.FuturesOnOptions(), "", False, False, []);
...
#MZ-MZ
        ...
        print(queryTime)
        #MZ self.reqHistoricalData(4102, ContractSamples.OptionForQuery(), queryTime,
        #MZ                   "1 M", "1 day", "MIDPOINT", 1, 1, False, [])

#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

        print(' ---- historicalDataOperations_req : reqContractDetails contract---------------')

#        self.reqHistoricalData(4102, ContractSamples.OptionForQuery(), queryTime,
#                           "1 D", "10 mins", "MIDPOINT", 1, 1, False, [])
#        print(ContractSamples.OptionForQuery)

        myContract = ContractSamples.USOptionContract
        print(ContractSamples.USOptionContract)
        #self.reqContractDetails(210, ContractSamples.USOptionContract())   # for stock
        #self.reqHistoricalData(4102, ContractSamples.USOptionContract(), queryTime,
        self.reqHistoricalData(4102, myContract(), queryTime,
                                                      "1 D", "10 mins", "MIDPOINT", 1, 1, False, [])
        #self.reqMktData(10001, ContractSamples.USOptionContract(), "", False, False, [])


        #self.reqContractDetails(210, ContractSamples.USOptionContract())  # USOptionContract for option

#        print (' ---- historicalDataOperations_req: ---------------')
#        print(ContractSamples.USOptionContract())
#        self.reqMarketDataType(4)
#        self.reqMktData(10001, ContractSamples.USOptionContract(), "", False, False, [])

        #print('---------YY1--------')
        #print(self.reqHistoricalData)
        #print('---------YY2--------')
...
#MZ-MZ
        pass
        #print('???')
        #self.reqHistoricalTicks(18001, ContractSamples.OptionForQuery(),
        #                        "20170712 21:39:33", "", 10, "MIDPOINT", 1, True, [])
        #print('??? ---2')
        # "20200102 00:00:00", "1 M", "8 H", "TRADES", 1, True, [])
...
#MZ-MZ This is where it is printed
        #print('----1')
...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        #print(' ---- test--->, self.myContract', self.myContract)
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
###############################################################################
        ...
        if(self.mysave):
            ...
            df.to_csv(self.myfilename2)

            #self.mykill_proc()

        cmd = "ps > junk.ps"
        os.system(cmd)
        junkfile = open("junk.ps", "r")  # as junkfile:
        texts = junkfile.readlines()
        time.sleep(2)
        for line in texts:
            line = line.rstrip().lstrip()
            if ("myHistoryData5" in line):
                a = line.split(" ")
                print('UID = ', a[0])
                print(' -- killed --> ', line)
                os.system('kill ' + a[0])
        ...
        junkfile.close()
        #cmd2 = 'python myHistoryData5.py'
        #time.sleep(1)
        #os.system(cmd2)

    #            if (os.stat(self.myfilename2).st_size) < 100:
#                os.remove(self.myfilename2)
#                print('------------ no datafile is saved since ths  is empty', self.myfilename)
#            else:
#                print('--------- file saved:', self.myfilename2)

        super().historicalDataEnd(reqId, start, end)
        print("HistoricalDataEnd. ReqId:", reqId, "from", start, "to", end)
        print("--- My file is just saved here in  HistoricalDataEnd  ", self.myfilename2)
#MZ
        #return
        #self.disconnect()
        #self.stop()
...
#MZ >>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    def optionsOperations_req(self):
        # ! [reqsecdefoptparams]
        print("here is option block ---- ")
        #self.reqSecDefOptParams(0, "IBM", "", "STK", 8314)  #all exchanges, check IBKR web for underlying ID https://www.interactivebrokers.com/en/index.php?f=463
        ...
        #self.reqSecDefOptParams(0, "LYFT", "", "STK", 359130923)
        #self.reqSecDefOptParams(0, "JETS", "", "STK", 192643858)
        #self.reqSecDefOptParams(0, "AMD", "", "STK", 4391)
        #self.reqSecDefOptParams(0, "TSLA", "", "STK", 76792991)
        #self.reqSecDefOptParams(0, "ES", "", "FUT", 383974339)  #12/18/2020  does not work
        #self.reqSecDefOptParams(0, "NQ", "", "FUT", 383974419)

        self.reqContractDetails(210, ContractSamples.OptionForQuery())   # for stock
        #self.reqContractDetails(210, ContractSamples.USOptionContract())  # USOptionContract for option
...
#MZ-MZ
        ...
        #MZ self.calculateImpliedVolatility(5001, ContractSamples.OptionAtBOX(), 5, 85, [])
        # ! [calculateimpliedvolatility]
        ##self.calculateImpliedVolatility(5001, ContractSamples.USOptionContract(), 5, 85, [])

        # Calculating option's price
        # ! [calculateoptionprice]
        #MZ self.calculateOptionPrice(5002, ContractSamples.OptionAtBOX(), 0.22, 85, [])
        # ! [calculateoptionprice]
        ##self.calculateOptionPrice(5002, ContractSamples.USOptionContract(), 0.22, 85, [])

        # Exercising options
        # ! [exercise_options]
        #MZ self.exerciseOptions(5003, ContractSamples.OptionWithTradingClass(), 1,
        #MZ                     1, self.account, 1)
        # ! [exercise_options]
...
#MZ-3  change contract spec in ContractSamples.py

        #self.reqContractDetails(210, ContractSamples.OptionForQuery())
        #self.reqContractDetails(210, ContractSamples.OptionAtBOX())
        #self.reqContractDetails(210, ContractSamples.USOptionContract())  ## ??????

        #print('----- in contractOperations')
        #print(ContractSamples.OptionForQuery())

        #self.reqContractDetails(211, ContractSamples.EurGbpFx())
        #self.reqContractDetails(212, ContractSamples.OptionAtBOX())
...
#MZ- This is where it is printed
...
##################################################### MZ ADDED ###########################################
    @printWhenExecuting
    def myread_historicalDataOperations_req(self):

        # Requesting historical data
        # ! [reqHeadTimeStamp]

       # to override
        #if(myContract1.secType == "OPT"):
        #    myContract1.strike = 20 #120
        #    myContract1.right = "C"
        #    myContract1.multiplier = "100"
        #    myContract1.lastTradeDateOrContractMonth = "20210621"
        ...
        #self.reqHeadTimeStamp(4101, myContract1, "TRADES", 0, 1)
        #queryTime = (datetime.datetime.today() - datetime.timedelta(days=10)).strftime("%Y%m%d %H:%M:%S")

        #self.reqContractDetails(210, myContract1)   # for stock
        #self.reqHistoricalData(4102, myContract1, queryTime,
        #                    "1 D", "10 mins", "MIDPOINT", 1, 1, False, [])   # returned in HistoricalData
#        self.reqMktData(10001, myContract1, "", False, False, [])


        # self.myContract = ContractSamples.mySTK_Contract()
        # self.myContract = ContractSamples.myOPT_Contract()
        # self.myContract = ContractSamples.myFUT_Contract()
        # self.myContract = ContractSamples.myFUTOPT_Contract()

        #myContract1 = self.myContract

        #myqueryTime = (datetime.datetime.today() - datetime.timedelta(days=10)).strftime("%Y%m%d %H:%M:%S")

        #myqueryTime = "20201021 24:00:00 GMT"  # end data

        #self.myHistDate = [myqueryTime, "1 D", "20 mins", "MIDPOINT"]  # TRADES BID-ASK MIDPOINT
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

## loop through contract data and myHistDate here ########################################
        #self.myContract = ContractSamples.myOPT_Contract()
        ...
        self.reqContractDetails(210+self.myID2, self.myContract2)   # for stock
        self.reqHistoricalData(4102+self.myID2, self.myContract2, self.myHistDate2[0],
                            self.myHistDate2[1], self.myHistDate2[2], self.myHistDate2[3], 0, 1, False, [])   # returned in HistoricalData

        print('---- at the end within cancelling ',self.myHistDate2)
        #self.stop()
...
#########################################################################################################
    ...
    @printWhenExecuting
    def myread_historicalDataOperations_cancel(self):
        # ! [cancelHeadTimestamp]
        self.cancelHeadTimeStamp(4101 + self.myID2)
        # ! [cancelHeadTimestamp]
        # Canceling historical data requests
        # ! [cancelhistoricaldata]
        self.cancelHistoricalData(4102 + self.myID2)
        self.cancelHistoricalData(4103 + self.myID2)
        self.cancelHistoricalData(4104 + self.myID2)
        # ! [cancelhistoricaldata]
        ## return  ##
...
#########################################################################################################
    def  mykill_proc():
        ...
        cmd2 = 'python myHistoryData5.py > junk2.out'
        time.sleep(1)
        os.system(cmd2)
...
#    df = []
    ...
    for i in range(len(myStrikes)):
        myStrikes[i] = 3300.0 + i * 10  # 3300 to 3500
        #myStrikes[i] = 3400.0 + i * 10  # 3400 to 3500
    ...
    myRights = ['C', 'P']
    for i in range(31):
        myStrikes[i] = 330.0 + i * 1  # 330 to 360
    myExpDates = ['1028', '1030',
    ...
    myfilenames = [None]*1000

    #mySymbol = 'ES'
###################################################################
    i = j = k = ii = 0
    for i in range(len(myRights)):
        for j in range(len(myStrikes)):
            for k in range(len(myExpDates)):
                #myContracts[ii] = ContractSamples.mySTK_Contract()
                myContracts[ii] = ContractSamples.myOPT_Contract()
                #myContracts[ii] = ContractSamples.myFUT_Contract()
                #myContracts[ii] = ContractSamples.myFUTOPT_Contract()
                myContracts[ii].symbol = mySymbol
                if ((myContracts[0].secType == 'OPT') or (myContracts[0].secType == 'FOP')):
                    myContracts[ii].right   = myRights[i]
                    myContracts[ii].strike  = myStrikes[j]
                    myContracts[ii].lastTradeDateOrContractMonth = myExpDates[k]
                    #print('------- in main my Contracts[ii]: ', ii, ' ',myContracts[ii])
                ii = ii + 1


        ###############################################################################
    NF = ii
    if(( myContracts[0].secType == 'STK') or (myContracts[0].secType == 'FUT')):
        NF = 1

    print('------- in main program -> total contract == ', NF)
    for ii in range(NF):
        ...
        if ((myContracts[ii].secType == 'OPT') or (myContracts[ii].secType == 'FOP')):
            myfilenames[ii] = myfilenames[ii] + '_' + myContracts[ii].right + '_' + str(myContracts[ii].strike) \
                              + '_' + myContracts[ii].lastTradeDateOrContractMonth
        myfilenames[ii] = myfolder + myfilenames[ii] + '.csv'
        #print('----- in main my filenames[i]', ii, ' ', myfilenames[ii])

            #myData = [myfilenames[i], '\n']  # initialized

    for ii in range(NF):
        if (os.path.isfile(myfilenames[ii]) == False):
            data = [myfilenames[ii]]
            df = pandas.DataFrame(data)
            df.to_csv(myfilenames[ii])
            #print(' --- ??? true i = ', i)
            app = TestApp(myID2 = i, myContract2 = myContracts[ii], myfilename2 = myfilenames[ii], myHistDate2 = myHistDate)
            app.reqContractDetails(210, myContracts[ii])  # for stock
            # ! [connect]
            app.connect("127.0.0.1", args.port, clientId=0)
    #        # ! [connect]
            print('--- my at end ii = ',ii, myContracts[ii], '\n',myfilenames[ii])
    ...
    if(ii == NF-1):
        for ii in range(NF):
            print(' ')
            print('  looped through ii = ', ii, ' Contract:', myContracts[ii])
            print('  looped through ii = ', ii, ' File:', myfilenames[ii])
            #app.mykill_proc(myRelaunch=False)

    myCleanFile = True #False #True
    if(myCleanFile):
        for ii in range(NF):
            if (os.stat(myfilenames[ii]).st_size) < 200:
                os.remove(myfilenames[ii])
