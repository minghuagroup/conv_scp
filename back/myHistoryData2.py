...
## loop through contract data and save datafile here ########################################
        ...
        print('--- in myread_historicalDataOperations_req:',self.myContract)
        ...
        self.myHistDate = [myqueryTime,"150 D", "20 mins", "MIDPOINT"]  #TRADES BID-ASK MIDPOINT
        #+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        myfolder = 'data/' + str(self.myContract.symbol) + '/' +str(self.myContract.secType) +'/'

        if(os.path.isdir('data/' + str(self.myContract.symbol)) == False):
            os.mkdir('data/' + str(self.myContract.symbol))
        ...
        mytype = myjunk[3]
        self.myfilename = str(self.myContract.symbol) + '_' + str(self.myContract.secType) \
                              + '_' + myenddate + '_' + myduration + '_' + myfreq + '_' + mytype

        if ((self.myContract.secType == 'OPT') or (self.myContract.secType == 'FOP')):
                self.myfilename = self.myfilename + '_' + self.myContract.right + '_' + str(self.myContract.strike) \
                                  + '_' + self.myContract.lastTradeDateOrContractMonth

        self.myfilename = myfolder + self.myfilename + '.csv'


#################################################################################################
        print(' ---- in myread_historicalDataOperations_req : reqContractDetails contract---------------')
        self.data = [self.myfilename, '\n']  # initialized

        myContract1 = self.myContract
        print('------mycontract1:', myContract1)
        time.sleep(2)
        self.reqHeadTimeStamp(4101, myContract1, "TRADES", 0, 1)
        self.reqContractDetails(210, myContract1)   # for stock
        self.reqHistoricalData(4102, myContract1, self.myHistDate[0],
