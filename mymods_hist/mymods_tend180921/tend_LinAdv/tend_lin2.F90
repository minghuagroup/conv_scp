...
! Modified from tend_lin.F90: Zhang Minghua, 2018-08-15 
!    substepping in X, combined forth/back integration, Coriolis ! force 
!    added geostrophic adaption to reduce inertial gravity waves
!    arrays start with ZMH_, with logical control geostrophic_adj
!------------------------------------------------------------------------------------------------
   use shr_kind_mod, only: r8 => shr_kind_r8
   use IAP_grid,  only: NX, NY, NM, NL, NZ, JB, JE, IB, IE, period
   use Dyn_const 
   use stdatm,    only: DPALIB, NA, CB, CBB, PSB, deltac, H0B, TB, GHI0     !zhh 2007.4.23 !zmh added GHI0

   use IAP_prog,  only: PT, TT, UT, VT, WST, Psa, U, WS, WPV, PLY, GHI,     &
                        Pstar1, Pstar2, GZsm, deltap, P, V, GHS       !zmh 2018.8.20 added GHS
   use tendency,  only: DPsa, DT, DU, DV, SU, SV, ST, NADDSS
!zmh
   !use physconst, only: b0, CAPA
   use physconst, only: b0, CAPA, rair
   use mathconst, only: ZERO, HALF, ONE, TWO, FOURTH
   use times, only: yr, mon, day, ncsec
   use mzfunctions_mod, only: fout3d, fout2d
!zmh
!   use flexib,    only:  b1, b21, b22, c10, kappa, kappa2, IBCFFT, Kstd
   use flexib,    only:  b1, b21, b22, c10, kappa, kappa2, IBCFFT, Kstd, &
            geoadpu, geoadpv, zmh_nth, zmh_sth, zmh_nlitti,zmh_tend1,DTlin, Ndt                     !zmh added
   use time_manager,    only: get_nstep   !zmh
...
!----------------------------------Local workspace-----------------------------------------------
   ...
   real(r8), allocatable :: DIV2(:,:,:)            ! 
   real(r8), allocatable :: DIVX2(:,:,:)            ! 
   real(r8), allocatable :: DIVY2(:,:,:)            ! 
   real(r8), allocatable :: OO12(:,:,:)            ! 
   real(r8), allocatable :: OO2X2(:,:,:)            ! 
   real(r8), allocatable :: OO2Y2(:,:,:)            ! 
   real(r8), allocatable :: D12(:,:,:)            ! 
   real(r8), allocatable :: DPS12(:,:)            ! 
   real(r8), allocatable :: DsaY2(:,:)            ! 
   !real(r8), allocatable :: DTW2(:,:)            ! 
   !real(r8), allocatable :: DTW3(:,:)            ! 
!------------------------------------------------------------------------------------------------
   logical               :: sub_first                  ! first step for y integration  
   logical               :: zmh_Coriolis    = .true.  !.False. for default, .True. for geos adjustments
   logical               :: geostrophic_adj = .false.  !.False. for default, .True. for geos adjustments
   logical               :: zmh_uv          = .false. !false.   !.False. for default, .True. for geos adjustments

   character(len=10)     :: zmh_uv_scheme   = 'geos1' !geos2'  ! 'none' for default
                                                       ! 'geos1' using c*DT c*dpsa
                                                       ! 'geos2' using phis=0
                                                       ! 'geos3' using geostrophic 
   real(r8)              :: fac_ageo        =  0.0     ! 0. for geostropic, 1. for ageostrophic balance
   real(r8)              :: VT2, UT2, DTlin2           ! local variable for Coriolis force
!------------------------------------------------------------------------------------------------

   real(r8), allocatable :: ZMH_TT0(:,:,:)            ! 
   real(r8), allocatable :: ZMH_TT1(:,:,:)            ! 
   real(r8), allocatable :: ZMH_DU(:,:,:)             ! zonal component of pressure-gradient force
   real(r8), allocatable :: ZMH_DV(:,:,:)             ! meridional component of pressure-gradient force
   real(r8), allocatable :: ZMH_DT(:,:,:)            ! 
...
!
   ...
   real(r8) :: FS0, FS1, FS2, FS3, FS0V !zmh          ! Coriolis parameter 
...
!zmh
   integer  :: KK, I, J, K,ierr,J1,MJ2 ,ITER, NX2,NY2,NZ2,NNX, NNY,NNZ!zmh                 ! loop index
   integer  :: JM1,JP1,KP1,KM1, IZMH, NZMH, IZMH2, NZMH2, nstep !zmh                 ! loop index
...
! allacate temporary arrays
   ...
   allocate ( DIV2(IB:IE,beglev:endlev,beglatdyn:endlatdyn) )
   allocate ( DIVX2(IB:IE,beglev:endlev,beglatdyn:endlatdyn) )
   allocate ( OO12(IB:IE,beglev:endlev,beglatdyn:endlatdyn) )
   allocate ( OO2X2(IB:IE,beglev:endlev,beglatdyn:endlatdyn) )
   allocate ( DIVY2(IB:IE,beglev:endlev,beglatdyn:endlatdyn) )
   allocate ( OO2Y2(IB:IE,beglev:endlev,beglatdyn:endlatdyn) )
   allocate ( D12(IB:IE,NL,beglatdyn:endlatdyn) )
   allocate ( DsaY2(IB:IE,beglatdyn:endlatdyn) )
   allocate ( DPS12(IB:IE,beglatdyn:endlatdyn) )
   !allocate ( DTW2(NX,beglev:endlev) )
   !allocate ( DTW3(NX,beglev:endlev) )
! zmh 
!
! allocate arrays
   allocate ( WW(NX,NY) )
...
!zmh 
   ...
   allocate ( PY(NX,beglev:endlev,beglatdynex:endlatdynex) )
!!   allocate ( PX(NX,NL,NY) )
!zmh
   !allocate ( PX(NX,beglev:endlev,beglatdyn:endlatdyn) )
   allocate ( PX(NX,beglev:endlev,beglatdynex:endlatdynex) )
...
!zmh 
   allocate ( ZMH_TT0(IB:IE,beglev:endlev,beglatdyn:endlatdyn) )
   allocate ( ZMH_TT1(NX,NL,beglatdynex:endlatdynex) )

   allocate ( ZMH_DU(NX,beglev:endlev,beglatdynex:endlatdynex) )
   allocate ( ZMH_DV(NX,beglev:endlev,beglatdynex:endlatdynex) )
   allocate ( ZMH_DT(NX,beglev:endlev,beglatdynex:endlatdynex) )
...
!zmh for diagnostic purpose .and. for temporary storage
...
!
   DO J = loc_Jb,loc_JE 
      ...
      ENDDO
      DO I = IB,IE
         PXW(I,J)   = OUXH(J) * (PT(I,J)   + PT(I-1,J)) 
         PXW2(I,J)  = OUXH(J) * (PT(I+1,J) + PT(I-2,J))  
         DpDrX(I,J) = OUXH(J) * (pDrho(I+1,J) - pDrho(I-1,J))  
      ENDDO
      ...
      DO K = beglev ,endlev
         DO I = IB, IE
            DIV(I,K,J) = alpha(J)*(PXW(I+1,J) *UT(I+1,K,J) - PXW(I,J)*UT(I  ,K,J))   &
                       + betaw(J)*(PXW2(I+2,J)*UT(I+2,K,J) - PXW2(I-1,J)*UT(I-1,K,J))/3.0  ! zmh
...
! -------------------------------- COMPUTE DIVERGENCES & THEIR SUMS --------------------------
      DO K = beglev ,endlev
         DO I = IB, IE
            DIV2(I,K,J) = (PYP(I,J)*VT(I,K,J)-PYM(I,J)*VT(I,K,J-1))                          !!
         ENDDO
      ENDDO
    ENDDO
...
! -------------------------------- COMPUTE DIVERGENCES & THEIR SUMS --------------------------
      DO K = beglev ,endlev
         DO I = IB, IE
            DIV(I,K,J) = DIV(I,K,J) + DIV2(I,K,J)  !zmh
         ENDDO
      ENDDO
   end do 
...
! zmh 3 end
     ...
     DO J = loc_JB,loc_JE   !zmh
         DO I = IB, IE
            Dsa(i,j)   = (Dsa(i,j) + DsaY2(i,j))   	   !zmh
            !!!!!!!!!Dsa(i,j)   = 0.0
			  
!-------------------------------- COMPUTE DP'sa/DT & D(SIGMA)/DT -------------------------------

            !DPsa(i,j)  = P00*(DPS1) + kappa*Dsa(i,j)  !zmh


            DPsa(i,j)  = P00*(DPS12(I,J)) + kappa*Dsa(i,j)        !zmh      
...
!---------------------------------------------------------
            !IF ( abs(DPS1).GT.0.001 ) THEN   !zmh
            IF ( abs(DPS12(I,J)).GT.0.001 ) THEN

               PRINT*, 'At sub. tend_lin, IN THE INNER AREA: DP=',DPS12(I,J), 'i=',I, 'j=', J
               PRINT*, 'DIV(i,26,j)=', D12(I,26,J), 'DIVX=', DIVX, 'DIVY=', DIVY
            ENDIF
!---------------------------------------------------------
            WK5 = ONE / PT(I,J)
            DPS1 = DPsa(i,j)
            DO K = 1 ,NM      ! NM = NL - 1
               !DWK(K)  = WK5*((DPS1-kappa*Dsa(i,j))/P00 + D1(K))*DSIG(K)               
               DWK(K)  = WK5*((DPS1-kappa*Dsa(i,j))/P00 + D12(I,K,J))*DSIG(K)      !zmh         
            ENDDO
            WSK1  = ZERO
            DO K = 2 ,NL
               WSK1     = WSK1 - DWK(K-1)
...
!------------------------ Compute P-surface vertical velocity (WPV) ---------------------------
         DO K = beglev ,endlev
            DO I = IB,IE
               DIVX2(I,K,J) = OUXQ(J) * (U(I,K,J) + U(I+1,K,J)) * (P(I+1,J) - P(I-1,J))     
            ENDDO  !zmh
         ENDDO       !
...
!zmh 4
 IF(sub_first)then
         DO K = beglev ,endlev
            DO I = IB,IE
               DIVY2(I,K,J) = RDLATQ  * (V(I,K,J) + V(I,K,J-1)) * (P(I,J+1) - P(I,J-1))  
            ENDDO  !zmh
         ENDDO       !
 ENDIF
!zmh 4 end
         DO K = beglev ,endlev
            DO I = IB,IE
               WPV(I,K,J) = HALF * (WS(I,K+1,J) + WS(I,K,J)) * P(I,J)               &
                          + SIGL(K) * (DPsa(i,j) + DIVX2(I,K,J) + DIVY2(I,K,J) )  !zmh 
                          !+ SIGL(K) * (DPsa(i,j) + DIVX + DIVY)            
            ENDDO
...
!------------------ COMPUTE DTT/DT FOR J [JB,JE]  &  I [IB,IE]  &  K [1,NL] ---------------------
         DO K = beglev ,endlev
            DO I = IB,IE
               ...
               OO2X = OUXH(J)/Pstar1(I,J)*(alpha(J)*(DXP1*UT(I+1,K,J)+DXP0*UT(I,K,J))  &   !zmh
         ...
         DO K = beglev ,endlev
            DO I = IB,IE
               TOO0 = (1. - deltap(I,K,J)) * ( b0 * (1. + deltac(I,K,J))               & 
                    !+ Kstd*CAPA*TT(I,K,J)/PT(I,J) ) * (b1*OO1 + b22*OO2X + b21*OO2Y)        !zmh
                    + Kstd*CAPA*TT(I,K,J)/PT(I,J) ) * (b1*OO12(I,K,J) + b22*OO2X2(I,K,J) + b21*OO2Y2(I,K,J)) !zmh 
...
!!!!!!!!!!!!!!!!!!!
!!   DO I = 1 ,NX
!!         DPsa(I,1)  = 0.5*DPsa(I,1) *0.0
!!         DPsa(I,NY) = 0.5*DPsa(I,NY)*0.0
!!   ENDDO 
!!   DO I = 1 ,NX
!!     DO K = beglev ,endlev+1
!!         WS(I,K,1)  = 0.5*WS(I,K,1)*0.0
!!         WS(I,K,NY) = 0.5*WS(I,K,NY)*0.0
!!         WST(I,K,1)  = 0.5*WST(I,K,1)*0.0
!!         WST(I,K,NY) = 0.5*WST(I,K,NY)*0.0
!!     ENDDO
!!   ENDDO
!!   DO I = 1 ,NX
!!     DO K = beglev ,endlev
!!         WPV(I,K,1)  = 0.5*WPV(I,K,1)*0.0
!!         WPV(I,K,NY) = 0.5*WPV(I,K,NY)*0.0
!!     ENDDO
!!   ENDDO
!!!!!!!!!!!!!!!!!!!
...
!zmh 6.1
!!if(sub_first) then ! .and. (IZMH==1))then
...
!ZMH test!! 2018.9.3  !!!!!!!!!!!!!!!!!!!!!!!!
!if(zmh_uv) then !.and. (zmh_uv_scheme .eq. 'geos1') )then
!
!        P(I,J) = min(P(I,J),1060.)      !!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!
...
!ZMH test5 !!
!!         GHI1(I,NZ,J) = GHI1(I,NZ,J) * max(1. - DBLE(NSTEP)/720., 0.) 
...
!!endif
!!else  !zmh 7 not activated
if(izmh2<0)then

   call mp_send3d( commyz, dest, src, NX,NZ, NY,                     &
                   1, NX,beglev,endlev+1, beglatdyn, endlatdyn,       &
                   1, NX, beglev,endlev+1,beglatdyn, beglatdyn,  GHI )
   ...
   call mp_recv3d( commyz, src, NX,NZ,  NY,                             &
                   1, NX,beglev,endlev+1, beglatdyn, endlatdyn,       &
                   1, NX,beglev,endlev+1, endlatdyn, endlatdyn, GHI )

 endif
!zmh 7 end not activated
...
!
         DO K = beglev ,endlev
            DO I = IB,IE
               ...
               !DYP0  = Pstar2(I,J+1) - Pstar2(I,J)              !zmh
               !OYAYP = RDLATQ  * ( PT(I,J) + PT(I,J+1) )        !zmh
!
               ...
               !GH4   = GHI(I,K+1,J+1) + GHI(I,K,J+1)             !zmh
...
!zmh 11 move Coriolis force
 if( .not. zmh_Coriolis)then
         DO K = beglev ,endlev
            DO I = IB,IE
         ...
         DO K = beglev ,endlev
            DO I = IB,IE
               ...
               FS0V = FOURTH * ( fstar0  + fstar1 )          
               W12 = FS0V*FS0V*DTlin2*DTlin2
               DV(I,K,J) = - (PY(I,K,J) - FSU + FS0V*FS0V*DTlin2*VT(I, K,J))/(1.+W12)
...
! 
               ...
               DU(I,K,J) = - PX(I,K,J) - c10*FSV      
               DV(I,K,J) = - PY(I,K,J) + c10*FSU     
            ENDDO
         ENDDO
...
! 
               ...
               FS0V  = FOURTH * ( FS0   + FS1 +  FS2 + FS3 )

               W11 = FS0*FS0*DTlin2*DTlin2
               W12 = FS0V*FS0V*DTlin2*DTlin2
               DU(I,K,J) = - (PX(I,K,J) + FSV + FS0 *FS0 *DTlin2*UT(I, K,J))/(1.+W11)
               DV(I,K,J) = - (PY(I,K,J) - FSU + FS0V*FS0V*DTlin2*VT(I, K,J))/(1.+W12)
...
!!!!!!!!!!!!!!! test !!!!!!!!!!!!!!!!!!!!!
   DO J  = beglatdyn, endlatdyn
       ...
       W12 = sqrt(geoadpv(J))
         DO K = beglev ,endlev
            DO I = 1, NX !IB,IE
              !!DU(I,K,J) = DU(I,K,J)*W11
              !!DV(I,K,J) = DV(I,K,J)*W12
              !!DT(I,K,J) = DT(I,K,J)*W11
            ENDDO
         ENDDO
         DO I = 1, NX !IB,IE
              !!DPsa(I,J) = DPsa(I,J)*W11
         ENDDO
  ENDDO
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!


     CALL nliter_uvtp(UL, VL, TL, PsaL, PstL, DTlin2, 1)  !ISTAR)   

   ENDIF

  ENDDO  ! IZMH forth and back integration

 ENDDO  ! IZMH2 Embedded substepping
! ===============================================

1002 continue

!zmh 13 not activated
! ADD J == 1 to activate this
!if(zmh_Coriolis)then
if(I<0)then
! add Coriolis force
      ...
      IF (J.EQ.1) THEN    !AT THE NORTH POLAR  
         ...
         DO K = beglev ,endlev
            DO I = IB,IE
               ...
               W11 = FS0V*DTlin
               W12 = W11*W11
               VT(I,K,J) = (VT(I,K,J) + FSU*DTlin ) / (1.+W12)
...
! 
            ...
            ! exact scheme:
               !UT(I,K,J) = COS(FS0*DTlin)*UT(I,K,J) - SIN(FS0*DTlin)*VT2
               !VT(I,K,J) = COS(FSV*DTlin)*VT(I,K,J) + SIN(FSV*DTlin)*UT2

            ! damping scheme
               W11 = FS0*DTlin
               W12 = W11*W11
               UT(I,K,J) = (UT(I,K,J) - FSV*DTlin ) / (1.+W12)
...
!zmh 14
if(zmh_nlitti)then  ! for diagnostic only
      DO J = beglatdyn, endlatdyn
         DO I = 1, NX
            DPsa(I,J)    = ( Psa(I,J) - PsaL(I,J))/DTlin
         END DO
      END DO
      DO J = beglatdyn, endlatdyn
         DO K = beglev,endlev 
            DO I = 1, NX
               DU(I,K,J) = (UT(I,K,J) - UL(I,K,J) )/DTlin
               DV(I,K,J) = (VT(I,K,J) - VL(I,K,J) )/DTlin
               DT(I,K,J) = (TT(I,K,J) - TL(I,K,J) )/DTlin
            END DO
         END DO
      END DO
...
!=============================== zhh =====================================
 ...
 !!                 stop
               ENDIF
!============================ 2007.8.5 ====================================
               DU(I,K,J) = DU(I,K,J) + SU(I,K,J)  !diagnostoc only if(zmh_nlitti)
...
if(zmh_tend1)then  
      ...
      DO J = beglatdyn, endlatdyn
         DO K = beglev,endlev 
            DO I = 1, NX
               UT(I,K,J) = UT(I,K,J) + SU(I,K,J)*DTlin
               VT(I,K,J) = VT(I,K,J) + SV(I,K,J)*DTlin
               TT(I,K,J) = TT(I,K,J) + ST(I,K,J)*DTlin  !
            END DO
         END DO
      END DO
      ENDIF
...
! ******************************** zmh *********************************************
!
!         NO NEED to GO BEYOND THIS POINT IF BALANCED FLOW IS NOT CALCULATED
!                     
! ******************************** zmh ****************************************
!  
!                     Geostrophic adjustments start here
!  
! *****************************************************************************

if(geostrophic_adj)then !!!!!!!!!!!!!!
  
i=1
if(i<0)then
  call fout3d(TB(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'tb')
  call fout3d(TT(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'tt')
  call fout3d(ZMH_TT1(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'tt1')
  call fout3d(GHI0(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'ghi0')
  call fout3d(GHI(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'ghi')
  call fout3d(PX(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'px0')
  call fout3d(PY(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'py0')
  call fout3d(DU(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'du0')
  call fout3d(DV(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'dv0')
endif
...
!-------------------
      ...
      do k=beglev,endlev
         do j=beglatdyn,endlatdyn
            do i=1,NX
              ZMH_DT(I,K,J) = DT(I,K,J)
            end do
            call period(ZMH_DT(1,k,j))
         enddo
      end do

! smooth, because there are zonal bands in DT associated with decomposition due to VT input. Why?


      call mp_send3d( commyz, dest, src, NX,NZ, NY,                     &
                   1, NX,beglev,endlev, beglatdynex, endlatdynex,       &
                   1, NX, beglev,endlev,beglatdyn, beglatdyn,  ZMH_DT )

      call mp_recv3d( commyz, src, NX,NZ,  NY,                             &
                   1, NX,beglev,endlev, beglatdynex, endlatdynex,       &
                   1, NX,beglev,endlev, endlatdynex, endlatdynex, ZMH_DT )


     do j=beglatdyn,endlatdyn
       JP1 = min(J+1,endlatdynex)
       JM1 = max(J-1,beglatdynex)
      do k=beglev,endlev
           do i=IB,IE
                 ZMH_TT0(I,k,J) = (ZMH_DT(I,K,J) + ZMH_DT(I-1,K,J) + ZMH_DT(I+1,K,J)          &
                                + ZMH_DT(I,K,JM1) + ZMH_DT(I-1,K,JM1) + ZMH_DT(I+1,K,JM1)     &
                                + ZMH_DT(I,K,JP1) + ZMH_DT(I-1,K,JP1) + ZMH_DT(I+1,K,JP1) )/9.0
           enddo
       enddo
     enddo
      ...
      do k=beglev,endlev
         do j=beglatdyn,endlatdyn
            do i=IB,IE
              ZMH_DT(I,K,J) = ZMH_TT0(I,K,J) 
            end do
            call period(ZMH_DT(1,k,j))
         enddo
      end do

 ! do hydrostatic integration    
      ...
      IF (J.EQ.1) THEN
         DO K = beglev ,endlev
            DO I = IB,IE
               ...
               !OPCT0 = b0 * TT(IB,K,1) * (1. - deltap(IB,K,1)) / Pstar1(IB,1)
               !OPCT3 = b0 * TT(I,K,JB) * (1. - deltap(I,K,JB)) / Pstar1(I,JB)
               OPCT0 = b0 * ZMH_DT(IB,K,1) * (1. - deltap(IB,K,1)) / Pstar1(IB,1)
               OPCT3 = b0 * ZMH_DT(I,K,JB) * (1. - deltap(I,K,JB)) / Pstar1(I,JB)
!
               PY1   = OYAYP * ( (GHI(I,K+1,JB) + GHI(I,K,JB))    &       
                     -           (GHI(IB,K+1,1) + GHI(IB,K,1)) )          ! (4.4)
               PY2   = RDLATH * ( OPCT0 + OPCT3 ) * DYP0                  ! (4.5)
...
!
         DO K = beglev ,endlev
            DO I = IB,IE
               ...
               OYAYP = RDLATQ  * ( PT(I,J) + PT(I,J+1) )
!
               OPCT0 = b0 * ZMH_DT(I  ,K,J) * (1. - deltap(I  ,K,J)) / Pstar1(I  ,J)
               OPCT2 = b0 * ZMH_DT(I-1,K,J) * (1. - deltap(I-1,K,J)) / Pstar1(I-1,J)
               OPCT3 = b0 * ZMH_DT(I,K,J+1) * (1. - deltap(I,K,J+1)) / Pstar1(I,J+1)
               OPCT4 = b0 * ZMH_DT(I+1,K,J) * (1. - deltap(I+1,K,J)) / Pstar1(I+1,J)
               OPCT5 = b0 * ZMH_DT(I-2,K,J) * (1. - deltap(I-2,K,J)) / Pstar1(I-2,J)
               GH0   = GHI(I  ,K+1,J) + GHI(I  ,K,J)
               GH1   = GHI(I-1,K+1,J) + GHI(I-1,K,J)
               GH2   = GHI(I+1,K+1,J) + GHI(I+1,K,J)
               GH3   = GHI(I-2,K+1,J) + GHI(I-2,K,J)
...
! ------------------------ COMPUTE GEOSTROPHIC WINDS ZMH_DU, ZMH_DV ------------------------------------
#if (defined SPMD)

   call mp_send3d( commyz, dest, src, NX,NZ, NY,                     &
                   1, NX,beglev,endlev, beglatdynex, endlatdynex,       &
                   1, NX, beglev,endlev,beglatdyn, beglatdyn,  PX )

   call mp_recv3d( commyz, src, NX,NZ,  NY,                             &
                   1, NX,beglev,endlev, beglatdynex, endlatdynex,       &
                   1, NX,beglev,endlev, endlatdynex, endlatdynex, PX)

   call mp_send3d( commyz, dest, src, NX,NZ, NY,                     &
                   1, NX,beglev,endlev, beglatdynex, endlatdynex,       &
                   1, NX, beglev,endlev,beglatdyn, beglatdyn,  PY )

   call mp_recv3d( commyz, src, NX,NZ,  NY,                             &
                   1, NX,beglev,endlev, beglatdynex, endlatdynex,       &
                   1, NX,beglev,endlev, endlatdynex, endlatdynex, PY)
      ...
      IF (J.EQ.1) THEN    !AT THE NORTH POLAR  
         DO K = beglev ,endlev
            DO I = IB,IE
               ...
               W11     = max(fstar0+fstar1, 2.0e-4)
               ZMH_DU(I,K,JB) = ( PY(I,K,JB) + PY(I-1,K,JB) - 2.* fac_ageo * SV(I,K,JB))*2./ c10 / W11 
...
!         
         ...
         DO K = beglev ,endlev
            DO I = IB,IE
               ...
               FS0  = FF(J  ) + CUR(J  ) * U(I  ,K,J)*0.0 ! zmh 2018-09-12
               FS1  = FF(J  ) + CUR(J  ) * U(I+1,K,J)*0.0
               FS2  = FF(JP1) + CUR(JP1) * U(I  ,K,JP1)*0.0
               FS3  = FF(JP1) + CUR(JP1) * U(I+1,K,JP1)*0.0

               WPJ = FS0 

               W11 = zmh_nth(J) * max(WPJ, 1.0e-4) + zmh_sth(J) *  min(WPJ, -0.9999e-4)

               ZMH_DU(I,K,J) =   (PY(I,K,J) + PY(I-1,K,J) +PY(I,K,JM1)+PY(I-1,K,JM1) )/c10/4./W11    

               WPJ = FS0 + FS1 + FS2 + FS3
               W11 = zmh_nth(J) * max(WPJ, 4.0e-4) + zmh_sth(J) *  min(WPJ, -3.9999e-4)

               ZMH_DV(I,K,J) = - (PX(I,K,J) + PX(I+1,K,J) +PX(I,K,JP1)+PX(I+1,K,JP1) )/c10/W11 
...
!
      ...
      DO K = beglev ,endlev
        CALL PERIOD(ZMH_DU(1,K,J) )
        CALL PERIOD(ZMH_DV(1,K,J) )
      ENDDO

    ENDDO !J index

  if(beglatdyn .le. 2)then
       J   = 1
       JP1 = 2
         DO K = beglev ,endlev
            DO I = 1 ,NX
               ZMH_DU(I,K,J)= 0.0
    !           ZMH_DU(I,K,JP1)= DU(I,K,JP1+1)  !assuming at least 3 latitudes include north pole
            ENDDO
         ENDDO
   endif      

   if (myid_y.eq.(npr_y-1)) then   !North pole
       DO J=1,3                    !hard-wired
        !W11 = (DBLE(J)-0.5)/3.5
        W11 = (DBLE(J)-0.0)/3.0
         DO K = beglev ,endlev
            DO I = 1 ,NX
               ZMH_DV(I,K,J)= ZMH_DV(I,K,4)*W11
            ENDDO
         ENDDO
       ENDDO
   end if

   if (myid_y.eq.0) then           !South pole
       DO J=NY-3,NY-1
         !W11 = (DBLE(NY-J+1)+0.5)/3.5
         W11 = (DBLE(NY-J+1)+0.0)/3.0
         DO K = beglev ,endlev
            DO I = 1 ,NX
               ZMH_DV(I,K,J)= ZMH_DV(I,K,NY-4)*W11
            ENDDO
         ENDDO
       ENDDO
...
! =============== 
      ...
      call mp_send3d( commyz, dest, src, NX,NZ, NY,                     &
                   1, NX,beglev,endlev, beglatdynex, endlatdynex,       &
                   1, NX, beglev,endlev,beglatdyn, beglatdyn,  ZMH_DU )

      call mp_recv3d( commyz, src, NX,NZ,  NY,                             &
                   1, NX,beglev,endlev, beglatdynex, endlatdynex,       &
                   1, NX,beglev,endlev, endlatdynex, endlatdynex, ZMH_DU )

      call mp_send3d( commyz, dest, src, NX,NZ, NY,                     &
                   1, NX,beglev,endlev, beglatdynex, endlatdynex,       &
                   1, NX, beglev,endlev,beglatdyn, beglatdyn,  ZMH_DV )

      call mp_recv3d( commyz, src, NX,NZ,  NY,                             &
                   1, NX,beglev,endlev, beglatdynex, endlatdynex,       &
                   1, NX,beglev,endlev, endlatdynex, endlatdynex, ZMH_DV )
     ...
     do j=beglatdyn,endlatdyn
      ...
      do k=beglev,endlev
           do i=IB,IE
                 ZMH_TT0(I,k,J) = (ZMH_DU(I,K,J) + ZMH_DU(I-1,K,J) + ZMH_DU(I+1,K,J)          &
                                + ZMH_DU(I,K,JM1) + ZMH_DU(I-1,K,JM1) + ZMH_DU(I+1,K,JM1)     &
                                + ZMH_DU(I,K,JP1) + ZMH_DU(I-1,K,JP1) + ZMH_DU(I+1,K,JP1) )/9.0

                 ZMH_TT1(I,k,J) = (ZMH_DV(I,K,J) + ZMH_DV(I-1,K,J) + ZMH_DV(I+1,K,J)          &
                                + ZMH_DV(I,K,JM1) + ZMH_DV(I-1,K,JM1) + ZMH_DV(I+1,K,JM1)     &
                                + ZMH_DV(I,K,JP1) + ZMH_DV(I-1,K,JP1) + ZMH_DV(I+1,K,JP1) )/9.0
           enddo
       enddo
     enddo
      ...
      do k=beglev,endlev
         do j=beglatdyn,endlatdyn
            do i=IB,IE
              ZMH_DU(I,K,J) = ZMH_TT0(I,K,J)
              ZMH_DV(I,K,J) = ZMH_TT1(I,K,J)
            end do
            call period(ZMH_DU(1,k,j))
            call period(ZMH_DU(1,k,j))
         enddo
...
i=1
if(i<0)then
  call fout3d(PX(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'px')
  call fout3d(PY(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'py')
  call fout2d(PT(IB:IE, beglatdyn:endlatdyn)                ,NX2,    NY2, IB,  beglatdyn, 'pt')
  call fout2d(PSA(IB:IE, beglatdyn:endlatdyn)                ,NX2,    NY2, IB,  beglatdyn, 'psa')
  call fout3d(UT(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'ut')
  call fout3d(VT(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'vt')
  call fout3d(TT(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'tt')

  call fout2d(DPSA(IB:IE, beglatdyn:endlatdyn)                ,NX2,    NY2, IB,  beglatdyn, 'dpsa')
  call fout3d(DU(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'du')
  call fout3d(DV(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'dv')
  call fout3d(DT(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'dt')
  call fout3d(ZMH_DU(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'dug')
  call fout3d(ZMH_DV(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'dvg')
  call fout3d(ZMH_DT(IB:IE, beglev:endlev, beglatdyn:endlatdyn) ,NX2,NZ2,NY2, IB, beglev, beglatdyn, 'dtg')
endif

   DO J  = beglatdyn, endlatdyn
     !write(*,*)'J,geoadpu(J)',J,geoadpu(J),geoadpv(J)
      DO K = beglev ,endlev
         DO I = 1 ,NX
             
               DU(I,K,J)= geoadpu(J)*DU(I,K,J) + (1.0-geoadpu(J))*ZMH_DU(I,K,J)
               DV(I,K,J)= geoadpv(J)*DV(I,K,J) + (1.0-geoadpv(J))*ZMH_DV(I,K,J)
               DT(I,K,J)= geoadpu(J)*DT(I,K,J) + (1.0-geoadpu(J))*ZMH_DT(I,K,J)

         ENDDO
      ENDDO

         DO I = 1 ,NX
               DPsa(I,J)= geoadpu(J)*DPsa(I,J) 
         ENDDO
   ENDDO        
...
if(zmh_nlitti)then
   DO J  = beglatdyn, endlatdyn
      DO K = beglev ,endlev
         DO I = 1 ,NX
               UT(I,K,J)= UL(I,K,J) + DU(I,K,J)*DTlin
               VT(I,K,J)= VL(I,K,J) + DV(I,K,J)*DTlin
               TT(I,K,J)= TL(I,K,J) + DT(I,K,J)*DTlin
         ENDDO
      ENDDO
         DO I = 1 ,NX
               Psa(I,J)= PsaL(I,J) * DPsa(I,J)*DTlin
         ENDDO
   ENDDO        
endif
!zmh 16

 endif !(geostrophic_adj)

!------------------------------------------------------------------------------
!                     Geostrophic adjustments finished
!=============================== zmh =====================================
!
1001 continue
