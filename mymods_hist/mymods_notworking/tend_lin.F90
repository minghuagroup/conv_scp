...
! Modified: Zhang Minghua, 2018-08-15, added geostrophic adaption to reduce inertial gravity waves
...
!------------------------------------------------------------------------------------------------
   logical               :: ite_first                  ! first step for y integration  
   logical               :: zmh_Coriolis    = .false.  !.False. for default, .True. for geos adjustments
...
!zmh
   ...
   integer  :: JM1,JP1,KP1,KM1, IZMH, NZMH, IZMH2, NZMH2 !zmh                 ! loop index
...
! ZMH SUBSETEPPING and COMBINATION OF FORWARD/BACKWARD INTEGRATION STEPS
!----------------------------------------------------------------------------------------------

   NZMH2 = 2  !embedded substepping in X
...
!zmh 11 move Coriolis force
 if( .not. zmh_Coriolis)then
         ...
         DO K = beglev ,endlev
            DO I = IB,IE
               DV(I,K,1) = - PY(I,K,1)  !Coriolis force moved
               DU(I,K,1) = ZERO
            ENDDO
         ENDDO
  endif
...
! 
         ...
         DO K = beglev ,endlev
            DO I = IB,IE
               DU(I,K,J) = - PX(I,K,J)        ! zmh Corilois force moved outside the loop  
               DV(I,K,J) = - PY(I,K,J) 
            ENDDO
         ENDDO
  endif
...
! 
               ...
               VT2  = FOURTH * (VT(I,K,  J) + VT(I-1,K,  J)                      &
                    +             VT(I,K,J-1) + VT(I-1,K,J-1) )           
               UT2  = FOURTH * (  UT(I,K,J)   +  UT(I+1,K,J)                      &
                    +             UT(I,K,J+1) +  UT(I+1,K,J+1) )          ! (4.6)


            ! exact scheme:
               !UT(I,K,J) = COS(FSU*DTlin)*UT(I,K,J) - SIN(FSU*DTlin)*VT2
               !VT(I,K,J) = COS(FSV*DTlin)*VT(I,K,J) + SIN(FSV*DTlin)*UT2

            ! damping scheme
               W11 = FSU*DTlin
               W12 = W11*W11
               UT(I,K,J) = (UT(I,K,J) - W11*VT2 ) / (1.+W12)

               W11 = FSV*DTlin
               W12 = W11*W11
               VT(I,K,J) = (VT(I,K,J) + W11*UT2 ) / (1.+W12)


            END DO
            call period( UT(1,K,J) )
