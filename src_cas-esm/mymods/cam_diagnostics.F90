...
#ifdef wrf
   diag_physvar_ic,    &
   solve_rh_psl         ! output rh and psl, juanxiong he
#else
   diag_physvar_ic
#endif
...
subroutine diag_init
  ...
  ! Declare the history fields for which this module contains outfld calls.
   ...
   ! yhy
   call addfld ('PRECDP'  ,'m/s     ',1,    'A','Deep Convective precipitation rate (liq + ice)' ,phys_decomp)
   call addfld ('PMID      ','Pa      ',pver,    'A','Mid Pressure',phys_decomp)
   call addfld ('PDEL      ','Pa      ',pver,    'A','Thickness in Pressure',phys_decomp)
   call addfld ('ZDEL      ','m       ',pver,    'A','Thickness in m',phys_decomp)
...
!wangty modify
   ...
   ! juanxiong he
   !---------------------------------------------
   call addfld ('USTARV  ','m/s     ',1,    'A','U* over the sea',phys_decomp)
   call addfld ('FVV     ','m/s     ',1,    'A','U* over the land',phys_decomp)
   call addfld ('REV     ','m/s     ',1,    'A','resistence over the sea',phys_decomp)
   call addfld ('RAMV1   ','m/s     ',1,    'A','resistance over the land',phys_decomp)
#endif
...
!wangty modify
   ...
   ! juanxiong he
   !---------------------------------------------
   call add_default ('USTARV  ', 1, ' ')
   call add_default ('REV     ', 1, ' ')
   call add_default ('FVV     ', 1, ' ')
   call add_default ('RAMV1   ', 1, ' ')
#endif
...
!wangty modify
#ifdef wrf
          state%psl(:ncol) = psl_tmp(:ncol)  ! added by juanxiong he
#endif
       end if
    end if
!
...
!wangty modify
    ...
    ! juanxiong he
    !-----------------------------------------------------
    call outfld('USTARV', cam_in%ustar,     pcols, lchnk)
    call outfld('REV',    cam_in%re,     pcols, lchnk)
    call outfld('FVV',    cam_in%fv,     pcols, lchnk)
    call outfld('RAMV1',  cam_in%ram1,     pcols, lchnk)
#endif
! 
...
#ifdef wrf
    !-----------------------------------------------------
    ! added by juanxiong he for export rhref
    !-----------------------------------------------------
       cam_in%rhref(:ncol) = ftem(:ncol)
!       do i=1,ncol
!         if(cam_in%rhref(i).gt.100.0.or.cam_in%rhref(i).lt.0) then
!          print *,cam_in%rhref(i)
!         end if
!       end do
#endif
...
! Arguments
!
   type(physics_state), dimension(begchunk:endchunk), intent(inout) :: state
...
!
 do lchnk = begchunk,endchunk
    ...
    ftem(:ncol,:) = state(lchnk)%q(:ncol,:,1) * state(lchnk)%pdel(:ncol,:) * rga
    ...
    ! Relative humidity
       call aqsat (state(lchnk)%t    ,state(lchnk)%pmid  ,tem2    ,ftem ,pcols   , &
          ncol ,pver  ,1       ,pver    )
       ftem(:ncol,:) = state(lchnk)%q(:ncol,:,1)/ftem(:ncol,:)*100._r8
       state(lchnk)%rh(:ncol,:) = ftem(:ncol,:)  ! added by juanxiong he

!
! Sea level pressure
!
    call cpslec (ncol, state(lchnk)%pmid, state(lchnk)%phis, state(lchnk)%ps, state(lchnk)%t, psl_tmp, gravit, rair)
    state(lchnk)%psl(:ncol) = psl_tmp(:ncol)  ! added by juanxiong he

    enddo

    return
    end subroutine solve_rh_psl
#endif
